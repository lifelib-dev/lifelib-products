...
**Status:** Draft, 2026-08-20. Current coverage: United States (6 individual life and
6 individual annuity product types), United Kingdom (7 product types, including pension
annuities) and Japan (9 product types, three of them 第三分野 health products).
**All twenty-eight products ship an executable reference model** in
`<library>/products/<product>/`, beside the documents that specify it — twelve in
`lifelib/libraries/uslib/`, seven in `lifelib/libraries/uklib/` and nine in
`lifelib/libraries/jplib/`, each one reproducing its own technical notes' worked example,
asserted cell by cell. All three are lifelib libraries in shape: they sit at the path
...
lifelib/libraries/                   the path lifelib keeps its libraries on, and so does this
  <library>/                         uslib/, uklib/, jplib/ — named for the lifelib library each becomes
...
All three libraries are in this shape and ready to merge into lifelib — see
...
That is 235 pages across the three libraries, and it should report zero. Run the model
suites with:
...
|---|---|---|
| `[S#]` | bracketed text | Fact taken from a primary product document (brochure, specimen policy, prospectus, producer guide, 約款, 契約締結前交付書面) listed in the product's `sources.md` |
...
1. **Research** — per product type, collect primary product documentation (insurer
   ...
   公的 statistical series (患者調査, 全国がん登録, 介護保険事業状況報告) for Japan. Everything
...
Scope note: all three libraries cover individual business, but what "individual life
...
frequency x severity x limit rather than a sum assured. Group insurance, 共済 (cooperative
insurance) and institutional business (bulk purchase annuities, pension risk transfer) are
out of scope in all three.
