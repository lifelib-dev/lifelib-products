# Critical Illness Cover — Representative Product Specification (United Kingdom)
...
references), both numbered per `uk/_research/critical-illness.md` and resolved against
...
`uk/products/term-assurance/` (companion product folder). Term-chassis provisions —
...
critical illness ("Life Cover and Critical Illness", Zurich [S11]; "Life or Critical
Illness Cover", Royal London [S8]; joint-life cover on a first-event basis [S11]); and
(b) **standalone** — CI only, with no death benefit (Zurich's standalone CIC pays only
...
business, not these contracts) [REG-R17]. At the policyholder level the L&G retail
