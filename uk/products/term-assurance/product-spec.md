# Term Assurance — Representative Product Specification (United Kingdom)
...
references), both numbered per `uk/_research/term-assurance.md` and resolved in
`sources.md` (same directory; numbering frozen, never renumbered), and [REG-R#] (the
cross-product reference library `uk/references/regulatory-and-actuarial-references.md`,
whose own R-numbering is distinct; research provenance in
`uk/_research/regulatory-actuarial.md`) — were extracted from the cited document. Values
...
composite is drawn from three insurers' current retail products: Legal & General
(Life Insurance / Decreasing Life Insurance / Increasing Life Insurance / Family and
Personal Income Plan) [S1]–[S5], Aviva Life Insurance+ [S6][S7], and Royal London
Personal Menu Plan life cover [S8][S9].
...
| Residence at outset | UK resident (L&G: living in the UK ≥ 183 days in the last tax year; Aviva also admits Channel Islands/Isle of Man/Gibraltar) | [S1][S6] |
...
1. Packaging varies: Aviva writes one policy with three payout bases [S6][S7]; Royal
   London a menu plan with five payout shapes (level/increasing/decreasing lump sum,
   level/increasing income) [S8]; L&G sells the shapes as separate products (LI, DLI,
   ILI, FPIP) [S2]. The composite treats benefit shape as a model-point parameter with
...
2. Joint life first death is the standard joint basis across all three insurers
   [S1][S2][S6]. Royal London additionally writes dual life and joint life second event
   [S9]; both are excluded from the composite.
3. Envelope = the L&G/Aviva limits, which agree closely (entry to 77, expiry by 90,
   terms 1–50 [S2][S3][S7]); FIB limits per L&G FPIP (entry to 64, expiry by 70, terms
   5–40 [S2][S4]). Royal London is materially wider (entry 18–88, expiry to 89, terms
   1–72 [S9]) and is treated as an outlier. L&G's minimum-expiry-age-29 rule [S2][S3]
   is L&G-specific and not carried into the composite. Terminal illness cover is not
   provided on L&G's 1-year (ILI 2-year) minimum terms [S2]; TIC automatic for terms
   of 2+ years [S4].
4. L&G publishes £10m (level/decreasing), £4m increasing, £10,000/month FPIP (£4,000
   with CI) [S2][S4]; Aviva publishes no monetary maximum [S6]; Royal London is
   unlimited (£5m cap with the increasing option) [S9].
5. Only L&G publishes a value ("Cover from only £5 a month", "Fixed premiums from £5 a
   month") [S5]; Aviva references a "minimum premium limit" without a public value
   [S6]; the often-quoted market range of £5–£10/month is [unverified].
...
| Payment method | Direct debit, in sterling (Aviva: from a UK, Channel Islands, Isle of Man or Gibraltar bank account) | [S6] |
...
| Annual-mode refund at claim | L&G refunds the remaining months' premium in the policy year on a full-cover claim; not modeled | [S1]; scope **[std]** (7) |

7. Monthly direct debit is the dominant retail mode (Aviva requires direct debit
   [S6]); the composite standardizes on monthly and ignores the annual-mode claim-time
   refund [S1] and Aviva's deduction of grace-window unpaid premiums from claims [S6]
   as immaterial modal refinements.
8. Disclosed rating factors: Aviva lists age, occupation, health, lifestyle, smoking
   habits, type and amount of cover [S7]; FCA adds that reinsurers shape risk-based
   adjustments [R8]. None of the three insurers lists sex as a rating factor, but the
   gender-neutral pricing requirement itself was not confirmed from a fetched document
   [unverified]. No insurer publishes premium rate tables — the technical notes
   specify a [std] mortality proxy basis from public tables and take the office
   premium itself as a model-point input (no premium-rate table is constructed).
...
9. Observed: L&G — the client chooses the decreasing rate from 5%, 7%, 8% or 10% [S4],
   the rate appearing in the policy schedule [S1]; Royal London — default schedule at a
   yearly rate of 6%, or a chosen rate in 0%–15% where the mortgage-interest-rate
   feature applies [S8][S9]; Aviva — a fixed rate set at application and shown in the
   schedule, value/range not published [S6][S7]. 6% is chosen as the representative
   default because it is the only observed insurer *default* [S8] and sits inside both
   the L&G menu range (5–10%) and the RL selectable range. L&G and Royal London warn
   that cover may not repay the mortgage if the actual loan rate exceeds the schedule
   rate [S1][S3][S8].
10. FIB benefit is expressed as a monthly amount [S6]; L&G FPIP caps it at
    £10,000/month (£4,000 with CI) [S2]. £1,000/month is a round-number modeling value
    (£12,000/year, same order as the anchor lump-sum cell over a mid-length run-off).
...
11. Observed indexation bases: L&G — RPI, no increase if the change is below 1%, cover
    cap 10%, premium × 1.5 capped 15% [S1][S2]; Aviva — RPI (measured over the 12
    months ending 12 weeks before the anniversary month) capped 10% with premium × 1.5
    capped 15%, or fixed 3%/5%, RPI ≤ 0% → no change [S6][S7]; Royal London — RPI with
    a minimum of 2% and maximum of 10%, or fixed 2–5%, premium × 1.2 [S8][S9]. The
    composite takes RPI/10%-cap/×1.5/15%-cap (the L&G–Aviva mode), the Aviva ≤ 0%
    floor, and the 3-consecutive-declines removal rule (L&G and Aviva; RL removes
    after 2 [S8]). Aviva's FIB-shape escalation uniquely carries no premium increase
    [S6]; RL income shapes increase premiums × 1.2 [S8]; both excluded.
12. Observed GIO caps: L&G — lower of 100% of original cover and £200,000, events
    exercised within 6 months, not after age 55 [S1] (FPIP variant capped at
    £1,400/month [S2]); Aviva "life change benefit" — total across exercises lower of
    original cover and £200,000 (FIB: £8,000/year equivalent), new policy within 180
    days of the event, repeatable until 55 [S6]; Royal London "cover increase options"
    — lowest of half the original cover and £200,000 (income covers: £10,000/year)
    [S8]. The composite takes the £200,000/100% cap and 6-month window; all three
    implement the increase as a separate policy at then-current rates [S1][S6][S8].
13. Observed WOP: L&G — 26 consecutive weeks of incapacity before waiver;
    own-occupation, or 3-of-6 specified work tasks if not in paid work [S1][S2];
    Aviva — deferred period per the policy schedule; own-occupation, or 2-of-6 work
    tasks where work stopped > 12 months before; claims to age 71 [S6][S7]; Royal
    London — no WOP exists on the plan (zero occurrences in the 84-page plan details)
    [S8]. The composite includes WOP as an optional rider with the L&G 26-week
    deferred period (the only concretely published deferral).
...
14. Royal London's suicide clause runs "from the date cover started or restarted"
...
(L&G frames it as the "first year" [S1]; Royal London runs it from start or restart
...
dissolution or mortgage change without full underwriting) exist at L&G and Aviva
[S1][S6] but are out of scope, as is L&G's replacement-cover option for the surviving
life after a first-death claim [S1].
...
contains a renewal or conversion option, and L&G's old conversion option is
...
premiums) [S2][S6][S8]; Aviva fracture cover and global treatment [S6]; children's
covers; free pre-completion covers (L&G free life cover ≤ £300,000 / accidental death
benefit ≤ £300,000 [S2]; Aviva house purchase cover ≤ £500,000 [S6]); joint-policy
separation and replacement options [S1][S6]; Royal London's mortgage repayment
guarantee (pays the actual outstanding mortgage balance rather than the notional
schedule) [S8]; dual life and joint life second event bases [S9]; renewable/
convertible term (not present in any fetched current product [S2][R8]); Aviva
commutation of FIB during payment is described contractually but not exercised in the
base model [S6].
...
1. **Packaging.** Separate single-shape products (L&G [S2]) vs one policy with three
   payout bases (Aviva [S6]) vs a menu plan with five shapes including income options
   (Royal London [S8]). Composite: one chassis, benefit shape as a parameter — the
   Aviva structure, which maps cleanly to a model-point field.
2. **Age/term envelope.** L&G and Aviva agree closely (entry to 77, expiry by 90,
   terms 1–50) [S2][S3][S7]; Royal London is much wider (18–88, expiry 89, terms to
   72) [S9]. Composite: the L&G/Aviva envelope — two of three insurers, and the
   tighter, more typical bounds.
3. **Decreasing schedule rate.** Client-picked menu 5/7/8/10% (L&G [S4]) vs default
   6% or chosen 0–15% (RL [S8][S9]) vs fixed rate in the schedule, value unpublished
   (Aviva [S6][S7]). Composite: client-selected with 6% default (footnote 9). Royal
   London's actual-balance mortgage repayment guarantee [S8] is a distinctive design
   and excluded.
4. **FIB premium under escalation.** Aviva escalating FIB instalments carry no
   premium increase [S6]; RL income shapes increase premiums × 1.2 [S8]; L&G FPIP
   increases premiums × 1.5 [S2]. Composite: FIB escalation excluded entirely —
   the variation is too wide to standardize honestly.
5. **Indexation loading.** Premium multiplier 1.5 (L&G [S1][S2], Aviva [S6][S7]) vs
   1.2 (RL [S8]); RPI floor: <1% no increase (L&G [S1]) vs ≤0% (Aviva [S6]) vs 2%
   minimum applied (RL [S8]). Composite: ×1.5 (the mode) with the Aviva floor.
6. **GIO caps.** 100% of original cover (L&G [S1], Aviva [S6]) vs 50% (RL [S8]);
   all cap at £200,000. Composite: 100%/£200,000.
7. **Waiver of premium.** 26-week deferral, 3-of-6 tasks fallback (L&G [S1]) vs
   schedule-set deferral, 2-of-6 tasks (Aviva [S6]) vs not offered at all (RL [S8]).
   Composite: optional rider, 26-week deferral — the published mode; RL shows WOP is
   not universal.
8. **Suicide clause.** Year one (L&G [S1]) vs 12 months (Aviva [S6]) vs 12 months
   from start or restart (RL [S8]). Substantively identical; composite: 12 months.
9. **Minimum premium.** £5/month published (L&G [S5]) vs unpublished minimum premium
   limits (Aviva [S6], RL). Composite: £5/month, the only public value.
10. **What does not vary.** Guaranteed level premiums for life-only cover, embedded
    ...
    L&G and Aviva [S1][S6] (not extracted from the Royal London plan details). These
    are the invariant core of the composite.
...
deliberate/reckless vs careless misrepresentation) [REG-R20], which the L&G policy
remedies mirror contractually [S1]. FSCS protection is 100% of a valid claim with no
upper limit [S1].

**Tax.** Protection policies are written to satisfy the qualifying-policy conditions
(the L&G conditions reference para 19(3) of Schedule 15 to ICTA 1988 for option
