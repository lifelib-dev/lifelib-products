...
value is exhausted [R2 VM-01 definition; S2, S4, S9]. The owner elects a guarantee
...
schedule) [R1 AG 38 8E Designs #1/#2; R2; S4]. Flagship lifetime-GUL products use
...
2. Guaranteed shadow credits are contractual internal parameters, distinct from the
   base credited rate [R2 shadow-account definition; S4 mechanics]; AG 38 8E caps them
...
1. **Guarantee mechanism.** Shadow accounts dominate flagship lifetime GUL
   ...
   multi-charge-set variants punitively [R1 Designs #1–#3; R2]. **Choice:** single
...
guaranteed shadow credits at a Moody's-based index + 3%. [R1; REG-R6; REG-R7]
...
framework before state adoption of Model 787. [R6; REG-R11; REG-R12]
...
design attribution [unverified]]. The 2021 change to dynamic "insurance interest
rates" (2% transition rate for 2021) materially affects GUL premium/corridor limits
[R4; REG-R13]. Heavy prefunding (single-pay/short-pay, the 1035 "sweet spot" [S2])
