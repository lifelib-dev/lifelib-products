...
:mod:`~DeferredIncomeAnnuityUS.Projection` as ``data``.
:mod:`~DeferredIncomeAnnuityUS.Projection` is parameterized by ``point_id``, so each
...
``us/models/deferred-income-annuity/``, rather than data stored inside the model. The
...
:func:`~DeferredIncomeAnnuityUS.Projection.premium_by_mth` collapses it to a
