...
"""The by-contract projection of the :mod:`~DeferredIncomeAnnuityUS` model.
...
1-based ``t`` of :mod:`TermLifeUS` and :mod:`ImmediateAnnuityUS`, and the reason is that
...
- ``lives_if(t, life)`` is the probability of being alive at the **start** of month
  ...
  That is the *same* function of elapsed time as ``ImmediateAnnuityUS.lives_if``, so the
...
``DeferredIncomeAnnuityUS`` folder without its parent's CSVs produces a model that reads
and then fails on first evaluation.

Each table has a filename Reference and a reader Cells, both on
:mod:`~DeferredIncomeAnnuityUS.Data`:
...
follow :mod:`ImmediateAnnuityUS` **exactly**, because the payout phase of a DIA *is* a
...
``payment_factor_life``, exactly as in :mod:`ImmediateAnnuityUS`. ``L`` is *also* the
...
:mod:`ImmediateAnnuityUS`, because a DIA's income level changes when a *premium* is paid
...
``ImmediateAnnuityUS`` writes ``premium_pp()``, this model writes
...
def surv_to_payout():
    ...
    **The one place the payout chassis cannot be carried across unexamined.**  A SPIA
    is already in payment, so ``ImmediateAnnuityUS`` weights its certain floor by
...
def liability_cf(t):
    ...
    That makes this product differ from the :mod:`~ImmediateAnnuityUS` payout chassis it
...
def net_cf(t):
    ...
    The sign convention of ``TermLifeUS.net_cf``.  Unlike the immediate-annuity chassis
