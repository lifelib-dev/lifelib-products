...
:mod:`~DeferredIncomeAnnuityUS` is the executable counterpart of
...
:mod:`ImmediateAnnuityUS`, carrying the same cells names. The DIA-specific change is the
...
:mod:`~DeferredIncomeAnnuityUS.Projection`
    ...
    Reference, which resolves to the single :mod:`~DeferredIncomeAnnuityUS.Data` Space.
...
deliberate departure from the 1-based ``t`` of :mod:`TermLifeUS` and
:mod:`ImmediateAnnuityUS`: the notes' income start month ``T = 240`` and premium months
...
elapsed months" and carries the same meaning as in :mod:`ImmediateAnnuityUS`; what does
