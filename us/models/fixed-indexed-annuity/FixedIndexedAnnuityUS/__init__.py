...
:mod:`~FixedIndexedAnnuityUS` is the executable counterpart of
...
:mod:`FixedDeferredAnnuityUS`: the surrender-benefit composition order and the NAIC
...
model uses the chassis name :func:`~FixedIndexedAnnuityUS.Projection.mgsv_pp`
throughout.

**Spaces.** The model contains two:

:mod:`~FixedIndexedAnnuityUS.Data`
    Reads the seven input CSVs and holds their filename References. It takes no
    parameters, so each file is read **once per model**.

:mod:`~FixedIndexedAnnuityUS.Projection`
    ...
    Reference, which resolves to the single :mod:`~FixedIndexedAnnuityUS.Data` Space.
...
the contrast with :mod:`FixedDeferredAnnuityUS`, whose ``t`` counts **months**: that
...
library-wide convention set by :mod:`TermLifeUS` and ``savings.CashValue_SE``, and it is
