...
"""The by-contract projection of :mod:`~FixedIndexedAnnuityUS`.
...
``FixedIndexedAnnuityUS`` folder without its parent's CSVs produces a model that reads
and then fails on first evaluation.

The readers and the filename References live on the sibling
:mod:`~FixedIndexedAnnuityUS.Data` Space, reached here through the ``data`` Reference, so
...
:mod:`FixedDeferredAnnuityUS`, whose ``t`` counts months. The library's product
...
``age(t - 1)``. This differs from :mod:`TermLifeUS` and :mod:`FixedDeferredAnnuityUS`,
...
Cells names follow :mod:`FixedDeferredAnnuityUS` — the deferred annuity chassis this
...
weight carried by that same row's cash flows — :mod:`TermLifeUS` has
...
period as in :mod:`TermLifeUS` and :mod:`FixedDeferredAnnuityUS`. The notes define it that
...
``shock_lapse_rate`` follows :mod:`TermLifeUS`.

**d and c collide across the library.** ``d`` is the performance-trigger rate in these
notes, deaths in :mod:`TermLifeUS` and the floor's withdrawal deduction on the chassis;
...
``BasicTerm_S.pols_maturity`` and the construction follows :mod:`TermLifeUS`.
"""
...
def age(t):
    ...
    with :mod:`TermLifeUS` and :mod:`FixedDeferredAnnuityUS`, where ``age(t)`` is the age
...
def wd_free_pp(t):
    ...
    The chassis name for this quantity (``FixedDeferredAnnuityUS``, ``UniversalLifeUS``,
    ``RegisteredIndexLinkedAnnuityUS``), so the library spells the free-allowance portion
...
def pols_if(t):
    ...
    The library convention, set by :mod:`TermLifeUS` and ``savings.CashValue_SE``: this is
