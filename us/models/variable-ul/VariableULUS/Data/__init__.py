...
:mod:`~VariableULUS.Projection` as ``data``. :mod:`~VariableULUS.Projection` is
...
``us/models/variable-ul/``, rather than data stored inside the model. The model folder
...
:func:`~VariableULUS.Projection.inv_return_mth`, so the table holds gross returns only.
