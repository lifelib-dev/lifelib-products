...
:mod:`~VariableULUS` is the executable counterpart of
...
``us/products/universal-life/technical-notes.md``, and :mod:`~VariableULUS` follows
:mod:`UniversalLifeUS` name for name wherever the two products share a concept. Where
...
* **Charges are quoted on the initial face amount.** The per-$1,000 monthly charge and
  the surrender charge both scale with ``F_0``, so :func:`~VariableULUS.Projection.units`
  takes no ``t``.
...
:func:`~VariableULUS.Projection.withdrawals` in its own ``withdrawals`` column of
``result_cf()``; :func:`~VariableULUS.Projection.claims` covers deaths and surrenders
...
double counts. The model projects the gross view in :func:`~VariableULUS.Projection.claims`
and derives the general-account view arithmetically from the same run in
:func:`~VariableULUS.Projection.result_net`; ``check_net_view()`` pins the two together.
...
* The grace and default cascade. The trigger is a complete formula and is implemented
  as :func:`~VariableULUS.Projection.is_default`, but the notes lapse the policy "at the
  ...
  a first premium; :func:`~VariableULUS.Projection.is_shortfall` and
  :func:`~VariableULUS.Projection.first_shortfall_month` are the companion diagnostics
...
* New policy loans, repayments and the loan value limit. Loan *interest accrual* on an
  ...
  is given, so :func:`~VariableULUS.Projection.loan_bal_pp` only rolls the model point's
  opening debt forward and :func:`~VariableULUS.Projection.la_pp` only rolls its
  collateral forward.
...
* Riders generally. :func:`~VariableULUS.Projection.rider_charge_pp` is the notes'
...
example, exactly as ``TermLifeUS`` switches conversion off for the same reason.
...
``TermLifeUS`` uses for its ``M(1)`` divergence; neither reading is "correct", and
