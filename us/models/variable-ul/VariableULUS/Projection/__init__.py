...
"""The by-policy monthly projection of the :mod:`~VariableULUS` model.
...
directory, ``us/models/variable-ul/``, read at run time rather than stored inside the
...
``VariableULUS`` folder without its parent's CSVs produces a model that reads and then
fails on first evaluation.

The readers live in the unparameterized :mod:`~VariableULUS.Data` Space, reached here
...
Cells names follow :mod:`UniversalLifeUS` -- the universal-life chassis this product is
...
def prem_persistency(t):
    ...
    **1.0 when the behavior module is off**, which is the default.  The base
    ...
    adjustment.  ``TermLifeUS`` switches conversion off for the same reason.
    """
    if not dyn_behavior_on:                                          # noqa: F821
        return 1.0
    phi = funding_ratio(t)
