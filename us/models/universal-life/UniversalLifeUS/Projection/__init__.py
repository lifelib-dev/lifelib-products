...
"""The by-policy monthly projection of the :mod:`~UniversalLifeUS` model.
...
directory, ``us/models/universal-life/``, read at run time rather than stored inside
...
``UniversalLifeUS`` folder without its parent's CSVs produces a model that reads and
then fails on first evaluation.

The readers live in the unparameterized :mod:`~UniversalLifeUS.Data` Space, reached
...
is ``rate_class`` too. The name is taken from ``TermLifeUS``/``BasicTerm_S``, which
