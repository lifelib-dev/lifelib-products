...
"""The by-contract projection of :mod:`~VariableAnnuityUS`.
...
``VariableAnnuityUS`` folder without its parent's CSVs produces a model that reads and
then fails on first evaluation.

The readers and the filename References live on the sibling
:mod:`~VariableAnnuityUS.Data` Space, reached here through the ``data`` Reference, so
...
wherever those models have an analogue, and :mod:`FixedDeferredAnnuityUS` — the deferred
...
:mod:`TermLifeUS` and by ``savings.CashValue_SE``. The notes' own end-of-month ``l(t)``
...
(:mod:`FixedDeferredAnnuityUS`, ``min(W, FW)``). On this product a second exemption
...
:mod:`FixedDeferredAnnuityUS` the same cells name means the portion of a withdrawal above
...
``E(t)`` in :mod:`TermLifeUS` is expenses, which here is :func:`expenses` as usual. ``M``
is the in-the-moneyness ratio here and the market value adjustment in
:mod:`FixedDeferredAnnuityUS`; a VA separate account has no MVA at all, so there is no
collision in the model, only in the reader's memory. ``c(t)`` is the CDSC here and the
Model #805 contract charge in :mod:`FixedDeferredAnnuityUS`; it becomes
...
``BasicTerm_S.pols_maturity`` and the construction follows :mod:`TermLifeUS` and
:mod:`FixedDeferredAnnuityUS`. It is bookkeeping determined by the horizon, not an added
assumption.
...
def prem_to_av_pp(t):
    ...
    value. Not to be confused with ``WholeLifeUS.premium_net_pp``, which is a *gross*
...
def wd_excess_pp(t):
    ...
    note on the divergence from :mod:`FixedDeferredAnnuityUS`.
    """
    if wd_pp(t) <= 0.0:
        return 0.0
...
def wd_free_pp(t):
    ...
    The chassis name for the same thing — :mod:`FixedDeferredAnnuityUS` has
...
def surr_benefit_pp(t):
    ...
    Account Options [S1]. Contrast :mod:`FixedDeferredAnnuityUS`, where
...
def pols_if(t):
    ...
    convention, set by :mod:`TermLifeUS` and by ``savings.CashValue_SE``.
