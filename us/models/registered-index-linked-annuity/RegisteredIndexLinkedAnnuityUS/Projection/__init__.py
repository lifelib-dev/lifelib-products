...
"""The by-contract projection of :mod:`~RegisteredIndexLinkedAnnuityUS`.
...
``RegisteredIndexLinkedAnnuityUS`` folder without its parent's CSVs produces a model that
reads and then fails on first evaluation.

The readers and the filename References live on the sibling
:mod:`~RegisteredIndexLinkedAnnuityUS.Data` Space, reached here through the ``data``
...
:mod:`FixedDeferredAnnuityUS`, whose beginning-of-month transaction convention puts the
...
this library's deferred annuity chassis :mod:`FixedDeferredAnnuityUS`, wherever those have
...
and the construction follows :mod:`TermLifeUS` and :mod:`FixedDeferredAnnuityUS`. The
...
def duration_bom(t):
    ...
    :mod:`FixedDeferredAnnuityUS` uses for its own ``duration(t)``.
    """
...
def lapse_shock_year():
    ...
    *surr_charge_table.csv* rather than hard-coded, so a different schedule moves the
    shock with it - the construction :mod:`UniversalLifeUS` uses. On the 7-7-6-5-4-3-0
...
def pols_if(t):
    ...
    The library-wide convention, following :mod:`TermLifeUS` and ``CashValue_SE``:
