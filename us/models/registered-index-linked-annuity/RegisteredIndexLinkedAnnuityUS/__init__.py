...
:mod:`~RegisteredIndexLinkedAnnuityUS` is the executable counterpart of
...
:func:`~RegisteredIndexLinkedAnnuityUS.Projection.credit_rate_term` credits,
:func:`~RegisteredIndexLinkedAnnuityUS.Projection.iv_factor` prices, and
:mod:`~RegisteredIndexLinkedAnnuityUS.Data` supplies the market state.

This model is built on the **deferred annuity base chassis**,
:mod:`FixedDeferredAnnuityUS`, and shares its cells names for every concept the two have
...
:mod:`~RegisteredIndexLinkedAnnuityUS.Data`
    Reads the seven input CSVs and holds their filename References. It takes no
    parameters, so each file is read **once per model**.

:mod:`~RegisteredIndexLinkedAnnuityUS.Projection`
    ...
    :mod:`~RegisteredIndexLinkedAnnuityUS.Data` Space.
...
duration(t) + 1``. Note the contrast with :mod:`TermLifeUS`, where ``t`` counts **years**,
and the smaller contrast with :mod:`FixedDeferredAnnuityUS`, whose beginning-of-month
...
:func:`~RegisteredIndexLinkedAnnuityUS.Projection.pols_maturity`.
...
so ``pols_if(1) = pols_if_init()`` as in :mod:`TermLifeUS` and the ``pols_if`` column of
...
decomposition to check; :func:`~RegisteredIndexLinkedAnnuityUS.Projection.check_av_roll_fwd`
and :func:`~RegisteredIndexLinkedAnnuityUS.Projection.check_pols_roll_fwd` are implemented
...
_name = "RegisteredIndexLinkedAnnuityUS"
