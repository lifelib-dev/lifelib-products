...
"""The by-policy projection of the :mod:`~WholeLifeUS` model.
...
``WholeLifeUS`` folder without its parent's CSVs produces a model that reads and then
...
``l_t``. That is the library-wide convention (``TermLifeUS.pols_if(1)`` is
...
worked example exactly, the same way ``TermLifeUS`` ships ``conv_rate_base = 0``. It is
...
Both are columns of ``result_cf()``. The pattern is ``ImmediateAnnuityUS`` and
``DeferredIncomeAnnuityUS``'s, which face the same clash. Nothing about the whole-life
