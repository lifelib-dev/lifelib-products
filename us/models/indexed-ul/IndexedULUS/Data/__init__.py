...
:mod:`~IndexedULUS.Projection` as ``data``. :mod:`~IndexedULUS.Projection` is
...
``us/models/indexed-ul/``, rather than data stored inside the model. The model folder
...
``IndexedULUS`` folder without its parent's CSVs produces a model that reads and then
...
illustrative table shipped with ``UniversalLifeUS``, so the chassis and this model share
a basis.
...
:func:`~IndexedULUS.Projection.prem_persistency` with the rate in a Reference. Nor is
there an index scenario file: the base deterministic run generates
:func:`~IndexedULUS.Projection.index_level` from a level annual return, and a stochastic
