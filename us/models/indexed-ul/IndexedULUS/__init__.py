...
:mod:`~IndexedULUS` is the executable counterpart of
...
* Stochastic index scenarios. The notes give the lognormal parameters (mu = 6.0%,
  ...
  prescribe. :func:`~IndexedULUS.Projection.index_level` is the single point of
  substitution.
...
* The grace and lapse-for-insufficiency cascade. The trigger and the no-lapse test are
  ...
  :func:`~IndexedULUS.Projection.is_shortfall`,
  :func:`~IndexedULUS.Projection.nlg_test_ok` and
  :func:`~IndexedULUS.Projection.nlg_in_effect`, and the notes' lapse suppression while
  the no-lapse guarantee is in effect *is* implemented in
  :func:`~IndexedULUS.Projection.lapse_rate`; but the notes leave the in-grace account
  ...
  **This has a consequence worth stating plainly.** A policy whose account value is
  ...
  :func:`~IndexedULUS.Projection.av_pp` first turns negative at policy month **605**
...
* The two $500 withdrawal limits. The product spec carries both -- "minimum withdrawal
  ...
  there to exercise ever starts; :func:`~IndexedULUS.Projection.ncsv_pp` floors at zero
...
* MEC status has no cash flow consequence for the insurer, so
  :func:`~IndexedULUS.Projection.is_mec` flags and does not project.
