...
:mod:`~FixedDeferredAnnuityUS.Projection` as ``data``.
:mod:`~FixedDeferredAnnuityUS.Projection` is parameterized by ``point_id``, so each
...
``us/models/fixed-deferred-annuity/``, rather than data stored inside the model. The
