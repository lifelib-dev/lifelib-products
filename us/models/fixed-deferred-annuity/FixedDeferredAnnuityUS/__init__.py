...
:mod:`~FixedDeferredAnnuityUS` is the executable counterpart of
...
:mod:`~FixedDeferredAnnuityUS.Projection`
    ...
    Reference, which resolves to the single :mod:`~FixedDeferredAnnuityUS.Data` Space.
...
anniversaries fall at ``t = 12, 24, ...``. Note the contrast with :mod:`TermLifeUS`,
...
convention (``pols_if(1) == pols_if_init()``, as in :mod:`TermLifeUS`), and it is the
