...
"""The by-contract projection of :mod:`~FixedDeferredAnnuityUS`.
...
``FixedDeferredAnnuityUS`` folder without its parent's CSVs produces a model that reads
and then fails on first evaluation.

The readers and the filename References live on the sibling
:mod:`~FixedDeferredAnnuityUS.Data` Space, reached here through the ``data`` Reference, so
...
processing step 7, *not* deaths as in :mod:`TermLifeUS`; it becomes
...
same symbol is expenses in :mod:`TermLifeUS`, which here is :func:`expenses`. ``X`` is
the dynamic-lapse exponent here and the premium tax in :mod:`TermLifeUS`, split into
...
:mod:`TermLifeUS` (``pols_if(1) == pols_if_init()``) and ``savings.CashValue_SE``, and it
...
construction follows :mod:`TermLifeUS`.
"""
...
def pols_if(t):
    ...
    The library-wide convention, following ``TermLifeUS`` and ``CashValue_SE``: this is
