...
"""The by-policy monthly projection of the :mod:`~GuaranteedULUS` model.
...
``GuaranteedULUS`` folder without its parent's CSVs produces a model that reads and
then fails on first evaluation.

The readers live in the unparameterized :mod:`~GuaranteedULUS.Data` Space, reached
...
:mod:`UniversalLifeUS` -- the chassis this product is built on -- everywhere else.
...
The notes' ``risk_class`` is this model's ``rate_class`` -- the name ``TermLifeUS`` and
:mod:`UniversalLifeUS` both use for the underwriting class, and the one the model point
