...
:mod:`~GuaranteedULUS` is the executable counterpart of
...
explicitly, and this model follows :mod:`UniversalLifeUS` in structure, naming and
...
* **Catch-up premium payment.** The catch-up requirement ``C_t`` is a complete formula
  ...
  :func:`~GuaranteedULUS.Projection.catch_up_prem_pp`; the notes state expressly that
...
* **Grace cure.** The required cure payment is implemented as
  :func:`~GuaranteedULUS.Projection.cure_premium_pp`, but the notes give no cure
...
algorithm: :func:`~GuaranteedULUS.Projection.no_lapse_premium` bisects
:func:`~GuaranteedULUS.Projection.guar_min_sg` over the level annual premium with
