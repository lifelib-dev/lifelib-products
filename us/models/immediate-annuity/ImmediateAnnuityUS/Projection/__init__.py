...
"""The by-contract projection of the :mod:`~ImmediateAnnuityUS` model.
...
``ImmediateAnnuityUS`` folder without its parent's CSVs produces a model that reads and
then fails on first evaluation.

Each table has a filename Reference and a reader Cells, both on
:mod:`~ImmediateAnnuityUS.Data`:
...
house sign convention of ``TermLifeUS`` (income less outgo, so ``-liability_cf``). Both
...
def net_cf(t):
    ...
    **Income positive**, the sign convention of ``TermLifeUS.net_cf``, kept even though
