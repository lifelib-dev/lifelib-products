...
- **Purpose.** Project gross best-estimate liability cash flows (annuity instalments
  ...
  pointers).
- **Mortality is the model.** The contract has no premiums after outset [S2 §1.1](#uklib-pension_annuity-s2), no
  surrender value [S2 §12](#uklib-pension_annuity-s2) [S5 cl.14.7](#uklib-pension_annuity-s5), no account value and no policyholder options
  after the cancellation window [S1 p4](#uklib-pension_annuity-s1). The only decrements are deaths; the only
...
- **Projection frequency.** Monthly grid, t = 1, 2, ... months from the start date
  **[std]**. Payment dates fall on the grid per the frequency m; exact-day mechanics
  (Just's first-of-month payments and stub proportioning [S5 §§5.2–5.3](#uklib-pension_annuity-s5), L&G's
  working-day adjustment [S2 §2.4](#uklib-pension_annuity-s2)) are not modeled **[std]**.
- **Timing conventions [std].** Escalation is applied at the start of the month
  containing the policy anniversary (first at t = 13) [S2 §3.3](#uklib-pension_annuity-s2). Advance instalments
...
- **Currency and model points.** GBP throughout [S2 §1.3](#uklib-pension_annuity-s2). Single-policy model
...
|---|---|---|
| `purchase_price` P | currency | 100,000 [S1 p11](#uklib-pension_annuity-s1) |
| `annuitant_age` x_a | int (ALB) | 65 [S1 p11](#uklib-pension_annuity-s1) |
...
| `dependant_pct` δ | float ≤ 1 [S1 p9](#uklib-pension_annuity-s1) | 0.50 **[std]** |
| `overlap` | bool (with/without overlap [S2 §§5.9–5.11](#uklib-pension_annuity-s2)) | false **[std]** |
| `annual_income` A(1) | currency p.a. | 5,400 **[std]** (see Worked example) |
| `frequency` m | enum {12, 4, 2, 1} [S2 §2.2](#uklib-pension_annuity-s2) | 4 |
| `timing` | enum {advance, arrears} [S2 §2.3](#uklib-pension_annuity-s2) | arrears |
| `proportion` | bool (arrears only [S2 §4](#uklib-pension_annuity-s2)) | false |
| `escalation_type` | enum {level, fixed, rpi_catchup, lpi5} (spec menu) | fixed |
| `escalation_rate` g | float ≤ 0.10 [S2 §3.2](#uklib-pension_annuity-s2) | 0.03 **[std]** |
| `guarantee_months` n | int, 12–360, 0 if none [S1 p10](#uklib-pension_annuity-s1); XOR with VP [S2 §§6.7, 7.6](#uklib-pension_annuity-s2) | 0 |
| `vp_pct` v | float ≤ 1 [S1 p11](#uklib-pension_annuity-s1); v + δ ≤ 1 on first-death basis [S2 §7.3](#uklib-pension_annuity-s2) | 0.50 **[std]** |
| `vp_basis` | enum {first_death, last_survivor} [S2 §7.3](#uklib-pension_annuity-s2) | first_death |

The premium P is the amount applied to the annuity after PCLS and adviser charges
[S1 p4](#uklib-pension_annuity-s1); PCLS itself is pre-purchase and outside the model. A(1) is a pricing input:
no insurer publishes a rate card, so A(1) is taken from a quote or calibrated to the
anchor (£100,000 at 65 buying £6,657 p.a. with 50% VP, January 2026 [S1 p11](#uklib-pension_annuity-s1); the
...
| Input | Value | Basis |
|---|---|---|
| Instalment amount | A(y)/m at each payment date | [S1 p8](#uklib-pension_annuity-s1) [S2 §2.2](#uklib-pension_annuity-s2) |
| Escalation rule | per `escalation_type`: fixed g ≤ 10%; RPI 0-floor with catch-up (12 months ending six months before the anniversary); LPI = RPI capped 5%, floor 0, September year | [S2 §3.2, §3.3, defs](#uklib-pension_annuity-s2); LPI floor harmonization **[std]** (spec footnote 5) |
| Dependant's income | δ × income, same escalation basis; % of the higher of income at death and at guarantee end | [S2 §§5.12–5.13](#uklib-pension_annuity-s2) |
| Overlap rule | with: dependant stream runs during remaining guarantee; without: starts at guarantee end | [S2 §§5.9–5.11](#uklib-pension_annuity-s2) |
| Guarantee period | n months of instalments certain, escalation continuing as if alive | [S2 §§6.5–6.6](#uklib-pension_annuity-s2) [S7 §4.2](#uklib-pension_annuity-s7) |
| Value protection | max(0, v × P − G(death)) on the chosen basis; v + δ ≤ 1 (first-death) | [S1 p11](#uklib-pension_annuity-s1) [S2 §7, §7.3](#uklib-pension_annuity-s2) |
| Surrender value | none, at any time | [S1 p4](#uklib-pension_annuity-s1) [S2 §12](#uklib-pension_annuity-s2) [S5 cl.14.7](#uklib-pension_annuity-s5) |
| Charges to policyholder | none (priced into the rate) | [S1 p6](#uklib-pension_annuity-s1) |

### (b) Insurer-discretionary current elements

**None post-purchase.** The contract is non-participating [S7 §7.9](#uklib-pension_annuity-s7) with all options
fixed at outset [S1 p4](#uklib-pension_annuity-s1): there are no bonus rates, no reviewable premiums, no market
...
an in-force element); its snapshot is the January 2026 anchor quote [S1 p11](#uklib-pension_annuity-s1), and
...
| Enhanced/impaired rating | Overlay on qx: q_rated = min(1, θ_a × q_base), θ_a ≥ 1 (equivalently a rated-age offset); standard life θ = 1.0 | existence [S1 p5](#uklib-pension_annuity-s1) [S4] [S6] [S9]; overlay **[std]** (iii) |
| Lapse / surrender | None — no surrender value exists | [S1 p4](#uklib-pension_annuity-s1) [S2 §12](#uklib-pension_annuity-s2) [S5 cl.14.7](#uklib-pension_annuity-s5) [R1] |
...
(iii) Insurers' rating structures (postcode, condition-specific factors [S1 p5](#uklib-pension_annuity-s1) [S9])
...
[S1 p6](#uklib-pension_annuity-s1)); £30 p.a. is a round placeholder for in-payment administration. Acquisition
...
| g | fixed escalation rate (0.03 **[std]**, ≤ 0.10 [S2 §3.2](#uklib-pension_annuity-s2)) |
| I(k), peak | RPI reference index at anniversary k and its running maximum (catch-up state) [S2 defs](#uklib-pension_annuity-s2) |
| δ | dependant's percentage (0.50 **[std]**, ≤ 1 [S1 p9](#uklib-pension_annuity-s1)) |
| n | guarantee period in months (0 or 12–360 [S1 p10](#uklib-pension_annuity-s1)) |
| v | value-protection percentage (0.50 **[std]**, ≤ 1 [S1 p11](#uklib-pension_annuity-s1)); v + δ ≤ 1 on first-death basis [S2 §7.3](#uklib-pension_annuity-s2) |
| P | purchase price (100,000 [S1 p11](#uklib-pension_annuity-s1)) |
...
| w(t) | dependant-stream availability: 1 if overlap or t > n, else 0 [S2 §§5.9–5.11](#uklib-pension_annuity-s2) |
...
### Escalation update (start of month 12(y−1)+1, y ≥ 2) [S2 §3.3](#uklib-pension_annuity-s2)
...
RPI catch-up pseudocode (path-dependent ratchet [S2 defs](#uklib-pension_annuity-s2); Aviva operates the same
rule [S9]):
...
guarantee end" [S2 §5.12](#uklib-pension_annuity-s2) exactly, because under the (non-decreasing **[std]** menu)
escalation options the as-if-alive income path is monotone, so the higher-of base
plus same-basis escalation [S2 §5.13](#uklib-pension_annuity-s2) reproduces δ × A(y(t)) at every later date.
...
runs (annuity-certain floor [S2 §§6.5–6.6](#uklib-pension_annuity-s2) [S7 §4.2](#uklib-pension_annuity-s7)) and l_a(t) × inst(t) thereafter.
...
without overlap the dependant stream starts at guarantee end [S2 §§5.9–5.11](#uklib-pension_annuity-s2).
(Guarantee and VP never coexist in the representative design: n > 0 ⇒ v = 0
[S2 §§6.7, 7.6](#uklib-pension_annuity-s2).)

**Proportionate final payment** (arrears with proportion only [S2 §4](#uklib-pension_annuity-s2)): for a death
...
final partial period [S2 §4](#uklib-pension_annuity-s2).
...
dependant's instalments too [S2 §7.3](#uklib-pension_annuity-s2) [S5 §8.4](#uklib-pension_annuity-s5). (The Canada Life variant additionally
nets guarantee payments due, excluding future RPI/LPI increases [S7 §4.3](#uklib-pension_annuity-s7) —
...
not projected [S2 §1.1](#uklib-pension_annuity-s2)) and no surrender outgo [S2 §12](#uklib-pension_annuity-s2).
...
1. If t starts a policy year (t = 12(y−1)+1, y ≥ 2): apply the escalation update
   (including catch-up state) [S2 §3.3](#uklib-pension_annuity-s2).
...
transfer [S1 p4](#uklib-pension_annuity-s1) [S2 §12](#uklib-pension_annuity-s2) [S5 cl.14.7](#uklib-pension_annuity-s5) [S7 §7.5](#uklib-pension_annuity-s7) [S9 §3.9](#uklib-pension_annuity-s9), no alteration of options
[S1 p4](#uklib-pension_annuity-s1) [S4] [S6] [S9], and no premium flexibility [S2 §1.1](#uklib-pension_annuity-s2). Consequently the model has
...
- **Cancellation window.** The 30-day cooling-off [S1 p7](#uklib-pension_annuity-s1) [S2 §13](#uklib-pension_annuity-s2) is ignored
...
P = £100,000 [S1 p11](#uklib-pension_annuity-s1); annuitant male 65, dependant female 62 **[std]**; quarterly
(m = 4) in arrears, without proportion [S2 §§2.2–2.3, 4](#uklib-pension_annuity-s2); fixed escalation g = 3%
**[std]**; dependant δ = 50% **[std]**; value protection v = 50% on the annuitant's
(first) death **[std]** — v + δ = 100%, exactly at the contractual bound [S2 §7.3](#uklib-pension_annuity-s2);
no guarantee period (XOR rule [S2 §§6.7, 7.6](#uklib-pension_annuity-s2)). Starting income A(1) = £5,400 p.a.
...
[S1 p11](#uklib-pension_annuity-s1), and an escalating joint-life basis starts lower than a level one for the
same premium [S1 p8](#uklib-pension_annuity-s1) [S4] [S6]). Scenario: the annuitant dies in month 17; the
...
| 18 | Q6 date: no annuitant payment (arrears, without proportion [S2 §4](#uklib-pension_annuity-s2)); dependant stream starts | 0.00 | 695.25 | — | 7,485.75 |
...
the lump sum is 50,000 − 6,790.50 = 43,209.50 [S1 p11](#uklib-pension_annuity-s1) [S2 §7](#uklib-pension_annuity-s2). Had "with proportion"
...
this stub off the VP fund-value formula [S5 §8.3](#uklib-pension_annuity-s5)). The dependant's 695.25 continues
for her life, escalating 3% at each anniversary on the same basis [S2 §§5.12–5.13](#uklib-pension_annuity-s2).

Guarantee/VP interaction. Had the model point instead carried a 10-year guarantee
**[std default]** and no VP (the XOR rule forbids both [S2 §§6.7, 7.6](#uklib-pension_annuity-s2)), the death in
month 17 would change nothing until month 120: instalments of 1,390.50, escalating 3%
each anniversary as if the annuitant were alive [S7 §4.2](#uklib-pension_annuity-s7), continue to beneficiaries
...
δ × the income at the end of the guarantee period [S2 §§5.9–5.12](#uklib-pension_annuity-s2). With overlap, the
dependant's stream would run from t = 18 alongside the guarantee payments
[S2 §§5.9–5.11](#uklib-pension_annuity-s2). In expectation these scenario flows are reproduced by the E[ANN(t)]
...
- **Tax.** Pension annuities are pension business — non-BLAGAB, trade-profit basis
  [REG-R17] [S5 §14.11](#uklib-pension_annuity-s5); no policyholder fund tax enters the projection.
...
3. **Inflation exposure (RPI/LPI options).** RPI-linked instalments make the
   ...
   inflation option positions [S2 §3.2, defs](#uklib-pension_annuity-s2). A deterministic 3% path **[std]**
...
- **Guarantee double-counting.** During the guarantee, the annuitant stream is
  ...
  representative design [S2 §§6.7, 7.6](#uklib-pension_annuity-s2); engines supporting the Canada Life
  combinable variant must net guarantee payments off VPbal [S7 §4.3](#uklib-pension_annuity-s7) or the death
...
- **Overlap gating.** Without overlap the dependant stream is gated on t > n even
  ...
  [S2 §§5.9–5.11](#uklib-pension_annuity-s2).
- **Higher-of dependant base.** The δ × A(y(t)) simplification relies on
  ...
  [S5 §7.1.2](#uklib-pension_annuity-s5)), the contractual "higher of income at death and at guarantee end"
  [S2 §5.12](#uklib-pension_annuity-s2) must be implemented explicitly.
...
- **Catch-up state.** The RPI ratchet is path-dependent: peak must persist across
  ...
  overstates indexed income after deflation-recovery paths [S2 defs](#uklib-pension_annuity-s2).
- **Escalation timing.** Increases apply on the anniversary [S2 §3.3](#uklib-pension_annuity-s2), not on
  ...
  (1 April / 1 May at Just [S5 §7.2](#uklib-pension_annuity-s5)) — out of scope with GMP generally **[std]**.
- **VP balance timing.** VPbal must net instalments *paid before death*; netting the
  ...
  without proportion) understates the lump sum [S2 §§4, 7](#uklib-pension_annuity-s2). Symmetrically, on
