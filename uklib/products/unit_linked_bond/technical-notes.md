...
- **Projection frequency.** Monthly **[std]**. The contract accrues the AMC daily
  through the unit price [S2 §5.1.1](#uklib-unit_linked_bond-s2) and prices funds daily/at least monthly
  [S2 §3.2](#uklib-unit_linked_bond-s2) [S3 Part E](#uklib-unit_linked_bond-s3); the model discretizes to monthly steps with all
...
- **Timing conventions [std].** Fund growth, tax provision and fund-based charges
  ...
  deals, 28-day PruFund waits, deferral powers [S2 §4, §8](#uklib-unit_linked_bond-s2) [S5 Q9](#uklib-unit_linked_bond-s5)) are ignored.
...
- **Model points.** Single-policy model points projected on an expected
  ...
  footnote 3); per-segment values are the bond values ÷ 100 [S1] [S2 §2.4](#uklib-unit_linked_bond-s2). No
...
| `premium` | currency (single premium, net of set-up adviser charge [S2 §1, §12.2](#uklib-unit_linked_bond-s2)) | 100,000 **[std]** |
...
| `oac_rate` | annual rate on unit value (ongoing adviser charge) | 0 (module value 0.005 [S1] [S2 §7.1 example](#uklib-unit_linked_bond-s2)) |
| `gmdb_flag` | bool (return-of-premium rider [S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2) [S5]) | false **[std]** |
...
| `CumWD(n)` | Cumulative withdrawals + ongoing/ad hoc adviser charges to end of insurance year n (allowance-relevant [S2 §12.1.1](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5)) | on withdrawal/charge |
...
| `G(t)` | GMDB guaranteed amount = premium − withdrawals − ongoing/ad hoc adviser charges (if `gmdb_flag`) [S2 §10](#uklib-unit_linked_bond-s2) | on events |
...
| Withdrawal machinery | Regular/partial/segment surrender; 12-month regular cap = max(7.5% of plan value, 7.5% of total paid in) incl. ongoing adviser charges | [S1] [S2 §7.1, §7](#uklib-unit_linked_bond-s2) |
| Charge basis | AMC accrues daily through the unit price; adviser/rider charges by unit cancellation | [S2 §5.1.1, §12](#uklib-unit_linked_bond-s2) |
| Segmentation | 100 identical policies; premium and units divided equally | count **[std]**; mechanics [S1] [S2 §2.4](#uklib-unit_linked_bond-s2) |
| Liability cap | Benefits derived from fund assets only; no make-whole on external default | [S2 §3.1.9](#uklib-unit_linked_bond-s2) [S4] |
...
booklet [S3 Part D](#uklib-unit_linked_bond-s3)); the model holds the snapshot level:

| Input | Snapshot value | Basis |
|---|---|---|
| AMC `c` | 1.00% p.a. | **[std]** — per-fund AMC rate cards not fetched (research gap 5); only the discount tier table is public [S1] |
| Further costs `f` | 0.10% p.a. (fund-borne, not insurer income) | existence [S1] [S2 §3.1.7](#uklib-unit_linked_bond-s2); level **[std]** |
| Fund-size discount | Off (level net AMC assumed) | tiers [S1] [S2 §5.1.4](#uklib-unit_linked_bond-s2); scope **[std]** |
| Life-fund tax pass-through `t_pf` | 20% of gross fund return, in-price, neutral to insurer | mechanism [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5); rate proxy **[std]** of the policyholder rate [R6] |
| GMDB mortality-factor scale | = monthly mortality rate from the class-(c) basis at attained age (cost-of-insurance style), applied to max(0, G − u×UF) | design [S2 §5.2, §10](#uklib-unit_linked_bond-s2); scale **[std]** — factors not published |
| MVR / bonus rates | Not applicable — with-profits and PruFund funds out of scope; see `products/with_profits/` | [S2 §3.3](#uklib-unit_linked_bond-s2) [S3] |
...
deemed-disposal charge, and full-surrender settlement [S5 Q15](#uklib-unit_linked_bond-s5) [S4], and the company's
...
1. Update y, a, E(t).
2. **Fund growth and tax provision** (within unit price [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5)):
   `G$(t) = g_m × UF(t−1)`;  `TX(t) = t_pf × G$(t)`;
   `UF_g(t) = UF(t−1) + G$(t) − TX(t) = UF(t−1) × (1 + g_m(1 − t_pf))`.
3. **Fund-based charges** (AMC accrues via price [S2 §5.1.1](#uklib-unit_linked_bond-s2); further costs
   fund-borne [S2 §3.1.7](#uklib-unit_linked_bond-s2)):
...
5. **Death strain per death:**
   ...
   [S2 §10](#uklib-unit_linked_bond-s2) [S5].
...
7. **Allowance tracker (insurance-year end, policyholder side only):**
   ...
   policyholder-side (the insurer issues certificates [S5 Q15](#uklib-unit_linked_bond-s5)).
...
| Set-up adviser charge / commission | 0 — post-RDR adviser charges are pass-throughs facilitated by unit cancellation [S1] [S2 §12](#uklib-unit_linked_bond-s2) [S4] | 0 |
...
- **Withdrawal take-up [std].** `wd_pattern = allowance_5pct`: W(t) = 0.05 × P / 12
  ...
  [S2 §7.1](#uklib-unit_linked_bond-s2), and adviser charges consume the same allowance [S2 §12.1.1](#uklib-unit_linked_bond-s2) [S5 Q15](#uklib-unit_linked_bond-s5) —
...
- **Segment vs part-surrender election.** Whether a policyholder cashes whole
  ...
  [S5 Q12](#uklib-unit_linked_bond-s5) [R1 s484/s498](#uklib-unit_linked_bond-r1), not the insurer's cash flow (both cancel the same unit
...
[S2 §7.1](#uklib-unit_linked_bond-s2).
...
4. **Tax pass-through neutrality.** The 20%-of-gross-return in-price proxy **[std]**
   ...
   timing (deemed disposals, realised-gain charge dates [S5 Q15](#uklib-unit_linked_bond-s5) [S4]) and base
...
5. **Mortality — only if GMDB is enabled.** Base death strain is 0.001 × UF (≈ £1
   ...
   charge scale [S2 §5.2](#uklib-unit_linked_bond-s2) is a [std] guess — enable only with its own sensitivity
   set [S1] [S2 §10](#uklib-unit_linked_bond-s2) [S5].

Known modeling pitfalls:

- **Charge-base ordering.** AMC accrues on the post-growth, pre-cancellation fund
  (in-price accrual [S2 §5.1.1](#uklib-unit_linked_bond-s2)). Charging c_m on UF(t−1) or after withdrawals
  changes the margin by ~½ month's growth/withdrawal — small monthly, systematic
  over decades.
- **Counting pass-throughs as margin.** Further costs [S1] [S2 §3.1.7](#uklib-unit_linked_bond-s2) and the tax
  provision [S4] [S5 Q15](#uklib-unit_linked_bond-s5) reduce the unit fund but are not insurer income; booking
...
- **Treating the 5% allowance as a product feature.** It is policyholder tax
  ...
  [S2 §7.1](#uklib-unit_linked_bond-s2)) and generates no insurer cash flow. Model it in behavior only.
- **Adviser charges are not insurer income.** Post-RDR set-up/ongoing/ad hoc adviser
  charges are facilitated pass-throughs by unit cancellation [S2 §12](#uklib-unit_linked_bond-s2) [S4]; they
...
- **Smoothed funds must not be bolted on.** PruFund EGR/smoothing-limit mechanics
  [S2 §3.3.7–3.3.10](#uklib-unit_linked_bond-s2) and MVR-bearing with-profits funds [S3] change the unit-price
