# Unit-Linked Investment Bond — Representative Product Specification (United Kingdom)
...
[S2 §18.5](#uklib-unit_linked_bond-s2), and its linked benefit design places it in RAO Class III "Linked
...
[S2 §3.1.5](#uklib-unit_linked_bond-s2) [S3 Part D](#uklib-unit_linked_bond-s3) [S5 Q1](#uklib-unit_linked_bond-s5) — and the insurer's liability is capped at the value
derived from the assets underpinning each fund, with no make-whole if an external
fund manager defaults [S2 §3.1.9](#uklib-unit_linked_bond-s2) [S4] [S1].
...
| Legal class | RAO Class III linked long-term; contract of long-term insurance | [R4] [S2 §18.5](#uklib-unit_linked_bond-s2) |
| Governing law | England (and Wales) | [S1] [S2 §18.4](#uklib-unit_linked_bond-s2) [S3] [S4] [S5] |
| Lives assured | Single or joint lives, benefits on last death; composite models single life | design [S1] [S2 §9.2](#uklib-unit_linked_bond-s2) [S3] [S4] [S5]; single-life choice **[std]** (1) |
...
| Policy segmentation | 100 identical mini-policies ("segments"), premium and units divided equally | count **[std]** (3); mechanics [S1] [S2 §2.4](#uklib-unit_linked_bond-s2) |
...
3. Observed segment counts: 20 default, up to 999 on request, minimum £1,000 per
   segment when more than 20 [S1] [S2 §2.4](#uklib-unit_linked_bond-s2); 100 [S3]; 1,000 [S4]; 1,000 [S5].
   Standardized to 100 so each segment is exactly 1% of the bond (£1,000 per segment
   at the anchor premium, consistent with the Prudential per-segment floor [S2 §2.4](#uklib-unit_linked_bond-s2)),
   ...
   chargeable-event outcomes [S1] [S4] [S5 Q12](#uklib-unit_linked_bond-s5) [R1 s484/s498](#uklib-unit_linked_bond-r1) [R2].
...
| Parameter | Representative value | Basis |
|---|---|---|
| Sum assured | 100.1% of the bid value of units (0.1% death uplift) | [S1] [S2 "Sum Assured"](#uklib-unit_linked_bond-s2) [S5]; choice **[std]** (6) |
| Unit valuation on death | Units valued on the working day notice of death is received (12:00 cut-off); number of units = units in credit at the date of death, adjusted for post-death transactions | [S1] [S2 §4.2.7, §9](#uklib-unit_linked_bond-s2) |
| Adviser charges after death | Ongoing/ad hoc adviser charges paid between death and processing are reclaimed and included in the claim | [S2 §9](#uklib-unit_linked_bond-s2) |
| Deferral powers | Do not apply to death benefit payment | [S1] [S2 §8](#uklib-unit_linked_bond-s2) |
| On payment | Plan cancelled; no further benefits | [S2 §9.5, §10.5](#uklib-unit_linked_bond-s2) |
| Return-of-premium GMDB rider | Optional at outset only; described under Riders; excluded from the base model | [S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2) [S5]; exclusion **[std]** (7) |
...
7. Both optional guarantees observed (Prudential Return of Premium Death Benefit
   [S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2); Quilter Capital Protected Death Benefit [S5]) are elected at
...
| Parameter | Representative value | Basis |
|---|---|---|
| Annual management charge (AMC) | 1.00% p.a. of unit value, accrued daily through the unit price (modeled as 1/12 monthly) | mechanism [S2 §5.1.1](#uklib-unit_linked_bond-s2); level **[std]** (8) |
| Fund-level further costs | 0.10% p.a., borne within the fund (not insurer income) | existence [S1] [S2 §3.1.7](#uklib-unit_linked_bond-s2); level **[std]** (9) |
| AMC fund-size discount | Not modeled; documented option — Prudential tiers 0.30% (<£25k) to 0.575% (£3m+), applied monthly on AUM per premium | [S1] [S2 §5.1.4](#uklib-unit_linked_bond-s2); scope **[std]** (10) |
| Life-fund tax pass-through | 20% of the gross fund investment return, deducted within the unit price (neutral pass-through to the insurer) | mechanism [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S3 Part E](#uklib-unit_linked_bond-s3) [S4] [S5 Q15](#uklib-unit_linked_bond-s5); rate proxy **[std]** (11) |
| Bid-offer spread / allocation rate | None — single-priced units, 100% allocation | [S3 Part D single-priced default](#uklib-unit_linked_bond-s3) [S4] [S5]; clean scope **[std]** (12) |
| Establishment / early cash-in charges | None in the clean design | [S4] [S5]; scope **[std]** (12) |
| Switching | Free, unlimited (right to introduce charges reserved); anti-market-timing powers reserved | [S1] [S2 §6.3.1.2](#uklib-unit_linked_bond-s2) [S3] [S5 Q10, Q11](#uklib-unit_linked_bond-s5) |
| Dilution levy | Reserved power, collected through unit pricing; not modeled | [S2 §3.2.6](#uklib-unit_linked_bond-s2); scope **[std]** |
...
8. Actual AMC percentages are per-fund and live in fund guides that were not fetched
   ...
   documented on the legacy booklet [S3 Part D](#uklib-unit_linked_bond-s3) — so the model treats it as a
...
9. Fund-level "further costs" (transaction/underlying costs) are confirmed to exist
   and be borne within funds [S1] [S2 §3.1.7](#uklib-unit_linked_bond-s2) but no values are published in the
...
10. The tier table [S1]: <£24,999: 0.30%; £25,000–£49,999: 0.35%; £50,000–£99,999:
    ...
    premium [S2 §5.1.4](#uklib-unit_linked_bond-s2). Omitted from the base model (a level net AMC is assumed);
...
11. Onshore life funds bear corporation tax on income and gains under I-E/BLAGAB, at
    ...
    price [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S3 Part E](#uklib-unit_linked_bond-s3); the platform products levy explicit periodic tax
    ...
    proceeds) [S4] [S5 Q15](#uklib-unit_linked_bond-s5). The composite standardizes to a flat 20% of the gross
...
12. The clean design has no initial charge, no allocation-rate machinery and no exit
    ...
    early-exit layer recorded [S5 Q7](#uklib-unit_linked_bond-s5). Legacy layers (bid-offer One-Off Charge,
...
| Parameter | Representative value | Basis |
|---|---|---|
| Regular withdrawals | Monthly (available frequencies monthly/3-/4-/6-/12-monthly); amount as fixed £, % of premium, or % of unit value; minimum £50 per payment; ≥ £500 must remain per fund | [S1] [S2 §7.1](#uklib-unit_linked_bond-s2) |
| Product cap on regular withdrawals | In any 12 months, greater of 7.5% of plan value and 7.5% of total paid in, with ongoing adviser charges aggregated inside the cap | [S1] [S2 §7.1](#uklib-unit_linked_bond-s2) |
| One-off partial withdrawal | Any time, part surrender spread across all segments; irrevocable once received | [S2 §7](#uklib-unit_linked_bond-s2) |
| Segment surrender | Full surrender of one or more individual segments | [S1] [S2 §2.4.5](#uklib-unit_linked_bond-s2) [S4] [S5 Q12](#uklib-unit_linked_bond-s5) |
...
| Tax-deferred allowance (policyholder side) | Cumulative 5% of each premium per insurance year — statutorily, allowable element = premium × y/20, y capped at 20; unused allowance carries forward; excess over the cumulative allowance is an "excess event" gain at insurance-year end | [R2] [R1 s498/s507](#uklib-unit_linked_bond-r1) [S1] [S4] [S5 Q15](#uklib-unit_linked_bond-s5) |
| Adviser charges | Set-up (deducted before investment; remainder is the Premium), ongoing (periodic, by unit cancellation spread across segments), ad hoc (one-off); ongoing/ad hoc treated as withdrawals for tax and counted inside the 7.5% cap; base model carries them at zero | [S1] [S2 §12.1–12.4](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5); zero **[std]** (15) |
| Maximum Limit Test | Caps total ongoing + ad hoc adviser charges per policy year; re-tested on partial withdrawals and instruction changes | [S2 §12.3.2, §12.7](#uklib-unit_linked_bond-s2) |
| Cooling-off | 30 days from plan documents; refund reduced by any fall in value; adviser fees not refunded | [S1] [S4] [S5 Q19](#uklib-unit_linked_bond-s5) |
| Settlement frictions | Same-working-day unit cancellation before 12:00 cut-off; up to 2 working days for large deals; deferral up to 6 months (property funds) / 1 month (others); not modeled | [S2 §4.1.1, §4.4, §8](#uklib-unit_linked_bond-s2) [S1] [S3]; scope **[std]** |

13. The Prudential provisions confirm partial/full withdrawals at any time with no
    penalty layer in the current product [S1] [S2 §7.2–7.3](#uklib-unit_linked_bond-s2); Aviva platform states
    "without penalty" [S4]. Quilter caps a one-off part surrender at 95% of value
    with £1,000 minimum remaining [S5 Q12](#uklib-unit_linked_bond-s5) — remaining-balance minima are
...
14. 5% of premium p.a. equals the statutory tax-deferred allowance exactly [R2], sits
    inside the 7.5% product cap [S2 §7.1](#uklib-unit_linked_bond-s2), and is the pattern every fetched KFD uses
...
15. A representative ongoing adviser charge, when the module is enabled, is 0.5% p.a.
    ...
    maximum 7% withdrawals") [S1] [S2 §7.1](#uklib-unit_linked_bond-s2). The anchor cell carries all three adviser
    ...
    [S2 §12.1.1](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5)).
...
adviser charge [S1] [S2 §1, §12.2](#uklib-unit_linked_bond-s2)) buys units at the bid price across the chosen
funds; premium and units are divided equally between the 100 segments (**[std]**
count; mechanics [S1] [S2 §2.4](#uklib-unit_linked_bond-s2)). Each segment is an identical mini-policy that can be
assigned or fully surrendered separately [S1] [S2 §2.4](#uklib-unit_linked_bond-s2).

**Units and unit pricing.** Units are notional [S2 §3.1.5](#uklib-unit_linked_bond-s2). Prudential
...
minimum value / units in issue, rounded to the nearest 0.1p [S2 §3.2.1–3.2.4](#uklib-unit_linked_bond-s2).
Externally-linked funds follow the external manager's prices with the same
expansion/contraction logic [S2 §3.2.3, §3.2.5](#uklib-unit_linked_bond-s2); Aviva values funds every business
day on the same swinging-basis design [S3 Part E](#uklib-unit_linked_bond-s3); platform deals receive forward
pricing at the next dealing point [S5 Q9](#uklib-unit_linked_bond-s5). The reference model abstracts all of this
...
fund value and reflected in the bid price [S2 §5.1.1](#uklib-unit_linked_bond-s2). Fund-level further costs are
borne within the fund [S1] [S2 §3.1.7](#uklib-unit_linked_bond-s2). Tax on the life fund's income and gains is
allowed for inside the unit price (insured funds) [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S3 Part E](#uklib-unit_linked_bond-s3) or charged
explicitly to the policy (platform products) [S4] [S5 Q15](#uklib-unit_linked_bond-s5). With AMC rate `c`, further
...
received [S2 §4.2.7, §9](#uklib-unit_linked_bond-s2). The insurer's death strain per claim is DB − unit fund
= 0.1% of unit value (plus any GMDB in-the-money amount if the rider is attached
[S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2)). The plan then terminates [S2 §9.5](#uklib-unit_linked_bond-s2).
...
[S1] [S2 §7.1](#uklib-unit_linked_bond-s2). One-off partial withdrawals are part surrenders spread across all
segments; alternatively whole segments are surrendered [S2 §2.4.5, §7](#uklib-unit_linked_bond-s2) [S4] [S5 Q12](#uklib-unit_linked_bond-s5).
...
ongoing/ad hoc adviser charges [S2 §12.1.1](#uklib-unit_linked_bond-s2) [S4] [S5 Q15](#uklib-unit_linked_bond-s5)) with the cumulative
...
death [S5 Q15](#uklib-unit_linked_bond-s5) [R1 s484](#uklib-unit_linked_bond-r1). Gains are taxed as income of the policyholder with a
basic-rate credit (onshore) [S4] [S5 Q15](#uklib-unit_linked_bond-s5) [R6]; top-slicing relief (s535–s537) and
deficiency relief (s539) exist in the statute [R1] [unverified beyond section
references]. The insurer issues Chargeable Event Certificates [S5 Q15](#uklib-unit_linked_bond-s5). None of this
...
reserved) [S1] [S2 §6.3.1.2](#uklib-unit_linked_bond-s2) [S3] [S5 Q11](#uklib-unit_linked_bond-s5); the insurer may refuse, limit or charge
switches on suspicion of market timing [S3] [S5 Q10](#uklib-unit_linked_bond-s5), and may close, merge or rename
funds with notice [S2 §3.1.3](#uklib-unit_linked_bond-s2) [S3]. Fund-count limits vary (10 [S1] [S2 §3.1.4](#uklib-unit_linked_bond-s2) to
...
[S1] [S4] [S5 Q19](#uklib-unit_linked_bond-s5). There is no paid-up mechanism — a single-premium contract carries
...
- **Return-of-premium guaranteed minimum death benefit (GMDB).** Prudential "Return
  ...
  restartable [S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2). Quilter's "Capital Protected Death Benefit" is the
...
- PruFund smoothed funds inside the wrapper — Expected Growth Rate accrual, quarterly
  and daily smoothing limits, 28-day waits, quarter dates [S2 §3.3.7–3.3.10](#uklib-unit_linked_bond-s2); see the
  ...
  [S2 §3.3.10](#uklib-unit_linked_bond-s2).
- PruFund Protected Fund guarantee (Guaranteed Minimum Fund Value at a chosen
  ...
  cancellations) [S2 §5.3, §11](#uklib-unit_linked_bond-s2).
...
1. **Death uplift.** 100.1% of unit value [S1] [S2] [S5] vs 101% [S4] [S3 pre-2006
   plans; S5 pre-25/11/2024](#uklib-unit_linked_bond-s3). Chosen: 100.1% — anchor-document value, currently
...
3. **Charge architecture.** Clean fund-based AMC with tiered discount [S1] [S2] or
   ...
   Yearly Management Charge [S3 Part C/D](#uklib-unit_linked_bond-s3). Chosen: clean structure; the legacy stack
   ...
   variation.
4. **Life-fund tax presentation.** Implicit in daily unit pricing [S2 §3.2.1](#uklib-unit_linked_bond-s2) [S3
   Part E](#uklib-unit_linked_bond-s3) vs explicit periodic "charge in respect of tax" to the policy, including
   an annual deemed-disposal charge [S4] [S5 Q15](#uklib-unit_linked_bond-s5). Chosen: implicit-in-price 20%
...
5. **Fund menu.** ~10 concurrent internal/mirror funds [S1] [S2 §3.1.4](#uklib-unit_linked_bond-s2); 30 (legacy)
...
6. **Withdrawal caps.** 7.5%-of-value/paid-in product cap including ongoing adviser
   charges [S1] [S2 §7.1](#uklib-unit_linked_bond-s2) vs minima-only (£25 minimum, £1,000 residual [S5]; residual
...
7. **Smoothed-fund option.** PruFund range with published smoothing mechanics
   [S2 §3.3](#uklib-unit_linked_bond-s2) vs Aviva Smooth Managed funds (quarterly switch limit) [S4] vs
   with-profits with MVR (legacy) [S3] vs none [S5]. Chosen: excluded — smoothing
   belongs to the with-profits reference product.
8. **Guarantee riders.** Return-of-premium GMDB [S1] [S2 §5.2, §10](#uklib-unit_linked_bond-s2) [S5] vs none
...
9. **Settlement frictions.** 28-day PruFund waits, 2-working-day large-deal delay,
   6-month property deferral [S1] [S2 §4.4, §8](#uklib-unit_linked_bond-s2); suspension/deferment powers [S3] [S4];
   forward pricing and ~10-working-day payout [S5 Q9, Q16](#uklib-unit_linked_bond-s5). Chosen: ignored in the
...
aligned to illiquid assets (6-month property deferral [S1] [S2 §8](#uklib-unit_linked_bond-s2) [S3]). The Consumer
...
[S1] [S5 Q15](#uklib-unit_linked_bond-s5). All of it is policyholder-side: the model carries it as behavior, not
cash flow.
...
[REG-R4]: #uklib-reg-r4
[S1]: #uklib-unit_linked_bond-s1
[S2]: #uklib-unit_linked_bond-s2
[S3]: #uklib-unit_linked_bond-s3
[S4]: #uklib-unit_linked_bond-s4
[S5]: #uklib-unit_linked_bond-s5
[S7]: #uklib-unit_linked_bond-s7
[std]: #uklib-std
