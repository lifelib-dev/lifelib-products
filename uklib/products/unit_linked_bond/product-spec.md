...
3. Observed segment counts: 20 default, up to 999 on request, minimum £1,000 per
   ...
   chargeable-event outcomes [S1] [S4] [S5 Q12] [R1 s484/s498] [R2].
...
| Tax-deferred allowance (policyholder side) | Cumulative 5% of each premium per insurance year — statutorily, allowable element = premium × y/20, y capped at 20; unused allowance carries forward; excess over the cumulative allowance is an "excess event" gain at insurance-year end | [R2] [R1 s498/s507] [S1] [S4] [S5 Q15] |
...
allowable after 20 years [R2] [R1 s498/s507]. Withdrawals above the cumulative
...
[R1 s484], with gain = total benefit value − (allowable deductions + previous gains)
[R1 s491–s494]. On death the bond is treated as fully cashed in immediately before
death [S5 Q15] [R1 s484]. Gains are taxed as income of the policyholder with a
...
- Capital redemption variant [brief]: the same bond chassis written without lives
  ...
  product-level terms not in any fetched document — [unverified]].
...
risk margin [R5 TP 2.4]; the best estimate is the probability-weighted average of
...
obligations [R5 TP 3.1, 3.2] — for unit-linked business this is the rule-level anchor
...
factor (floor 0.25) for long-term business [R5 TP 1.2, 4A.1] [REG-R4]. This library
...
levels of the kind snapshotted here [REG-R12; outcome-location detail unverified].
...
[R1 s465–s467]. Gains can affect personal allowances and means-tested benefits
