...
# t is 0-based: t = 0 is the first policy year, so the end of the premium term,
# policy year m, is the period t = m - 1.
df = proj.result_cf()
rows = [t for t in (0, 1, 2, 3, 4) if t < proj.proj_len()]
mid = proj.prem_period() - 1
rows += [t for t in (mid - 1, mid, mid + 1) if t < proj.proj_len() and t not in rows]
print("first policy years (t = 0 is the first), and the periods around "
