...
`Projection[2]` its second cell. `result_cf()` returns a `DataFrame` indexed by the period
index `t` with one column per cash flow line, `pols_if` first and `pols_if_pay` and
...
## The time index: `t` is 0-based, and `k` is the anniversary

`t` is the **period** index and it is **0-based**: `t = 0` is the first policy year,
`t = proj_len() - 1 = n - 1` the last, `proj_len()` is the *number* of projected years so
`len(result_cf()) == proj_len()` and the frame is `range(proj_len())`, and the contractual
**policy year is the 1-based label `t + 1`**. Period `t` runs from time `t` to time `t + 1`:
premium, maintenance expense, commission and the loan balance sit at its start; death,
staged, maturity and surrender benefits at its end. `age(t) = x + t`, `pols_if(0) = 1`,
`lapse_rate(n - 1) = 0`, maturity falls in `t = n - 1`, and `pols_if(n) = 0`.

Beside it runs **`k`, the anniversary index**, `k = 0` at issue. Everything that is a *value
at a point in time* rather than a *flow during a period* is indexed by `k`, and that index
is already 0-based and does not move: `pol_val_pp(k)`, `pol_val_pre_pp(k)`,
`surr_charge_pp(k)`, `surr_val_pp(k)`, `cv_pp(k)`, `reserve_pp(k)`, `benefit_pct(k)`,
`benefit_pct_cum(k)`, `prem_cum_pp(k)` and `edu_epv(k, i)`. The two clocks meet at
`k = t + 1`, the **closing** anniversary of period `t`, which is the value the period's
surrender and death benefits are paid on — so `claims(t, "LAPSE")` reads `cv_pp(t + 1)`,
`death_ben_pp(t)` reads `pol_val_pre_pp(t + 1)`, and `result_val()` publishes anniversary
`t + 1` on row `t` (with `loan_pp`, a start-of-period balance, the one column read at `t`).
`SC(0)` is the deduction at issue and `W(n) = S` the value at maturity; neither number moves
when the period index is re-based, and neither does the 学資金 grid at anniversaries 3 / 6 /
12 / 15 / 18 / 20.
...
Wb(k)  = W(k) + S x g(k)
DB(t)  = max( P x min(t + 1, m) - S x G(t) - L(t),  Wb(t + 1) )
```

`DB(t)` is where the two clocks meet: a death in period `t` is paid at anniversary `t + 1`,
so the value limb is `Wb(t + 1)` and the premiums due are the `min(t + 1, m)` falling at
times `0 .. t`, while the staged benefits *already received* are those to anniversary `t`,
because the payment at `t + 1` is still inside `Wb(t + 1)`.
...
zero on the education cell — so that a single recursion covers both, rolling period `t` from
its opening anniversary `t` to its closing one `t + 1`:

```
(W(t) + pi 1{t < m}) (1 + i_cv) = q(t) DB_val + (1 - q(t)) Wb(t + 1)
```

with `W(0) = 0` at issue.
That is `check_pol_val_roll_fwd()`, and it pins the timing as well as the arithmetic:
premium credited at the start of the period, interest for the whole period, death benefit
and staged benefit at the end. Its `q(t)` is the **unadjusted** table rate, because the policy
...
`mort_rate(t)` reads the table at the insured's `x + t`; `mort_rate_ph(t)` reads it at
the policyholder's `y + t`. Reading one table at one age for both is the most likely
...
`mort_rate_ph(t)` is **zero from `t = m` on**, the premium-paying periods being
`t = 0 .. m - 1`. Every waiver trigger in the retrieved 約款 is
...
decrement into periods `t >= m` would terminate a further 0.8367% of policies and delete
their maturity benefits, in exactly the years in which 86% of the second cell's receipts
fall.
...
Premiums on a waived policy are **deemed paid**, so `prem_cum_pp(k)` keeps growing on a
policy that pays nothing and `cv_pp(k)` is the same series in both states [S1] [S10] [S13].
...
is `k`, the **anniversary** the payment falls on — a time point, already 0-based — so the
payment at `k` is the staged claim of period `k - 1` and `claims(t, "STAGED")` reads
`benefit_pct(t + 1)`. Total receipts range
...
There is **no tail and no terminal age**. `proj_len()` is the 保険期間 exactly — the *number*
of projected years, so the last row is `t = n - 1` and `len(result_cf()) == proj_len()` —
everything
closes at the end of that period, `pols_if(n) = 0`, and the closing cash flow is a certain
payment of `S` to the survivors
rather than a decrement. `lapse_rate(n - 1)` is zero **[std]**: a surrender at the end of the
...
so that the roll-forward closes in the final period, where they neither die nor surrender.
...
| `lapse_table.csv` | Surrender and premium-default rates keyed by `policy_year`, 4 / 3 / 2 percent and 1.0 / 0.8 / 0.6 percent | **[std]**; the industry 解約・失効率 of 5.6% is an amount-weighted all-product ceiling only [R9] [REG-R31] |
...
own clock. None of them shifted when the period index was re-based to 0, and each says so
here so that a reader can check the readers against the files:

| File | Column | Decision | Reason |
|---|---|---|---|
| `lapse_table.csv` | `policy_year` | **1-based contractual label; values unchanged** | It is the policy year the 約款-style duration table is written in, values 1, 2, 3. `lapse_rate_base(t)` and `default_rate(t)` map through it as `t + 1`, capped at the last row |
| `benefit_schedule_table.csv` | `k` (was `t`) | **anniversary, `k = 0` at issue; values unchanged** | It is a time *point*, not a period: the 学資金 at `k` = 3 / 6 / 12 / 15 / 18 / 20 are sourced anniversaries and they feed `edu_epv(k, i)`, whose discounting is `v^(s-k)` from anniversary `k`. The column was renamed from `t` to `k` so that it cannot be mistaken for the frame's period index; `claims(t, "STAGED")` reads it at `t + 1` |
| `mort_table.csv` | `age` | **attained age; values unchanged** | Not a time index at all — a table key read at `age(t) = x + t` |
| `model_point_table.csv` | `policy_term`, `prem_term` | **lengths; values unchanged** | Counts of years, not points on the frame. There is no `duration_init`, no entry year and no point on the time axis in this file: every model point is new business at issue and opens the frame at `t = 0` |
...
| *(no symbol)* | `pols_maturity` | The notes write the closing payment as `S x R(n - 1)` and give it no symbol. It is named so that the in-force roll-forward closes in the final period, where the survivors neither die nor surrender |
...
| Renewal commission | 3% of premium, policy years 2 to `m` (`1 <= t < m`), paying state only | Reference on `Projection` |
...
- every one of the notes' known modelling pitfalls, one test named after each: that maturity
  is certain and not a decrement, that `lapse_rate(n - 1)` is zero in the final period, that
  `pol_val_pp(n)` at the final anniversary is
  `sum_assured()`, that the two lives read the table at two ages, that `mort_rate_ph(t)` is
  zero from `t = m` on and by how much that moves the block, that the waiver produces no benefit
...
- the roll-forward identities rebuilt independently of the checks, and all six `check_*()`
  cells with their per-period residuals over `range(proj_len())`, on every one of the nine
  shipped model points;
- each optional module in both positions, and the structural product facts: no tail states —
  `list(result_cf().index) == list(range(n))`, `pols_if_pay(0) == 1`, `pols_if_pay(n) == 0` —
...
The two clocks show in the test module itself: the cash-flow and in-force literals are
0-based periods, while the `W` / `SC` / `CV` table, the 学資金 grid and every `prem_cum_pp`
literal are anniversaries and are unchanged. Loop and parameter names say which is meant —
`t` for a period, `k` for an anniversary.
