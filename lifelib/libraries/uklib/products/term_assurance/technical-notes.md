...
- **Time index.** `t` is 0-based and counts policy years from issue: the first policy
  year is `t = 0`, policy year = `t + 1`, and the frame is `t = 0 … n − 1` (`n` is the
  number of years, the exclusive end). Year `t` runs from time `t` to time `t + 1`:
  `l(t)` is the in-force at its start, start-of-year flows fall at time `t` and
  end-of-year flows at time `t + 1`. Where the notes say "policy year k" they mean the
  contractual label, i.e. `t = k − 1`.
- **Timing conventions [std].** Premiums received at the start of each policy year
  ...
  (start of `t = 0`, the first policy year).
- **Age basis.** Age nearest birthday at entry, plus curtate policy year — attained
  age in year `t` is `x + t` **[std]**. (The fetched product documents do not
...
| Renewal commission | 2.5% of premiums from policy year 2 (`t ≥ 1`) | **[std]** |
...
| Symbol | Meaning |
|---|---|
| `t` | year index, 0-based from issue, t = 0..n−1 (policy year = t + 1); attained age in year t = x + t (x = issue age) |
| `k` | policy month, k = 0..N, N = 12n (monthly grid / benefit schedules); year t spans months 12t..12t+12 |
...
| `l(t)` | in-force probability at start of year t; l(0) = 1 |
...
| `E0`, `e(t)` | initial expense (150); maintenance expense = 30 × 1.03^t |
| `c0`, `c_r` | initial commission (1.5 × P_a); renewal commission rate (0.025, from t = 1) |
...
generates expected instalment outgo:
    ...
    FIB outgo in later year u, s < u ≤ n − 1:  12 × I × D(s)

so total FIB claim outgo in year t is

    Claims_fib(t) = I × [ 6 × D(t) + 12 × FIBcum(t) ],   FIBcum(t) = Σ_{s<t} D(s)

and one death in year s generates `6 + 12(n − 1 − s)` instalments in all — exactly
`N − k` at the mid-year death month `k = 12s + 6`.
...
Annual processing for year t = 0..n−1 **[std]**:
...
1. **Start of year:** premium income `P_a × idx_p(t) × l(t)` (where `idx_p(t)` is the
   ...
   t ≥ 1). At t = 0 additionally `E0` and `c0` (per policy issued, l(0) = 1).
...
Level/decreasing shapes:
    ...
    CF(t) = P_a × idx_p(t) × l(t)                                   (premiums)
          ...
          − e(t) × l(t)                                             (maintenance)
          − c_r × P_a × idx_p(t) × l(t) × 1{t ≥ 1}                  (renewal commission)
...
placeholders `q(0) = 0.00055, q(1) = 0.00060, q(2) = 0.00065` are **[std]
...
216.00, `e(t)` = 30 × 1.03^t, `c_r` = 2.5% from t = 1, `ec` = 250. Rows are labelled
by the 0-based `t`; policy year = t + 1.

| t | l(t) | Premiums `P_a·l(t)` | Claims `SA0·D(t)` | Claim exp `ec·D(t)` | Maint. + initial exp | Commission | Net CF(t) |
|---|---|---|---|---|---|---|---|
| 0 | 1.000000 | 144.00 | 82.50 | 0.14 | 180.00 | 216.00 | −334.64 |
| 1 | 0.899505 | 129.53 | 80.96 | 0.13 | 27.79 | 3.24 | +17.41 |
| 2 | 0.827048 | 119.09 | 80.64 | 0.13 | 26.32 | 2.98 | +9.02 |

Trace, t = 0 (policy year 1): `D(0) = 1.0 × 0.00055 = 0.00055`; claims = 150,000 ×
0.00055 = 82.50; claim expense = 250 × 0.00055 = 0.14; expenses = E0 + e(0) = 150.00 +
30.00 = 180.00; commission = c0 = 216.00; CF(0) = 144.00 − 82.50 − 0.14 − 180.00 −
216.00 = −334.64.
Update: `l(1) = 1.0 × (1 − 0.00055) × (1 − 0.10) = 0.899505`.

Trace, t = 1 (policy year 2): premiums = 144 × 0.899505 = 129.53; `D(1) = 0.899505 ×
0.00060 = 0.000540`; claims = 150,000 × 0.000540 = 80.96; claim expense = 0.13;
maintenance = 30 × 1.03 × 0.899505 = 27.79; renewal commission = 0.025 × 129.53 = 3.24;
CF(1) = 129.53 − 80.96 − 0.13 − 27.79 − 3.24 = +17.41.
Update: `l(2) = 0.899505 × (1 − 0.00060) × (1 − 0.08) = 0.827048`.
