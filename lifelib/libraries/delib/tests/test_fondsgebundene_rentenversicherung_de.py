...
tax-threshold lapse step at 300 to 301 and the last two -- together with **every column
total at full precision**, which is where a slice of rows cannot hide an error.  Goldens are
hard-coded rather than pickled so a reviewer can compare them with the notes by eye, and
tolerances follow the precision the notes display: money to the cent, ``pols_if`` and unit
counts to six decimals, rates to eight.

Beyond the worked example this module asserts: the notes' three independent rebuilds (month
...
# non-unit boundary; every other benefit is the policyholder's own fund coming back.
...
# `fund` death benefit so charge_risk is 0.00 everywhere.
# t: (pols_if, premiums, prem_to_av, charge_acq, charge_admin_prem, charge_admin_fund,
...
# The reduction in yield across the four shipped charge scales.  The cells differ in
...
def test_worked_example_panel_c_row(de_frv_anchor, t):
    ...
    ``AFT_WD`` equals ``AFT_CHARGE`` throughout, there being no *Teilentnahme* on this
    cell, and the notes omit the column for that reason; it is asserted here so the
    omission stays a product fact rather than a gap.
    """
...
def test_the_einmalbeitrag_totals_and_conversion(fondsgebundene_rentenversicherung):
    ...
    A single premium has no *Beitragssumme* to zillmer against and no five-year spreading
    to obey, so the window is zero months and the instalment count one.
    """
    p = fondsgebundene_rentenversicherung.Projection[2]
    df = p.result_cf()
...
def test_the_nettotarif_gap_is_the_acquisition_load(fondsgebundene_rentenversicherung):
    ...
    exceeds the tariff's own charge and those cells carry a projected loss.  That is the
    flat assumption showing, not a product fact -- a real *Nettotarif* pays no acquisition
    commission at all -- and model.md says so rather than leaving it to be discovered.
    """
...
def test_pitfall_15_the_applied_factor_is_the_higher_of_guaranteed_and_current(
    ...
    # The Kapitalwahlrecht is a reporting split and moves no cash flow.
...
def test_the_in_force_roll_forward_closes_every_month(de_frv_anchor):
    """Everyone who starts a month dies, surrenders, reaches Rentenbeginn or stays."""
    p = de_frv_anchor
    n = p.proj_len()
...
def test_the_two_result_frames_have_the_shape_the_notes_publish(de_frv_anchor):
    """Eighteen cash flow columns in the notes' order, and the Standmitteilung state vector."""
    p = de_frv_anchor
    df = p.result_cf()
    ...
    assert df.index.name == "t" and list(df.index) == list(range(1, 361))
    ...
    assert fund.index.name == "t" and list(fund.index) == list(range(1, 361))
    assert (fund["units_pp"] >= 0.0).all() and (fund["nar_pp"] >= 0.0).all()
...
def test_the_shipped_tables_mark_their_own_provenance():
    """Six CSVs beside run.py, each saying what it is and -- for two -- what it is not.
