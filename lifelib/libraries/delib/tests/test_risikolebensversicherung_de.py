...
**The frame is 0-based.**  ``t = 0`` is the first policy year, the contractual policy year
is ``t + 1``, ``proj_len()`` is the number of policy years and the frame's *exclusive* end,
so ``result_cf()`` runs ``t = proj_start() .. proj_len() - 1`` and an in-force point opens
at ``t = duration_y``.  Every golden **value** below is the notes' own; only the keys and
the literal ``t`` arguments moved down by one.  The three schedule CSVs stay keyed on the
1-based ``policy_year``, so the CSV assertions near the bottom of the module are unmoved.
...
* the first-order *Deckungskapital*: zero at ``t = 0`` by the equivalence, zero at
  ``t = n`` by exhaustion, a peak of 7 553,29 EUR at ``t = 15``, and the Thiele step;
...
#     commissions, net_cf) -- the notes' worked-example table, in full.  claims_lapse,
# claims_maturity and liability_cf are omitted there for width and asserted in the row
# test all the same: the two zeros are a statutory fact about this product.
WORKED_EXAMPLE = {
    0:  (35, 1.000000, 1275.41, 733.01, 542.40, 183.24, 271.57, 637.71, -359.51),
    1:  (36, 0.939408, 1198.13, 688.60, 509.54, 188.49, 107.06, 6.89, 386.16),
    2:  (37, 0.901210, 1149.41, 660.60, 488.82, 198.01, 104.37, 6.61, 351.61),
    3:  (38, 0.864508, 1102.60, 633.69, 468.91, 214.42, 101.76, 6.34, 311.18),
    4:  (39, 0.837880, 1068.64, 614.18, 454.47, 227.56, 100.24, 6.14, 280.24),
    5:  (40, 0.812008, 1035.64, 595.21, 440.43, 241.48, 98.74, 5.95, 249.03),
    6:  (41, 0.786867, 1003.58, 576.78, 426.80, 256.23, 97.27, 5.77, 217.51),
    7:  (42, 0.762432, 972.41, 558.87, 413.54, 271.86, 95.81, 5.59, 185.60),
    8:  (43, 0.738680, 942.12, 541.46, 400.66, 288.42, 94.38, 5.41, 153.25),
    9:  (44, 0.715587, 912.67, 524.53, 388.13, 305.94, 92.96, 5.25, 120.39),
    10: (45, 0.693130, 884.03, 508.07, 375.95, 324.49, 91.56, 5.08, 86.94),
    11: (46, 0.671287, 856.17, 492.06, 364.11, 344.12, 90.17, 4.92, 52.85),
    12: (47, 0.650036, 829.06, 476.48, 352.58, 364.89, 88.79, 4.76, 18.04),
    13: (48, 0.629355, 802.69, 461.32, 341.36, 386.84, 87.43, 4.61, -17.56),
    14: (49, 0.609224, 777.01, 446.57, 330.44, 410.04, 86.09, 4.47, -54.02),
    15: (50, 0.589621, 752.01, 432.20, 319.81, 434.55, 84.75, 4.32, -91.42),
    16: (51, 0.570527, 727.66, 418.20, 309.45, 460.42, 83.42, 4.18, -129.82),
    17: (52, 0.551923, 703.93, 404.57, 299.36, 487.72, 82.10, 4.05, -169.30),
    18: (53, 0.533788, 680.80, 391.27, 289.53, 516.51, 80.78, 3.91, -209.93),
    19: (54, 0.516105, 658.25, 378.31, 279.94, 546.84, 79.47, 3.78, -251.78),
    20: (55, 0.498853, 636.24, 365.67, 270.58, 578.77, 78.17, 3.66, -294.93),
    21: (56, 0.482016, 614.77, 353.32, 261.45, 612.36, 76.86, 3.53, -339.44),
    22: (57, 0.465576, 593.80, 341.27, 252.53, 647.67, 75.56, 3.41, -385.37),
    23: (58, 0.449515, 573.32, 329.50, 243.82, 684.73, 74.25, 3.29, -432.78),
    24: (59, 0.433815, 553.29, 317.99, 235.30, 723.59, 72.94, 3.18, -481.72),
}
# The notes' Total row, summed at full precision and then rounded -- and, beside it, the
# same columns summed from the twenty-five *rounded* cells.  Five of the seven differ.
...
RESERVE = {"peak_t": 15, "peak": 7553.290695, "next": 7511.937517,
...
DECL_WITHDRAWN = {
    0:  (1.000000, 1275.41, 1275.41, 0.00, 183.24, 287.85, 637.71, 166.62),
    1:  (0.939408, 1198.13, 1198.13, 0.00, 188.49, 122.34, 11.98, 875.32),
    2:  (0.901210, 1149.41, 1149.41, 0.00, 198.01, 119.04, 11.49, 820.88),
    12: (0.650036, 829.06, 829.06, 0.00, 364.89, 99.37, 8.29, 356.51),
    23: (0.449515, 573.32, 573.32, 0.00, 684.73, 81.57, 5.73, -198.71),
    24: (0.433815, 553.29, 553.29, 0.00, 723.59, 80.00, 5.53, -255.84),
}
...
# WORKED_EXAMPLE.
EINMAL = {
    0: (50, 1.000000, 6676.93, 3728.98, 2947.95, 238.29, 175.87, 133.54, 3181.28),
    1: (51, 0.937691, 0.00, 0.00, 0.00, 244.67, 29.32, 0.00, -274.00),
    2: (52, 0.897762, 0.00, 0.00, 0.00, 256.51, 28.68, 0.00, -285.19),
    3: (53, 0.859312, 0.00, 0.00, 0.00, 277.16, 28.05, 0.00, -305.21),
    4: (54, 0.830845, 0.00, 0.00, 0.00, 293.44, 27.71, 0.00, -321.15),
    5: (55, 0.803073, 0.00, 0.00, 0.00, 310.58, 27.38, 0.00, -337.95),
    6: (56, 0.775968, 0.00, 0.00, 0.00, 328.60, 27.04, 0.00, -355.64),
    7: (57, 0.749502, 0.00, 0.00, 0.00, 347.55, 26.70, 0.00, -374.24),
    8: (58, 0.723645, 0.00, 0.00, 0.00, 367.44, 26.35, 0.00, -393.79),
    9: (59, 0.698372, 0.00, 0.00, 0.00, 388.29, 26.01, 0.00, -414.30),
}
...
def test_worked_example_row(de_rlv_anchor, t):
    """Every cell of the notes' twenty-five-row table, to the displayed precision -- with
    the two zero claim columns and ``liability_cf``, which the table omits for width.
    """
...
def test_the_worked_example_totals_are_summed_at_full_precision(de_rlv_anchor):
    ...
    Five of the seven money columns differ by a cent, and the notes print both; asserting
    ...
    for column, rounded in ROUNDED_CELL_SUMS.items():
        cells = sum(round(float(df.loc[t, column]), 2) for t in df.index)
...
def test_the_anchor_cell_is_the_configuration_the_notes_describe(de_rlv_anchor):
    ...
    assert (p.proj_start(), p.proj_len(), p.cover_end_age()) == (0, 25, 60)
...
def test_the_first_period_rebuilt_from_the_table_rate_up(de_rlv_anchor):
    """The notes' second check, arithmetic a reader can follow: ``q2(0) = 0,00040 x
    1,095^5``, a claim inside the paragraph 161 window paying 291 000 EUR, and an expense
    line of four numbers of which only one is large.  ``t = 0`` is policy year 1.
    """
    p = de_rlv_anchor
    q2 = 0.00040 * 1.095 ** 5
    ...
    assert p.claims(0, "DEATH") == AP(291000.0 * q2, abs=5e-6)   # = 183,241389
    g = p.prem_gross_level_pp()
    acq_net = (0.025 - 0.020) * 25 * g
    admin = 0.00030 * 300000.0
    collection = 0.03 * p.prem_paid_pp(0)
    claim_exp = 250.0 * q2
    assert acq_net == AP(159.426485, abs=5e-6) and admin == AP(90.0, rel=1e-15)
    assert collection == AP(21.990342, abs=5e-6) and claim_exp == AP(0.157424, abs=5e-6)
    assert p.expenses(0) == AP(acq_net + admin + collection + claim_exp, rel=1e-12)
    assert p.expenses(0) == AP(271.574251, abs=5e-6)
    assert p.commissions(0) == AP(0.020 * 25 * g, rel=1e-12)
    assert p.commissions(0) == AP(637.705941, abs=5e-6)
    assert p.net_cf(0) == AP(-359.510179, abs=5e-6)
    # The first period's strain is the initial commission: 87 % of its billed premium.
    assert p.commissions(0) / p.premiums(0) == AP(0.87, abs=0.005)


def test_the_third_period_rebuilt_through_two_decrement_steps(de_rlv_anchor):
    """The notes' third check: l(1) and l(2) built by hand, then claims_death(2) -- the
    third policy year on the 0-based frame.  The contractual 300 000 EUR instead of the
    paragraph 161 benefit gives 204,13 against 198,01 -- a 3 % error running three years,
    which a totals-only test would miss.
    """
    p = de_rlv_anchor
    q0, q1, q2 = (0.00040 * 1.095 ** k for k in (5, 6, 7))
    l1 = (1.0 - q0) * (1.0 - 0.06)
    l2 = l1 * (1.0 - q1) * (1.0 - 0.04)
    assert l1 == AP(0.93940809, abs=5e-9) and l2 == AP(0.90120993, abs=5e-9)
    assert p.pols_if(1) == AP(l1, rel=1e-12) and p.pols_if(2) == AP(l2, rel=1e-12)
    assert p.mort_rate(1) == AP(q1, rel=1e-12) and p.mort_rate(2) == AP(q2, rel=1e-12)
    assert p.claims(2, "DEATH") == AP(291000.0 * l2 * q2, abs=5e-6)
    assert p.claims(2, "DEATH") == AP(198.005742, abs=5e-6)
    wrong = 300000.0 * l2 * q2
    assert wrong == AP(204.13, abs=CENT)
    assert wrong / p.claims(2, "DEATH") == AP(1.0 / 0.97, rel=1e-12)


def test_closure_one_the_decrements_account_for_the_whole_policy(de_rlv_anchor):
    """Deaths + lapses + expiries = pols_if_init() exactly, and pols_if(n) = 0 -- the state
    one step past the last projected period.  The split between the last two is what
    ``w(24) = 0`` decides and no cash flow moves either way, which is why the identity and
    not the split is load-bearing.
    """
    p = de_rlv_anchor
    n = p.proj_len()
...
def test_closure_two_the_suicide_wedge_is_the_only_gap_between_events_and_amounts(
        de_rlv_anchor):
    """9 916,82 of events at the contractual sum against 9 899,20 paid, and the 17,62 gap
    is exactly ``0,03 x 300 000`` times the death rates of ``t = 0, 1, 2``, policy years 1
    to 3.  A switch applied to every period, to a lapse, or over the wrong window breaks
    this and no total.
    """
    p = de_rlv_anchor
    n = p.proj_len()
    ...
    assert 300000.0 * events == AP(9916.822736, abs=5e-6)
    assert paid == AP(9899.201951, abs=5e-6)
    wedge = 300000.0 * events - paid
    assert wedge == AP(17.620785, abs=5e-6)
    first_three = sum(p.pols_death(t) for t in (0, 1, 2))
    assert first_three == AP(0.00195786504, abs=5e-12)
    assert 0.03 * 300000.0 * first_three == AP(wedge, rel=1e-9)


def test_closure_three_the_cash_flow_statement_is_check_net_cf(de_rlv_anchor):
    """12 243,747304 - 9 899,201951 - 2 396,505040 - 752,813300 = -804,772987.  Which
    ...
    assert prem == AP(12243.747304, abs=5e-6) and claims == AP(9899.201951, abs=5e-6)
    assert exp == AP(2396.505040, abs=5e-6) and comm == AP(752.813300, abs=5e-6)
    assert prem - claims - exp - comm == AP(-804.772987, abs=5e-6)
...
def test_the_declaration_withdrawn_variant_row(risikolebensversicherung, t):
    ...
    try:
        ...
        assert p.commissions(t) == AP(comm, abs=CENT)
        assert p.net_cf(t) == AP(net, abs=CENT)
        assert p.premiums(t) == AP(p.prem_gross(t), rel=1e-15)
    finally:
        model.close()
...
def test_the_einmalbeitrag_variant_row(risikolebensversicherung, t):
    ...
    outgo, the shape inverting against the level-premium cell.
    """
...
def test_the_einmalbeitrag_is_the_same_engine_at_a_boundary(risikolebensversicherung):
    ...
    *Beitragssumme*, so more of the gross premium is risk and more of it comes back.
    ...
    assert sum(round(float(df.loc[t, "net_cf"]), 2) for t in df.index) == AP(119.81, abs=CENT)
    assert all(p.commissions(t) == 0.0 for t in range(1, 10))
    assert all(p.claims(t, "DEATH") > 0.0 and p.net_cf(t) < 0.0 for t in range(1, 10))
...
def test_pitfall_2_the_model_carries_two_premium_streams(
    ...
    assert all(none.prem_gross(t) == none.premiums(t) for t in range(25))
    df = none.result_cf()
...
def test_pitfall_4_a_lapse_pays_nothing_at_any_duration(
    ...
    assert all(p.check_no_cash_value_resid(t) == 0.0 for t in (0, 11, n - 1))
...
def test_pitfall_5_a_level_premium_against_a_rising_rate_builds_a_reserve(de_rlv_anchor):
    ...
    *gezillmerte* companion is negative from the first day and zero at expiry.
    ...
    assert p.res_pp_at(0, "BEF_PREM") == AP(0.0, abs=1e-9)
    assert p.res_pp_at(n, "BEF_PREM") == 0.0
    interior = [p.res_pp_at(t, "BEF_PREM") for t in range(1, n)]
    ...
    assert all(p.check_res_roll_fwd_resid(t) == AP(0.0, abs=1e-6)
               for t in (0, 7, 15, n - 1))
...
def test_pitfall_6_sex_never_reaches_the_premium(risikolebensversicherung):
    ...
    assert m_df["net_cf"].sum() == AP(-804.77, abs=CENT)
    assert f_df["net_cf"].sum() == AP(4158.46, abs=CENT)
...
def test_pitfall_8_the_suicide_switch_runs_three_years_and_touches_death_alone(
    ...
    p = de_rlv_anchor
    n = p.proj_len()
    assert p.suicide_factor(0) == p.suicide_factor(1) == p.suicide_factor(2) == 0.97
    assert all(p.suicide_factor(t) == 1.0 for t in range(3, n))
    assert p.benefit_paid_pp(2) == AP(0.97 * p.benefit_pp(2), rel=1e-15)
    ...
    assert inforce.duration_y() == 12 and inforce.proj_start() == 12
    assert all(inforce.suicide_factor(t) == 1.0 for t in range(12, 30))
    assert list(inforce.result_cf().index) == list(range(12, 30))
    short = risikolebensversicherung.Projection[13]
    assert short.proj_len() == 5 and short.issue_age() == 60
    assert [short.suicide_factor(t) for t in range(5)] == [0.97, 0.97, 0.97, 1.0, 1.0]
...
def test_pitfall_9_each_increment_carries_its_own_three_year_window(
    ...
    assert (p.sum_uplift(4), p.sum_uplift(5)) == (1.0, 1.2)
    assert (p.sum_uplift(10), p.sum_uplift(11)) == (1.2, 1.4)
    assert p.benefit_pp(5) == AP(1.2 * p.sum_assured(), rel=1e-12)
    assert all(p.suicide_factor(t) == AP(0.97, rel=1e-15) for t in (0, 1, 2))
    assert all(p.suicide_factor(t) == AP(1.0, rel=1e-15)
               for t in (3, 4, 8, 9, 10, 14, 27))
    for t in (5, 6, 7):
        assert p.suicide_factor(t) == AP(1.0 - 0.03 * (0.2 / 1.2), rel=1e-12)
        assert 0.97 < p.suicide_factor(t) < 1.0
    for t in (11, 12, 13):
        assert p.suicide_factor(t) == AP(1.0 - 0.03 * (0.2 / 1.4), rel=1e-12)
        assert 0.97 < p.suicide_factor(t) < 1.0
    assert {risikolebensversicherung.Projection[1].sum_uplift(t)
            for t in range(25)} == {1.0}
...
def test_pitfall_10_the_frequency_loading_is_applied_once(risikolebensversicherung):
    """phi multiplies the billed amount, so both streams and the rebate carry it: on the
    monthly point ``prem_gross_pp(0)`` is exactly 1.05 times the unloaded level premium.
    Loading each stream separately breaks the split identity at every frequency.
    """
...
def test_pitfall_11_the_premium_cessation_rule_is_applied_once(de_rlv_anchor):
    """Premiums are in advance and claims at period end, so a claimant has already paid.  A
    further ``(1 - q2)`` factor applies the rule twice, costing 0,46 EUR in the first period
    here and more as the death rate climbs.
    """
    p = de_rlv_anchor
    for t in (0, 12, 24):
        assert p.premiums(t) == AP(p.prem_paid_pp(t) * p.pols_if(t), rel=1e-15)
    ...
    twice = p.prem_paid_pp(0) * p.pols_if(0) * (1.0 - p.mort_rate(0))
    assert p.premiums(0) - twice == AP(733.011403 * 0.00062969550, abs=5e-6)  # 0,4616 EUR
    # The claim expense, by contrast, *is* charged on the deaths and only on them.
    assert p.expenses(24) - p.maint_pp(24) * p.pols_if(24) == AP(
        250.0 * p.pols_death(24), rel=1e-12)
...
def test_pitfall_12_the_premium_stops_at_the_beitragszahlungsdauer(
        risikolebensversicherung):
    """Model point 6 has k = 12 against n = 20: the last premium falls at t = 11 and all
    three premium cells are zero from t = 12 while ``claims_death`` runs to expiry and the
    reserve falls, and ``check_prem_split()`` guards the zero branch as well as the paying
    one.
    """
    p = risikolebensversicherung.Projection[6]
    assert p.prem_term() == 12 and p.policy_term() == 20 and p.prem_gross_pp(11) > 0.0
    ...
    assert p.check_prem_split() is True
    reserve = [p.res_pp_at(t, "BEF_PREM") for t in range(12, 20)]
...
def test_pitfall_13_the_three_versicherungssumme_shapes_are_a_schedule(
    ...
    assert {flat_cell.benefit_pp(t) for t in range(25)} == {300000.0}
    ...
    assert linear.benefit_pp(n - 1) == AP(250000.0 / n, rel=1e-12)
    steps = {round(linear.benefit_pp(t) - linear.benefit_pp(t + 1), 6) for t in range(n - 1)}
    ...
    first = annuity.benefit_pp(0) - annuity.benefit_pp(1)
    last = annuity.benefit_pp(m - 2) - annuity.benefit_pp(m - 1)
...
def test_pitfall_16_the_lapse_table_is_the_term_life_one(
        risikolebensversicherung, de_rlv_anchor):
    """6 % / 4 % / 4 % / 3 %, and zero in the final period -- a property of the last policy
    year, living in the formula and not in the table, whose own row still reads 3 %.  The
    table is keyed on the 1-based ``policy_year``, so ``lapse_rate(0)`` reads its row 1.
    The GDV whole-market *Stornoquote* is nowhere near it and is not used.
    """
    p = de_rlv_anchor
    assert p.lapse_rate(0) == 0.06 and p.lapse_rate(1) == p.lapse_rate(2) == 0.04
    assert all(p.lapse_rate(t) == 0.03 for t in range(3, 24))
    assert p.lapse_rate(24) == 0.0 and p.lapse_rate_base(24) == 0.03
    assert p.pols_lapse(24) == 0.0 and p.pols_lapse(23) > 0.0
    assert p.lapse_rate(0) > 0.0272 and p.lapse_rate(3) > 0.012
    ...
    assert longest.lapse_cum(39) == AP(0.71063050, abs=5e-9)
...
def test_pitfall_17_a_risikozuschlag_loads_the_mortality_and_never_the_benefit():
    ...
    try:
        ...
        rated_benefit = [rated.benefit_pp(t) for t in (0, 8, 17)]
        ...
        try:
            ...
            assert [plain.benefit_pp(t) for t in (0, 8, 17)] == rated_benefit
...
def test_all_five_checks_hold_on_the_anchor_with_zero_residuals(de_rlv_anchor):
    ...
    for t in (0, 1, 12, n - 2, n - 1):
        ...
        assert p.check_res_roll_fwd_resid(t) == AP(0.0, abs=1e-6)
...
def test_result_cf_shape_and_both_signs_of_the_net_flow(de_rlv_anchor):
    ...
    0-based frame, with ``liability_cf`` the eleventh and ``-net_cf`` exactly.
    """
    df = de_rlv_anchor.result_cf()
    ...
    assert list(df.index) == list(range(25)) and df.index.name == "t"
    assert df.index[0] == de_rlv_anchor.proj_start() == 0
    assert df.index[-1] == de_rlv_anchor.proj_len() - 1
    assert len(df) == de_rlv_anchor.proj_len() - de_rlv_anchor.duration_y()
    ...
    # A first-period strain, thin positive years, then a crossover at t = 13.
    assert df["net_cf"].iloc[0] == AP(-359.51, abs=CENT)
    assert (df["net_cf"].loc[1:12] > 0.0).all() and (df["net_cf"].loc[13:24] < 0.0).all()
...
def test_the_shipped_tables_mark_their_own_provenance():
    ...
    # the 0-based frame does not move: the readers map through ``t + 1``.
...
def test_round_trip_is_stable(tmp_path):
    ...
    try:
        ...
        for t, row in WORKED_EXAMPLE.items():
            assert p.prem_gross(t) == AP(row[2], abs=CENT)
            assert p.premiums(t) == AP(row[3], abs=CENT)
