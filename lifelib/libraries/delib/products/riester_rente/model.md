...
| Notes | Cells | Why |
|---|---|---|
| `Z*(t)`, `Ẑ(t)`, `Z(t)` | `zulage_entitlement_pp` / `zulage_granted_pp` / `zulage_pp` | Three amounts, and the product turns on the difference between them |
| `C(t)` | `contrib_total_pp` | The notes' `C` is the contribution *credited*; the cells is the cash **received**, so it carries the *Ratenzuschlag* that `admin_charge_pp` takes back out |
