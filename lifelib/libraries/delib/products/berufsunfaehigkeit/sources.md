...
`Fetched: no`**, and that page has not been touched here. Where this pass read the instrument
itself, the [R#] entry above records it and the [REG-R#] tag inherits nothing further.
...
retrieved**; the retrieval-conditions statement at the head of this file is superseded by the
per-entry `Retrieved` lines above and is rewritten separately.
