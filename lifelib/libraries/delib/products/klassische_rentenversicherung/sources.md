...
- Used for: the other half of that comparison — the **"12/62 rule"**, at least twelve years of contract duration and payment after completion of the 62nd year of life; that where it is met **only half the gain is taxable** (*Halbeinkünfteverfahren*), and that this applies **only to lump sums and payout-plan withdrawals, not to monthly annuity payments**; the 60/62 split at the 31 December 2011 line; and the **pre-2005 cohort rule — that the recast § 20 Abs. 1 Nr. 6 does *not* reach contracts concluded before 1 January 2005, whose *rechnungsmäßige und außerrechnungsmäßige Zinsen* are entirely exempt where the predecessor regime's conditions are met, so the *Altvertrag* is better off than the half-income cohort, not merely grandfathered into it**; those conditions themselves are [unverified] and asserted nowhere in delib. It is also the argued shape behind the **duration-12 step in `lapse_table.csv`**, whose levels remain `[std]`
