...
at risk and the three decrement rates. That second frame is roughly what a German
*Standmitteilung* reports [S17], which is no coincidence: the statement's line items are this
model's state vector. `model.doc` describes the product and the basis,
`model.Projection.doc` maps the notes' symbols onto the cells names, and `model.Data.doc`
says what each input file is and what a replacement must preserve.
...
euro without the matching units being cancelled while every euro total still looks plausible;
the account identity fails when the month's return is applied at the wrong point in the
order. An implementation can pass either one alone.

The fund's **TER** is a third category and is deliberately given no `charge_*` cells: borne
...
explicitly double-counts; ignoring it overstates the policyholder's return. On the base path
5.00 % gross less a 0.45 % TER is 4.55 % p.a., `(1.0455)^(1/12) − 1 = 0,371482 %` a month.
...
shortened term still spread over sixty months would understate every instalment and leave the
ledger short. `check_acq_charge()` closes it against an expectation **counted rather than
...
Every entry is **[std]**. The rationale is what makes it honest; a guessed figure dressed as
an observation would not be.

| Standardization | Value | Rationale |
|---|---|---|
| Acquisition rate, composite | 2.50 % of the *Beitragssumme* | The rate is the *Höchstzillmersatz* [R12] [REG-R16], the only anchor in the stack; taking **the cap** demonstrates the binding constraint instead of inventing a level |
...
| *Zuzahlungskosten* | 2.50 % | Set equal to the regular rate; on a single premium it is the whole acquisition charge, there being no *Beitragssumme* to zillmer |
...
| Lapse cap / final month | 40 % / `lapse_rate_mth(n) = 0` | A cap so a dynamic module cannot produce an absurd rate; a zero final month because a surrender and an annuitisation are then one event |
...
| *Kickback* credited back | 0.00 % p.a. | A passive fund pays no trail, sidestepping two unresolved questions: whether an insurer may retain a *Bestandsprovision* [R15], and how a credited rebate enters the PRIIPs cost calculation [R7] [R8] |
...
| Timing and processing order | premium in advance, return, fund charges, *Teilentnahme*, *Risikobeitrag*, deaths before lapses | The *Bewertungsstichtag* lag disappears on a monthly grid; observing the amount at risk before the charge that prices it makes `death_strain` exactly the *riskiertes Kapital* |
| Negative-fund safeguards | `min(.., remaining)` on the *Stückkosten* and the *Risikobeitrag* | A floor, not a tariff term; **no shipped model point triggers one** |
