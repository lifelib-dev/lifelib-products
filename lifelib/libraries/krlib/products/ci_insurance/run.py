...
print("projection = {} years (t = 0 .. {}) to attained age {}   CI cover runs {} years, "
      "to age 100".format(proj.proj_len(), proj.proj_len() - 1, proj.omega_age(),
                          proj.ci_cover_end()))
print("pyojun haeyak gongje-aek (statutory surrender-charge cap) = KRW {:,.2f}".format(
    proj.surr_chg_cap_pp()))
print("modules: lapse basis = {}   loan utilisation = {:.2%} at anniversary {}   "
...
df = proj.result_cf()
rows = [t for t in range(12) if t < proj.proj_len()]
m = proj.prem_period()
rows += [t for t in (m - 2, m - 1, m) if t < proj.proj_len() and t not in rows]
print("first twelve policy years (t = 0 .. 11), and the years around the end of the "
      "premium term:")
...
val = proj.result_val()
print("the acceleration, at three anniversaries (per policy, KRW):")
for d in (5, 10, m):
    if d > proj.proj_len():
        continue
    print("  anniversary {:>2} (the close of year t = {:>2})  account V = {:>14,.0f}   "
          "surrender pre-CI = {:>14,.0f}   post-CI = {:>14,.0f}".format(
              d, d - 1, val.loc[d - 1, "pol_val_pp"], val.loc[d - 1, "cv_pp"],
