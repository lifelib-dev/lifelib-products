...
recursions of `VA_KR_S` from this document alone, without returning to the research file.
Every parameter the model needs is therefore tabulated here with its base, its timing and
...
income peaked at ₩17.9 trillion (17.9조원) in 2021 and has fallen every year since —
₩12.7tn (2022), ₩12.2tn (2023), ₩11.6tn expected (2024), ₩10.1tn forecast (2025) — the
decline attributed to market uncertainty and explicitly to 「최저보증이율의 하락」 reducing
demand for guaranteed variable annuities [R9]. The cross-product forecast series puts the
line at ₩12.4tn for 2026 [REG-R46]. *New* business moved the other way: 초회보험료
(first-year premium) was ₩1.97tn in 2024 and **₩2.89tn in 2025, up 46.2%** [R3] — a
forecast of a 45.9% *fall* had been published a year earlier [R9], which is a standing
warning against treating any Korean variable-annuity volume projection as reliable. The
last retrieved stock measurement is ₩109.1tn of 변액보험 적립금 across about 8.3 million
policies at 2016-09-30, 「국민 약 6명당 1건」 [R1].

**Conduct.** The combination of a shrinking in-force block and a fast-growing new-business
line draws supervisory attention. The FSS ran a mystery-shopping exercise on variable
insurance sales over September–November 2025 across nine of twenty-two life insurers,
publishing the result on 2026-03-24: overall grade 「양호」, with the two weakest items the
explanation of **변액보험의 자산운용 방식** and of the **위법계약해지권**; 1,308
variable-insurance complaints in 2025, about **9%** of all life-insurance complaints [R3].
The product may be sold only by a 변액보험판매관리사 (*byeonaek boheom panmae gwallisa*)
who has passed the 생명보험협회 examination and completes 「매년 1회 4시간 이상」 of
refresher training [R2] [R6] [S1], and a 적합성 진단 (*jeokhapseong jindan*, suitability
diagnosis) must precede any recommendation to an 일반금융소비자 [R2] [R3] [R14].
...
— one of only two that cross the 특별계정 / 일반계정 boundary [REG-R15]. It is also the
only product in the library **barred by regulation from the 무해지 / 저해지환급형 forms**:
감독규정 제7-66조제4항제1호 excludes 변액보험 from the dispensation that permits a surrender
value below the 별표 14 floor [REG-R19]. A Korean variable annuity must therefore carry a
full 표준해약공제액-bounded surrender value, and the cliff-shaped surrender curve that
dominates `WholeLife_KR_A`, `Term_KR_A`, `CI_KR_A`, `Cancer_KR_S` and `LTC_KR_S` cannot
appear here.
...
1. Six of the retrieved current products divide as follows: guarantee charged and fixed at
   issue [S1] [S10]; guarantee electable and revocable an unlimited number of times, at
   연 0.85% of the account value while it is on [S9]; guarantee present but **unfunded**,
   paid for with a CPPI overlay instead of a fee [S6]; and no GMAB offered at all
   [S4] [S5]. The 2017 census of all 36 products on sale found **27** guaranteeing the
   annuity fund, 3 of those letting the policyholder decline, and **9 designed with no
   GMAB** [R1] — the report gives 26 in one place and 27 in another, and the discrepancy is
   in the source. The composite takes the **charged, issue-fixed 보증형** because it is the
   only shape that exercises a guarantee charge, a guarantee base and a guarantee payout in
   one run; the elective form [S9] would make the charge a policyholder-behaviour variable
   with no retrieved take-up data, and the CPPI form [S6] replaces the charge with an asset
   rule that a single deterministic path cannot distinguish from a fixed allocation.
2. Four level rules are documented: **premium refund** (「대부분의 상품은 일정비율을
   100%로 설정」), **step-up** (110/120/130% of premiums paid, ratcheting on attainment),
   **ratchet** (periodic re-observation of the account value) and **roll-up** (premiums
   accumulated at a stated rate) [R1 <표 Ⅲ-2>]. Twenty-two of the 36 census products set
   the guarantee **above** premiums paid [R1]. The composite takes premium refund at 100%
   because it is the textbook identity — 「연금개시시 계약자적립금(최저보증 포함) =
   Max(기납입보험료, 연금개시시 계약자적립금)」 [R2] — because it is the level the only
   retrieved charged-GMAB contract uses [S1], and because it is the only rule whose strike
   is a modelled quantity rather than a second recursion. The roll-up base and the ratchet
   are specified in full under "Contractual mechanics" and carried as variants, since
   [R1]'s fifteen-year worked comparison shows that **the guarantee design, not the
   guarantee level, is what drives the option cost**.
3. Korean variable annuities are written in 구좌 — repeatable units of the basic premium —
   with the withdrawal residual floor and the maximum basic premium both expressed per
   구좌 [S1] [S2] [S10]. One 구좌 keeps the model point one contract.
4. The 계약체결비용 taken below is a tied-agent level. Bancassurance and online acquisition
   costs were capped at **50%** of the tied-agent level from 2016 [R1], and the one
   variable annuity [R1] found buyable directly online carried 계약체결비용 of **1.3%** of
   the basic premium against the 5.17–6.12% of the tied-channel documents retrieved here —
   roughly a quarter of the price, with **no acquisition commission at all**. Channel is
   therefore a first-order parameter and is named rather than left implicit.
5. 남자 40세 / 기본보험료 30만원 / 10년납 / 60세 연금개시 is the illustration point three
   ...
   composite's parameters can be checked against published surrender-value tables. Male,
   because all three publish the male cell. The 20-year pre-annuitisation period puts the
   contract in the **>12년** band of the mandatory bond ladder, where the 채권형 floor is
   50% [S1] [R1] — so a 50/50 allocation sits exactly on the constraint and makes it
   visible. Ten years of premiums with ten years of deferral exercises the step in the
   monthly deduction at 납입완료 (*nabip wallyo*), which is the feature of the Korean
   expense stack a model most easily gets wrong.
...
6. No retrieved 상품요약서 quantifies 기타비용 separately; [R2]'s cash-flow identity names
   it (특별계정 투입보험료 = 납입보험료 − (계약체결비용 + 납입 중 계약유지비용 +
   기타비용)) and [S7 제2조] confirms it is deducted at premium payment, but no rate was
   retrieved. Set to zero so the composite's premium allocation reproduces the observed
   ratio exactly; the line is kept because the identity needs it.
...
9. Both lines are **ex-post estimates of actual spend**, not contractual rates — [S2]
   states its 증권거래비용 figures are estimated from FY2023 spend and its 기초펀드
   보수·비용 from the FY2023 investment mix. Setting them to zero in the base run keeps the
   modelled charges contractual. Their omission understates the drag by up to about 0.5
   percentage points a year at the top of the observed ranges, and the technical notes say
   so.
10. Three scales were retrieved, all on the anchor cell and all the same function
    ...
    ₩1,077,000** [S5] (25.6%, which reproduces the 2017 industry **mean** row exactly) and
    **C = ₩1,180,000** [S4] (28.1%, above the 2017 maximum). The 2017 census range at year
    1 was 19.1%–27.6% with a mean of 25.6% [R1 <표 Ⅴ-2>], so the distribution has not moved
    in eight years. The composite takes **[S2]'s C = ₩830,000** — not the market mean —
    because the 해약공제 is the unamortised 계약체결비용 [R2] and the two must come from
    one carrier: pairing [S2]'s 5.17% acquisition cost with [S5]'s surrender charge would
    recover more on surrender than was ever loaded. The consequence is that the composite
    is a **cheap tied-channel contract at both ends** — bottom of the acquisition-cost band
    and bottom of the surrender-charge band — and the technical notes state the direction
    of that bias. All three observed values sit inside the statutory 표준해약공제액 cap
    computed on the anchor cell (below), at 50%, 66% and 72% of it respectively.
11. Two forms are published: **연금 연액의 0.5%** [S4] and 구좌당 매월
    min(영업보험료의 3.5%, ₩4,000) [S2]. The composite takes the proportional form because
    it is scale-free and needs no reference to a premium that has stopped being paid. The
    capped form is the variant.
...
**First-year charge total on the anchor cell**, at the base-run return, as a check on the
stack: 계약체결비용 ₩186,120 + 계약관리비용 ₩126,000 + 위험보험료 about ₩288 + guarantee
charges about ₩118,200 (of which ₩108,000 is the premium-based GMAB component) ≈
**₩430,600 on ₩3,600,000 of premium, about 12%**. [R1] puts the industry band at 「선취상품은
납입보험료의 5~15%를 계약체결비용(신계약비)과 계약관리비용(유지비)으로 차감한 후 85~95%만
투자」, and the composite's premium allocation of **91.33%** sits inside the 91.3%–91.5%
observed at three carriers on this same cell [S1] [S2] [S6].
...
12. Two funds is the minimum that exercises a pro-rata allocation, a per-fund charge and
    the mandatory bond floor at once. Real menus run from 5 to 51 funds. The 50/50 split is
    the **binding** value of the >12년 bond ladder [S1], chosen so the constraint is
    visible in the model rather than slack; no retrieved document prescribes an allocation.
    No rebalancing in the base run, so the drift of the realised mix away from 50/50 under
    a two-asset return path is itself a modelled quantity — which is what the automatic
    de-risking rules exist to control.
13. **Every return assumption in this product is [std].** The only realised returns
    retrieved are the top of a cross-sectional distribution reported in a trade news
    article — domestic equity 71.32% / 49.01% / 48.04% and overseas equity 44.61% / 44.16%
    / 43.08% annualised at 2026-04-30 [R10] — and one live fund panel showing the opposite,
    a 기준가격 of **904.24원 against the statutory 1,000.00 opening price after fourteen
    ...
    2.50%** [REG-R48], which is the middle of the three returns the 보험업감독규정 requires
    a Korean variable illustration to show [R2], and works back to a gross asset return of
    3.00% so that the 운용보수 is a modelled cash flow rather than an assumption. That the
    disclosed 투자수익률 is already net of the 운용보수 is inferred, not stated: [S6]'s
    illustration shows a gross-to-net gap of **0.01 pp** on a product with no GMAB charge
    and no death cover, which is far too small to contain a 0.4% management fee, while
    [S1]'s gap of **0.32 pp** is exactly its two account-based guarantee charges. The
    inference is sound on that evidence and is marked **[std]**.
...
17. The menu is uniform across carriers: 종신연금형 보증기간부 (10 / 15 / 20 years, to age
    100, or 기대여명보증; 정액형 or 체증형 at 3% [S1], 5% or 10% [S2] [S4] [S5]);
    종신연금형 보증금액부 (the shortfall against the consideration paid as a lump sum on
    early death); 확정연금형 (5 / 10 / 15 / 20 years [S1], plus 25 [S2], plus 30 / 50 / 60
    [S5]); 상속연금형 (interest only, account paid on death); and a front-loaded
    노후설계자금 slice [S1] [S2] [S5] [S6]. Two forms may be combined in 10% steps [S1].
    The composite takes the 10-year-guaranteed level lifetime annuity: it is the modal
    election, it is the only form that exercises both longevity and a guarantee period, and
    it is the form the 소득세법 종신형 연금보험 exemption route is written around, that
    route requiring any guarantee period to sit **within the published 기대여명 연수**
    [REG-R58].
18. No retrieved document states what is paid on an excluded death. The composite pays the
    계약자적립액 without the GMDB top-up, which is the treatment 표준약관 제22조 prescribes
    where a benefit becomes impossible — 「산출방법서에서 정하는 바에 따라 회사가 적립한
    사망 당시의 계약자적립액」 [REG-R25] — and which 상법 제736조 requires as a floor
    [REG-R50]. The choice is **[std]**; the underlying exclusion (2-year 자살면책) is
    sourced [S5] [S6].
...
| 정기중도인출 / 자동인출 서비스 | Out of scope | [S1] [S5] |
...
| 일반계정 전환 (voluntary transfer before annuitisation) | Described, not modelled | [R1 삼성, 동양] [S6] |
...
19. Both are switched off so the base run's account recursion is a clean function of
    ...
    **leaks the guarantee**. Additional premium raises the GMDB and GMAB base; a withdrawal
    lowers it. [R1] is explicit that the withdrawal facility is a guarantee-risk mitigant
    and not only a convenience: 「국내 보험회사는 최저보증리스크 경감과 가입자의 편의를 위해
    중도인출을 허용하고 있다. 중도인출은 정해진 한도하에서 연 12회까지 허용되며,
    **중도인출금은 최저보증한도에서 차감된다**.」 A model that omits the adjustment lets a
    policyholder withdraw the fund and keep the strike.
...
general account's shareholder equity**; and anything else necessary to maintain the account
[REG-R15]. Every row below is one of those transfers.
...
by newspaper advertisement or individual notification, keeping each fund's accounts at head
office for six months after the merger [S7 제37조제4항]. Asset-concentration limits apply to
special accounts on their own scale, separate from and generally higher than the general
account's, under 보험업법 제106조; the retrieved summary did not return their values, which
are **[unverified]** [R5]. [R2] describes the separate account as a 자본시장법 투자신탁 for
reporting purposes, which is why an 자산운용보고서 confirmed by the trustee must be produced
quarterly.
...
**Premium allocation observed.** The textbook publishes the allocation ratios actually used
for variable annuities, as a percentage of premium reaching the fund before the monthly
mortality deduction, by premium term: 일시납 94.4, 3년납 96.6, 7년납 90.9, 10년납 90.0,
15년납 90.0, 20년납 90.0, the same for both sexes at ages 20–45, with the explanation that
a variable annuity carries so little mortality cover that the natural premium is immaterial
— so the allocation is flat by age and **rises** after ten years when the 계약체결비용 stops
[R2]. Three carriers' first-year illustrations put the realised ratio at 91.3%, 91.3% and
91.4% on the anchor cell [S1] [S2] [S6]. The composite's 91.33% sits inside both.
...
The textbook reconciles the 「당일」 and 「전일말」 wordings in a footnote: 「「보험업감독업무
시행세칙」에서는 … 당일 특별계정 순자산가치를 특별계정 총좌수로 나누어 산출하도록 규정하고
있다. 여기서 당일의 의미는 당일초의 순자산가치를 반영해야 한다는 것으로 실제로는 전일말과
동일한 기준이다」 [R2]. Units increase when money enters (premium, repayment of a policy
loan) and decrease when money leaves (the 월공제액, a loan, a withdrawal); **the unit price
moves only with investment performance** [R2]. A price below 1,000 means the fund is below
its launch value, which the one live fund panel retrieved shows at 904.24원 after fourteen
years [S11] [R2].
...
and does not represent the lag; the technical notes record it as an unmodelled timing
difference.
...
falls back to the plain premiums-paid base — 최저사망적립금 보증강화종료나이 =
연금지급개시나이 + 21 (개시 55–59세), +19 (60–69), +17 (70–79), +15 (80) [S7 제2조]. The
composite has no enhanced form and does not carry it.
...
- **Roll-up, simple, day-counted.** 「기준 기본보험료 및 기준 추가보험료에 … 매년
  최저연금기준금액비율 해당액을 해당보험료 납입일 … 을 기준으로 일자 계산하여 더한 금액」,
  the ratio being 7/100 to the twentieth anniversary and 6/100 thereafter, 7/100 throughout
  where the pre-annuitisation period is under twenty years [S2]; or 5/100 during the premium
  term and 4/100 after it, the 5% band always using the **original** premium term even where
  a payment holiday extended it [S7]. [S2] publishes the conversion a model should target:
  「대표계약기준(40세 남성, 10년납, 연금개시나이 65세) 복리이자율로 환산시 **연복리
  4.32%**」. A compound variant credits 연복리 1.0% or 2.0% on a base **net of the risk and
  expense loadings but gross of the guarantee charge** [S10].
...
1. **There is no time value.** The GMAB payoff `max(0, K − AV(T))` evaluated at one `AV(T)`
   is the payoff of the option, not its value. `E[max(0, K − AV(T))] ≥ max(0, K − E[AV(T)])`
   by Jensen's inequality, with equality only in the degenerate case, so the single-path
...
- **The CPPI alternative.** Where there is no guarantee charge, there is a
  ...
  with the multiplier in the range **1.0–4.0**, notified to the policyholder on conversion
  and changeable by the insurer on written notice [S6]. It has an **absorbing barrier**:
  「성장자산펀드 편입비율이 0%이며, 특별계정계약자적립액이 [기준경과확정보증액 × 평가비율 ×
  1.02]보다 같거나 적은 경우에는 자동으로 특별계정에서 일반계정으로 전환되어
  공시이율(최저보증이율 연복리 1.75%)로 운용되며, 이럴 경우에는 특별계정으로 다시 전환되지
  않습니다」 [S6]. Once the cushion is exhausted the contract becomes a general-account
  declared-rate contract, irreversibly.
...
**A single-premium contract carries no surrender charge at all**, 「일시납 적립식 변액연금의
경우 계약체결 직후 계약체결비용이 모두 부과되므로 보험기간 중 계약이 해지되어도
해지공제비용이 부과되지 않는다」 [R1] [S5].
...
before [R2]. The governing codes are the 「변액보험 표준계약 권유준칙」 and the 「변액보험
모범판매규준」, under which the seller must explain the possibility of loss and the material
loss on early surrender and hand over a 변액보험 주요내용 확인서; a post-sale completeness
call (해피콜) is separately required by the 보험업감독규정 [R2].
...
is missed; the documented case is the 납입 일시중지, during which the 위험보험료, both
guarantee charges, the 부가보험료 excluding 기타비용 and any rider premium are taken from the
surrender value and the holiday ends if they cannot be met [S1]. Second, **the 해약공제 runs
off to zero at seven years while the account is still small**, so the economic penalty for
surrendering falls away long before the guarantee matures — which is exactly the shape that
makes lapse the dominant assumption on this product. [S2]'s illustration shows the 해약환급금
and the 계약자적립액 coinciding from year 7 onwards, and the year-1 difference of ₩702,000
reconciling to that document's published year-1 해약공제 of 71만원.
...
+ 100) ÷ (365 × 365 × 12), times 1.4 for the 10년 집중지급형 — and it is the only algebraic
annuity factor found in any Korean variable document, but **the unit of its output is not
stated in the four pages retrieved** and the two readings differ by a factor of a hundred:
its **form is verified, its level is [unverified]** [S8].
...
공제됨」 [R2] — and the textbook warns that 「이 상품은 공시이율적용 연금형태에 비해 연금액의
증감이 크게 나타난다」. On [S2]'s own illustration, at a −1.0% gross return the account is
