...
index ``t`` is 0-based: ``t = 0`` is the first period of a policy projected from issue (the
issue year on an annual grid, the issue month on a monthly one), period ``t`` runs from time
``t`` to time ``t + 1``, and the attained age is ``age_at_entry + t`` on an annual grid
