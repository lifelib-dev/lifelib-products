...
point — `EC_FR_A`'s in-force model points open partway through the term, at the duration
the policy has already run, which is why the frame's start is asserted as contiguity from
