...
| `P(y)` | `premium_mth_pp` — with `premium_pp` for `12 P(y)` | Library-wide `premium_pp` is the **annual** premium per policy, which is how `PER_FR_S` reads it. This contract is projected monthly, so every recursion here uses `premium_mth_pp`; `premium_pp` is published alongside it so the two periodicities cannot be confused |
