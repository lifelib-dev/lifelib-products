...
- **Time index.** `t` is **0-based**: period `t` is the policy year running from time `t`
  to time `t + 1`, so `t = 0` is the first policy year and the contractual **policy year
  is `t + 1`**. The frame is `t = 0 … proj_len − 1`, where `proj_len = n` is the *number*
  of policy years projected and the last row is the year that ends at the *échéance*. An
  in-force cell opens at `t = duration_ifo`, the completed policy years at the valuation
  date, which is an elapsed count and already 0-based. **The issue instant is not a row:**
  the initial *versement* creates the rights and strikes both provisions as the *opening
  state* of the first projected period, written `A_open(t)`, `pd_open(t)`, `N_open(t)`,
  `mg_open(t)` below and reached in the model as `own_assets_at(t, "BOY")` and its
  siblings; its cash flows are beginning-of-period flows of that first period. A few
  quantities are indexed by a **time point** `k` instead — `x + k`, `TEC(n − k)`,
  `i_pm(k)`, the discount factor — with `k = 0` at issue and `k = n` at the *échéance*, so
  period `t` opens at `k = t` and is struck at `k = t + 1`.
...
- **Age basis.** *Âge atteint* (age last birthday) **[std]** — art. A. 335-1 applies the
  ...
  [REG-R23]. `x + k` is the age at time `k`, so period `t` opens at `x + t` and its
  decrement, applied at EOY, is read at `x + t + 1`.
...
Every state variable below is the **closing** value of period `t`; its opening value is
written `X_open(t)`, which is `X(t−1)` in every period but the first and, in the first,
the state the initial *versement* or the in-force extract creates.
...
| `pols_if` | In-force probability at the **start** of period `t`; `pols_if(0) = 1`. The end-of-period count `l(t)` is `pols_if_at(t, "AFT_DECR")` | EOY decrements |
...
| PM definition (Chassis A) | `pm(t) = mg(t) × (1 + i_pm(t+1))^-(n-t-1)` — struck at the period's year end, time `t + 1` | [R2 R. 134-2](#frlib-eurocroissance-r2) |
| PM discount rate `i_pm` | ≤ **90 % of the last TEC*n***, linear interpolation between bracketing maturities, longest TEC beyond the curve, **floor 0 %**, method choice irreversible per account. As retrieved, A. 134-1 fixes *n* as the holder's **guarantee maturity** (per-engagement method 1°) or the account's 1°-engagement **duration** (method 2°); this model applies method 1° and re-reads *n* as the **remaining** term `n − k` at each valuation time `k` | haircut, interpolation and floor [R3 A. 134-1](#frlib-eurocroissance-r3); the `n − k` re-reading **[std]**, note below |
| Reference TEC10 | 2.50 % at valuation times `k` = 0–5, 1.00 % from `k` = 6 → `i_pm` 2.25 %, then 0.90 % | **[std]**, product-spec (5) |
...
| Maturity amount | at the *échéance*, the close of period `n − 1`: Chassis A `pm + parts × part_value`; Chassis B `max(parts × part_value, mg)` | [R2 R. 134-6](#frlib-eurocroissance-r2) |
...
| Parts levy (base 4°) | 0.80 % p.a. of parts, at BOY on the opening part value `u_open(t)` | [R13]; routing **[std]**, product-spec (8) |
...
| Symbol | Meaning |
|---|---|
| `t`, `k`, `x`, `n`, `g` | 0-based **period** index (`0…n−1`); **time point** in years since issue (`0…n`); `issue_age`; guarantee maturity in years, and the number of periods; guarantee level |
| `P(t)`, `P_net(t)` | gross and net scheduled *versement* at the **start of period** `t`; `P_net = P × (1 − f_e)` |
| `P_0`, `P_net,0` | the initial *versement* and its net amount, paid at inception: the **opening state** of the first projected period, not a step in its roll forward |
| `X_open(t)` | the opening value of a state variable: `X(t−1)` in every period but the first, and in the first the state `P_net,0` — or the in-force extract — creates |
| `f_e`, `f_p`, `f_perf`, `f_x` | entry 2.00 %, parts levy 0.80 % p.a., performance levy 10 %, exit 0.00 % |
| `mg(t)` | guaranteed amount payable at `n`, at the close of period `t` |
| `i_pm(k)` | PM discount rate **at time `k`** = 90 % × TEC(`n−k`), floored at 0 %. The haircut, the interpolation and the floor are [R3 A. 134-1](#frlib-eurocroissance-r3); reading the index maturity as the **remaining** term is **[std]**, see (a) above. Period `t` is struck at `i_pm(t+1)` and splits a start-of-period *versement* at `i_pm(t)` |
| `A(t)` | `own_assets(t)`, account assets attributable to the policy at the close of period `t`, excluding any outstanding insurer contribution |
...
| `r(t)` | gross asset return in period `t`, net of asset management fees |
| `L(t)`, `F(t)` | parts levy (BOY) and performance levy (EOY) of period `t` |
| `C(t)`, `G(t)`, `D(t)` | `insurer_contribution(t)` (L. 134-3); `pgt(t)` (Chassis B); `pcdd(t)` (fund-level) |
| `q(x+t+1)`, `w(t)`, `l(t)` | `mort_rate(t)`, read at the age attained at the **end** of period `t`; `lapse_rate(t)`; and the **end**-of-period in-force count — `pols_if_at(t, "AFT_DECR")` in the model, nil in the last projected period. The model's `pols_if(t)` is the **start**-of-period count — `pols_if(0) = 1` and `pols_if(t) = l(t−1)` thereafter — the weight on period `t`'s flows |
...
Period `t` closes at time `t + 1`, which is `n − t − 1` years short of the *échéance*, so
that is the horizon the striking discounts over.

A free *versement* paid at the end of period `t`, immediately after the striking, splits
as follows and buys parts at the part value just struck:

```
pm_added = g × P_net × (1 + i_pm(t+1))^-(n-t-1)   pd_added = P_net − pm_added
parts_added = pd_added / u(t)
```

A scheduled *versement* paid at the **start** of period `t` splits the same way one year
earlier, at `i_pm(t)` over `n − t`, and buys parts at the opening value `u_open(t)`.

In the last projected period, `t = n − 1`, the discount factor is 1, so
`pm(n−1) = mg(n−1)` identically: the *provision
...
```
L(t)   = f_p × pd_open(t)       (BOY, base 4°)   A_a(t) = A_open(t) − L(t)
...
G(t)  = max( mg(t) × (1 + i_pm(t+1))^-(n-t-1) − pd(t) − D(t), 0 )   (Chassis B) [R3 A. 134-2]
```
...
it is live on every decrement-bearing cell. On shipped model point 6 at the end of policy
year 7 — period `t` = 6 — the reported
...
`PV(t) = Σ_i mg_i(t) × (1 + i_pm(t+1))^-(n_i − t − 1) × (n_i − t − 1)p_(x_i+t+1)` over the
account's 2°
...
maturity_value(n−1)= pm + N × u at t = n−1                 A ;  max( N × u, mg ) at t = n−1   B
...
The nine steps below are the steps of **period `t`**. In the first projected period the
opening quantities `A_open`, `pd_open`, `N_open`, `mg_open` are the state the initial
*versement* creates — this is where the issue instant sits — and in every later period
they are the previous period's close.

1. **BOY** — parts levy `L(t) = f_p × pd_open(t)`; `N(t) = N_open(t) × (1 − f_p)`; assets
   reduced by `L(t)` (base R. 134-3 4°).
...
6. **EOY** — re-strike `pm(t)` from `mg(t)` and the current `i_pm(t+1)`; `pd(t)` = residual,
   floored at `N(t) × u_min`; `u(t) = pd(t) / N(t)`.
7. **EOY** — asset affectations: `C(t)` (Chassis A) or `G(t)` (Chassis B), **after** step 5.
8. **EOY** — free *versements* paid at year end (the worked example's top-up at the end of
   policy year 3, `t` = 2) split at the just-struck `i_pm(t+1)` and `u(t)`.
9. **EOY** — claims: deaths at `q`, surrenders at `w`, maturity in the last projected
   period `t = n − 1`;
   `l(t) = pols_if(t) × (1 − q(x+t+1)) × (1 − w(t))` with `pols_if(0) = 1` and
   `pols_if(t) = l(t−1)` thereafter, survivors maturing at `t = n − 1`, where `l(n−1) = 0`.
...
1. **Treating the Chassis B surrender value as guaranteed.** The single most important
   ...
   *versements*, not 11,760.00. (Policy year 6 is period `t` = 5.)
...
3. **Accumulating the PM instead of re-striking it.** `pm(t)` is `mg(t)` discounted at the
   *current* `i_pm(t+1)` [R2 R. 134-2](#frlib-eurocroissance-r2). Rolling `pm(t−1)` forward at last year's rate silently
   removes the **rate effect** — +587.44 of the +824.18 policy-year-6 move (`t` = 5) in the
   worked example.
4. **Levying an encours charge on the PD in a 1° account.** R. 134-3 3° permits that levy only
   ...
   be the number of parts and the policy-year-1 levy — `L(0)` — must be **15.64**, not 78.40.
5. **Forgetting that the entry charge cuts the guarantee.** The guarantee is a percentage of
   ...
   at the end of policy year 3, `mg(2)`, is **11,760.00**, not 12,000.00.
6. **Omitting the minimum part value.** A debit balance may reduce the part value only
   **within the limit of its minimum** [R2 R. 134-4](#frlib-eurocroissance-r2). Without the floor, Chassis A's `pd` goes
   negative in policy year 6 (`A(5) − pm(5) = −1,095.35`). Test: `part_value ≥ 5.0000` in
   every period on both chassis.
7. **Paying the maturity guarantee to a death claim.** Chapter IV has no death valuation
   ...
   policy-year-6 Chassis B death payout (`t` = 5) with the rider is 11,760.00, of which
   **1,860.78** is a rider
...
8. **Applying the maturity `max(·, mg)` before the maturity, or at all on Chassis A.** The
   `max` exists only in the last projected period, `t = n − 1`, and only on Chassis B
...
11. **Ignoring the A. 134-3 and A. 134-4 gates.** Revaluing the guarantees out of the
    ...
    `t = 4` — the close of policy year 5 — both A. 134-3 tests pass; at `t = 5` the second
    fails (`pd − N × u_min = 0.00` against `10 % × pm = 1,134.60`).
...
### Cash flow outputs (per period `t`, probability-weighted by `pols_if`)

`pols_if(t)` below is the **start**-of-period in-force count — the notes' `l(t−1)`, and
`pols_if_init` in the first projected period — and is the
exposure every flow on that same `result_cf()` row is weighted by. The end-of-period count
`l(t)`
is `pols_if_at(t, "AFT_DECR")` in the model; it weights the maintenance expense and nothing else.

| Output | Formula |
|---|---|
| `premiums` | `P(t) × pols_if(t)`, with the initial *versement* `P_0` in the first projected period |
| `claims_death` | `mort_rate(t) × pols_if(t) × death_payout(t)` |
| `claims_lapse` | `lapse_rate(t) × pols_if(t) × (1 − mort_rate(t)) × surrender_value(t)` |
| `claims_maturity` | `pols_if × (1 − mort_rate) × maturity_value` in the last projected period, `t = n − 1` |
| `withdrawals` | partial *rachats* — an owner election, not a claim |
| `expenses` | `pols_if(t) × (`acquisition 5 % of *versements*, plus an acquisition commission of 2 % of the initial *versement* at issue`) + `maintenance 0.20 % p.a. of `pm + pd` **at each striking**: `l(t) × (pm(t) + pd(t))` at the period's year end, plus, in the first projected period only, `pols_if(t) × (pm_open(t) + pd_open(t))` at the striking the frame opens on — all three levels [R13], as in the assumption table above |
...
The first row carries the initial *versement*, the entry charge on it, the acquisition
commission and the maintenance charged at the opening striking, **as well as** the first
policy year's own levy, return and striking. That is the merge of the issue instant into
the first period: no row is lost from any total, and the frame has `n − duration_ifo` rows
rather than `n − duration_ifo + 1`.
...
- **Guarantee-imminent suppression (Chassis B).** `w(t) = w_base(t) × 0.5` in the two
  policy years before the *échéance* — the periods with `n − t − 1 ≤ 2` years still to run
  at their close — whenever the guarantee is in the money, `N(t) × u(t) < mg(t)`
...
- **Duration-8 tax spike.** The assurance-vie annual abattement (€4 600 / €9 200) becomes
  available at eight years [REG-R40], so the rate of **policy year 8** — period `t` = 7 —
  is `w_base × 1.5` **[std]** where `n > 8`.
- **Lock-up.** Where `lock_up_years > 0`, `w(t) = 0` for `t < lock_up_years` — policy years
  1 to `lock_up_years` — except for the
...
of **€2 000.00** at the end of policy year 3 — period `t` = 2 — [R13]; entry charge **2.00 %** [R13], so net
...
provision path is the cash-flow path **[std]**. Gross asset return `r(t)` = 4.00 % in
periods `t` = 0–4 (policy years 1–5), **−25.00 %** in `t` = 5 (policy year 6) and 6.00 % in
`t` = 6–9; TEC10 = 2.50 % at valuation times `k` = 0–5 and
**1.00 %** from `k` = 6 at every maturity, so the A. 134-1 discount rate `i_pm` = 90 % × TEC = **2.25 %** then
**0.90 %** [R3] **[std]**. The policy-year-6 double shock — equities down and rates down together —
is what makes the rebalancing visible.

Each table below has **ten** rows, one per projected policy year, plus an *opening* line
that is **not** a row of the frame: it is the state the initial *versement* creates, which
the model reaches as `own_assets_at(0, "BOY")` and its siblings. Row `t` is the **close**
of policy year `t + 1`.

### Chassis A (1° engagement: euros and parts)

`L(t) = 0.80 % × pd_open(t)`; `A(t) = (A_open(t) − L(t)) × (1 + r(t)) − 10 % × max((A_open(t) − L(t)) × r(t), 0)`;
`pm(t) = mg(t) × (1 + i_pm(t+1))^-(9-t)`; `pd(t) = max(A(t) − pm(t), parts(t) × 5.00)`.
...
| 0 | 4.00 % | 15.64 | 10,136.60 | 8,021.51 | 2,115.09 | 193.9361 | 10.9061 | 0.00 | 10,136.60 |
| 1 | 4.00 % | 16.92 | 10,483.98 | 8,202.00 | 2,281.99 | 192.3846 | 11.8616 | 0.00 | 10,483.98 |
| 2 | 4.00 % | 18.26 | 12,802.49 | 10,063.85 | 2,738.65 | 212.8127 | 12.8688 | 0.00 | 12,802.49 |
| 3 | 4.00 % | 21.91 | 13,240.69 | 10,290.29 | 2,950.40 | 211.1102 | 13.9756 | 0.00 | 13,240.69 |
| 4 | 4.00 % | 23.60 | 13,692.90 | 10,521.82 | 3,171.08 | 209.4213 | 15.1421 | 0.00 | 13,692.90 |
| 5 | −25.00 % | 25.37 | 10,250.65 | 11,346.00 | 1,038.73 | 207.7460 | **5.0000** | 2,134.08 | 12,384.73 |
| 6 | 6.00 % | 8.31 | 10,795.42 | 11,448.11 | 1,030.42 | 206.0840 | 5.0000 | 1,683.11 | 12,478.53 |
| 7 | 6.00 % | 8.24 | 11,369.69 | 11,551.14 | 1,022.18 | 204.4353 | 5.0000 | 1,203.63 | 12,573.32 |
| 8 | 6.00 % | 8.18 | 11,975.03 | 11,655.10 | 1,014.00 | 202.7998 | 5.0000 | 694.07 | 12,669.10 |
| 9 | 6.00 % | 8.11 | 12,613.13 | **11,760.00** | 1,005.89 | 201.1774 | 5.0000 | 152.75 | **12,765.89** |

The `t` = 2 row is stated **after** the year-end *versement*. Immediately before it,
...
| 0 | 78.40 | 10,071.58 | 972.1600 | 10.3600 | 0.00 |
| 1 | 80.57 | 10,350.68 | 964.3827 | 10.7330 | 0.00 |
| 2 | 82.81 | 12,597.52 | 1,132.9370 | 11.1193 | 0.00 |
| 3 | 100.78 | 12,946.62 | 1,123.8735 | 11.5196 | 0.00 |
| 4 | 103.57 | 13,305.40 | 1,114.8825 | 11.9344 | 0.00 |
| 5 | 106.44 | **9,899.22** | 1,105.9635 | **8.9508** | **1,446.78** |
| 6 | 79.19 | 10,350.30 | 1,097.1158 | 9.4341 | 1,097.81 |
| 7 | 82.80 | 10,821.95 | 1,088.3388 | 9.9435 | 729.20 |
| 8 | 86.58 | 11,315.08 | 1,079.6321 | 10.4805 | 340.02 |
| 9 | 90.52 | **11,830.69** | 1,070.9951 | 11.0464 | 0.00 |

Again the `t` = 2 row is post-*versement*: immediately before it `pd = 10,637.52`,
...
51.37, 0.00, 58.92, 61.61, 64.41, 67.35 for `t` = 0–9.

### Exit values, and what the two chassis pay

| Event | Chassis A | Chassis B |
|---|---|---|
| Surrender at `t` = 5 (end of policy year 6) | 12,384.73 (**105.31 %** of net *versements*) | 9,899.22 (**84.18 %**) |
| Death at `t` = 5, no rider | 12,384.73 | 9,899.22 |
| Death at `t` = 5 with the *garantie décès plancher* | 12,384.73 (rider claim 0.00) | 11,760.00 (rider claim **1,860.78**, outside the account) |
| Maturity at `t` = 9 (the *échéance*) | 12,765.89 = 11,760.00 + 1,005.89 | 11,830.69 = `max(11,830.69, 11,760.00)` |
| Guarantee binding at maturity? | yes, by construction (`pm(9) = mg`) | no — the account recovered to 0.60 % above `mg` |
| Insurer's own-funds cost, peak | contribution **2,134.08** at `t` = 5 | PGT **1,446.78** at `t` = 5 |
...
**Checks.** *(i)* The PM unwinds exactly onto the guarantee: `pm(9) = 11,760.00 × 1.009^0 =
11,760.00 = mg(9)`, and the Chassis A maturity payout is that plus the parts at their floor,
...
unchanged
2.25 % rate `pm(5)` would have been `11,760.00 × 1.0225^-4 = 10,758.56`, a **time effect of
+236.74** on `pm(4) = 10,521.82`; re-striking at 0.90 % gives `11,760.00 × 1.009^-4 =
11,346.00`, a **rate effect of +587.44**; the two sum to +824.18, which is exactly
`11,346.00 − 10,521.82`. *(iii)* The policy-year-6 asset roll is `A(5) = (13,692.90 − 25.37) ×
(1 − 0.25) = 10,250.65` with no performance levy, because the financial performance was
negative; `pd(5)` would have been `10,250.65 − 11,346.00 = −1,095.35` without the floor, so
...
Chassis B. *(v)* The A. 134-3 gates behave as expected: at `t` = 4, `pd = 3,171.08 >
1.5 × (11,760.00 − 10,521.82) = 1,857.27` and `pd − N × u_min = 2,123.98 > 10 % × pm =
1,052.18`, so revaluing the guarantees is permitted; at `t` = 5 the second test fails,
`0.00 ≤ 1,134.60`. *(vi)* The A. 134-4 conversion headroom at `t` = 4 is the `C` solving
...
[S8] — the same path leaves `pm(5) = 9,076.80` and cuts the policy-year-6 Chassis A
contribution
...
*apport d'actifs* of the statutory maximum **10 % of the PD** at `t` = 5 — 989.92, credited to
...
2. **The level of the TEC.** `i_pm` is 90 % of it [R3 A. 134-1](#frlib-eurocroissance-r3), and a 150 bp fall adds
   587.44 to `pm(5)` — the close of policy year 6 — in the worked example, more than twice
   the time effect. A model with a
