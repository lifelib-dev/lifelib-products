...
`t` is **0-based**, the library convention. `t = 0` is the first projected year for every
model point — new business and in force alike, since this model's frame always opens at
the valuation date and carries the elapsed time in `duration_init` rather than in the
index. Year `t` runs from time `t` to time `t + 1`; `pols_if(t)` and `av_pp(t)` are the
count and the balance at its **start**, so `av_pp(0) == av_pp_init()` and
`pols_if(0) == pols_if_init()`, and `av_pp(t + 1)` — which the death and surrender
benefits are struck on — is the balance at its end.

`proj_len()` is the **number** of projected years, 40, so the frame is `t = 0 … 39` and
`result_cf()` and `result_pb()` each have 40 rows, indexed `0 … 39` with `index.name ==
"t"`. Two derived clocks read off it and are *not* the index:
...
`v = −1, −2, … , −ppb_vintages_init`, falling due at `t = 7, 6, … , 0` for the default
eight.

**Two input columns are on this axis and shifted with it**: the `t` key of
...
|---|---|---|
| Which percentage, which account | 85% of the `compte financier`; the `compte technique` less the insurer's share [R5, art. A132-11](#frlib-assurance_vie_euro-r5) | "90% of the financial account and 85% of the technical result" — EUR 3,319.09 against the correct EUR 3,071.86 at worked-example `t = 0` |
| The insurer's technical share | The **greater** of 10% of the credit balance and 4.5% of annual premiums | Dropping the premiums limb — EUR 108.00 against EUR 28.43 at `t = 5` |
| The financial base | `pm_avg_pp + ppb_pp`, because art. A132-14 works on average technical provisions [REG-R15] and the PPB is one of them [REG-R6] | Omitting the PPB (−EUR 41.81 at `t = 5`), or accreting the vintages as well, which pays its return twice |
...
forced release wins the credited rate goes *above* the target: `t = 5` wants EUR 426.99
...
exactly that at `t = 0`. `check_pb_allocation()` therefore states an allocation identity,
...
| `t` | Forced | Want, uncapped | Discretionary | Released | Drawn from |
|---|---|---|---|---|---|
...
`t = 0` to `t = 2` the want is *negative*, which is what a dotation year is (the three
dotations named below are its mirror image), and from `t = 7` the *cap* binds, partially
and then to nothing. Neither is the release: `t = 6` wants 606.30 and gets it, `t = 5`
...
nothing is forced out before `t = 4`.
...
- `check_guar_floor()` — the weaker and correct statement about the balance,
  ...
  EUR 99,061.85 at `t = 12` against EUR 139,600.82 — it never binds on a path with a
...
worked example's EUR 2,827.60 at `t = 0` is EUR 486.35.
...
every rate to the fourth decimal of a percentage. The `t = 5` trace, where every lever is
active at once:
...
`2.413706% + 0.142100% − 0.600000% + 0.403047% = 2.358853%` at `t = 5`, and
`2.082500% + 0.145673% − 0.600000% + 0.000000% = 1.628173%` at `t = 8` with the PPB
...
Read `t = 8` for what it says. At `r_fin = 2.45%` and a 0.60% charge the most the account
...
the `t = 5` trace at full precision, the `taux servi` decomposition from the other
...
of the eleven. The frame is pinned there too: `result_cf()` is indexed `0 … 39` and has
`proj_len() = 40` rows on every model point.
