...
``t`` is the 0-based period index: period ``t`` is contract year ``t + 1`` and runs from
anniversary ``t`` to anniversary ``t + 1``, where its transactions fall.  The notes'
anniversary-8 table is therefore period ``t = 7``, contract year 8, and the opening state
the notes state at anniversary 7 is that period's opening timing.

Model point 1 enters the projection **in force at anniversary 7** - seven completed
contract years, so its first projected period is t = 7 - on the balances the
...
# closes period t = 7 (contract year 8):
#
...
WORKED_EXAMPLE = [
    (1, "index return R(8)", lambda p: p.index_return(7), 0.090000, RATE),
    (2, "credit rate cr(8)", lambda p: p.credit_rate(7), 0.052500, RATE),
    (3, "index credit IC(8)", lambda p: p.index_credit_pp(7), 6720.00, CENT),
    (4, "AV after credit", lambda p: p.av_pp_at(7, "BEF_FEE"), 134720.00, CENT),
    (5, "rider charge Phi(8)", lambda p: p.rider_charge_pp(7), 1710.00, CENT),
    (6, "AV(2) after charge", lambda p: p.av_pp_at(7, "BEF_WD"), 133010.00, CENT),
    (7, "guaranteed rollup", lambda p: p.rollup_pp(7), 5000.00, CENT),
    (8, "stacking credit", lambda p: p.stack_pp(7), 10080.00, CENT),
    (9, "BB(3) before step-up",
     lambda p: p.benefit_base_pp_at(7, "BEF_STEP_UP"), 195080.00, CENT),
    (10, "BB(4) after step-up",
     lambda p: p.benefit_base_pp_at(7, "BEF_WD"), 195080.00, CENT),
    (11, "lifetime withdrawal LW(8)",
     lambda p: p.lw_pp_at(7, "BEF_WD"), 10144.16, CENT),
    (12, "free withdrawal amount FW(8)", lambda p: p.free_wd_allow(7), 12800.00, CENT),
    (13, "excess E(8)", lambda p: p.wd_excess_pp(7), 0.00, CENT),
    (14, "account value AV(8)", lambda p: p.av_pp(7), 122865.84, CENT),
    (15, "guaranteed minimum MGV(8)", lambda p: p.mgsv_pp(7), 84605.80, CENT),
    (16, "closing benefit base BB(8)", lambda p: p.benefit_base_pp(7), 195080.00, CENT),
]
...
def test_worked_example_opening_state(anchor):
    ...
    Anniversary 7 opens period 7, so the notes' opening state is that period's opening
    timing rather than a row of its own: there is no issue-instant row, and no
    pre-period row on an in-force cell either.
    """
    assert anchor.entry_year() == 7
    assert anchor.av_pp_at(7, "BEF_INV") == pytest.approx(128000.00, abs=CENT)
    assert anchor.benefit_base_pp_at(7, "BEF_ROLLUP") == pytest.approx(180000.00,
                                                                      abs=CENT)
    ...
    # Nothing was withdrawn before entry, so the first period's own withdrawal is the
    # whole of Wcum at the end of it.
    assert anchor.wd_cum_pp(7) == pytest.approx(10144.16, abs=CENT)
    assert anchor.phase_init() == "ACCUM"
    assert anchor.phase_open(7) == "INCOME"       # promoted by the first exercise
...
def test_the_step_up_does_not_bind_on_the_blended_baseline(anchor):
    ...
    assert anchor.step_up_applies(7) is True
    assert (anchor.benefit_base_pp_at(7, "BEF_WD")
            == anchor.benefit_base_pp_at(7, "BEF_STEP_UP"))
    assert anchor.benefit_base_pp_at(7, "BEF_WD") > anchor.av_pp_at(7, "BEF_WD")
...
def test_a_guaranteed_withdrawal_leaves_the_guarantee_untouched(anchor):
    ...
    assert anchor.wd_pp(7) == pytest.approx(anchor.lw_pp_at(7, "BEF_WD"), abs=EXACT)
    assert anchor.wd_excess_pp(7) == 0.0
    assert anchor.wd_reduction_rate(7) == 0.0
    assert anchor.wd_charge_base_pp(7) == 0.0
    assert anchor.wd_charge_pp(7) == 0.0
    assert anchor.wd_clawback_pp(7) == 0.0
    assert anchor.wd_mva_pp(7) == 0.0
    assert anchor.benefit_base_pp(7) == pytest.approx(195080.00, abs=CENT)
    assert anchor.lw_pp(7) == pytest.approx(10144.16, abs=CENT)
...
def test_surrender_trace_free_amount_and_charge_base(anchor):
    ...
    assert anchor.free_wd_allow(7) == pytest.approx(12800.00, abs=CENT)
    assert anchor.wd_free_pp(7) == pytest.approx(10144.16, abs=CENT)
    assert anchor.free_wd_remain(7) == pytest.approx(2655.84, abs=CENT)
    assert anchor.surr_charge_base_pp(7) == pytest.approx(120210.00, abs=CENT)
...
def test_surrender_trace_charge_clawback_and_mva(anchor):
    ...
    ``mva_ref_yield`` is a time-point reading, so the 3.50% in force at the transaction
    is read at anniversary 8 - the anniversary that closes period 7 - and its argument
    does not shift with the frame.
    """
    assert anchor.policy_year(7) == 8
    assert anchor.surr_charge_rate(7) == pytest.approx(0.03, abs=RATE)
    assert anchor.vest_rate(7) == pytest.approx(0.70, abs=RATE)
    assert anchor.surr_charge_pp(7) == pytest.approx(3606.30, abs=CENT)
    assert anchor.surr_clawback_pp(7) == pytest.approx(2359.26, abs=CENT)
    assert anchor.mva_term(7) == 2.0                       # n = 24 months
    ...
    assert anchor.mva_rate(7) == pytest.approx(-0.00963850, abs=FACTOR)
    assert anchor.mva_pp(7) == pytest.approx(-1158.64, abs=CENT)


def test_surrender_trace_mva_collar_does_not_bind(anchor):
    """The limit max(0, G - SC - CB - MGV) = 32,294.48 [S10], well outside -1,158.64."""
    limit = max(0.0, anchor.av_pp(7) - anchor.surr_charge_pp(7)
                - anchor.surr_clawback_pp(7) - anchor.mgsv_pp(7))
    assert limit == pytest.approx(32294.48, abs=CENT)
    assert abs(anchor.mva_pp(7)) < limit
...
def test_surrender_trace_net_proceeds_is_a_cent_rounding_artefact(anchor):
    ...
    being tuned away.
    """
    gross = anchor.av_pp(7)
    sc, cb, mva = (anchor.surr_charge_pp(7), anchor.surr_clawback_pp(7),
                   anchor.mva_pp(7))
    ...
    assert anchor.surr_value_pp(7) == pytest.approx(115741.634475, abs=CENT)
    assert from_displayed - anchor.surr_value_pp(7) == pytest.approx(0.0055, abs=1e-4)
    # The nonforfeiture floor does not bind: CSV = the surrender value, not MGV.
    assert anchor.surr_benefit_pp(7) == pytest.approx(anchor.surr_value_pp(7), abs=EXACT)
    assert anchor.surr_benefit_pp(7) > anchor.mgsv_pp(7)
...
def test_step_up_binds_under_the_pure_rollup_variant(fixed_indexed_annuity):
    ...
    assert p.benefit_base_pp_at(7, "BEF_ROLLUP") == pytest.approx(121000.00, abs=CENT)
    assert p.stack_factor() == 0.0
    assert p.rollup_rate(7) == pytest.approx(0.03, abs=RATE)
    assert p.rider_charge_pp(7) == pytest.approx(1149.50, abs=CENT)
    assert p.av_pp_at(7, "BEF_WD") == pytest.approx(133570.50, abs=CENT)
    assert p.rollup_pp(7) == pytest.approx(3000.00, abs=CENT)
    assert p.stack_pp(7) == 0.0
    assert p.benefit_base_pp_at(7, "BEF_STEP_UP") == pytest.approx(124000.00, abs=CENT)
    assert p.benefit_base_pp_at(7, "BEF_WD") == pytest.approx(133570.50, abs=CENT)
    assert p.lw_pp_at(7, "BEF_WD") == pytest.approx(6945.67, abs=CENT)
...
def test_step_up_binds_in_year_one_on_a_new_issue(fixed_indexed_annuity):
    ...
    AV(2)(0) = 106,050 against BB(3)(0) = 105,000 and the step-up binds.  Test it at
    every anniversary rather than assuming the stack dominates."
    """
    p = fixed_indexed_annuity.Projection[2]
    assert p.entry_year() == 0
    # The Initialisation block is the opening state of period 0, not a row of its own.
    assert p.av_pp_at(0, "BEF_INV") == pytest.approx(107000.00, abs=CENT)  # P(1+b) [S5]
    assert p.benefit_base_pp_at(0, "BEF_ROLLUP") == pytest.approx(
    ...
    assert p.index_credit_pp(0) == 0.0
    assert p.rider_charge_pp(0) == pytest.approx(950.00, abs=CENT)
    assert p.av_pp_at(0, "BEF_WD") == pytest.approx(106050.00, abs=CENT)
    assert p.benefit_base_pp_at(0, "BEF_STEP_UP") == pytest.approx(105000.00, abs=CENT)
    assert p.benefit_base_pp(0) == pytest.approx(106050.00, abs=CENT)   # the step-up
    # ... and the same cell rolled forward seven years reaches the notes' MGV(7), the
    # value closing period 6 and opening period 7.
    assert p.mgsv_pp(6) == pytest.approx(93811.84, abs=CENT)
...
def test_the_account_value_drains_by_11997_42_a_year(anchor):
    ...
    drain = anchor.lw_pp(7) + 0.0095 * anchor.benefit_base_pp(7)
...
def test_the_account_value_is_exhausted_during_contract_year_19(anchor):
    ...
    Contract year 19 is period 18, which closes at anniversary 19.
    """
    assert anchor.policy_year(18) == 19
    assert anchor.av_pp(17) > 0.0
    assert anchor.av_pp(17) == pytest.approx(2891.64, abs=CENT)
    assert anchor.av_pp(18) == 0.0
    assert anchor.age(18) + 1 == 81               # at the closing anniversary
    assert anchor.phase(17) == "INCOME"
    assert anchor.phase(18) == "DEPLETED"
    assert anchor.depletion_cause(18) is False
    # The insurer funds the shortfall in the depletion year itself.
    assert anchor.av_depletion_pp(18) == pytest.approx(
        10144.16 - anchor.av_pp_at(18, "BEF_WD"), abs=CENT)
...
def test_the_income_stream_survives_exhaustion(anchor):
    ...
    for t in range(19, 30):
        ...
        assert anchor.income_payments(t) == pytest.approx(
            10144.16 * anchor.pols_if(t), abs=CENT)      # pols_if(t) opens period t
...
def test_income_runs_to_the_end_of_the_projection(anchor):
    ...
    assert anchor.pols_if(anchor.proj_len() - 2) > 0.0
    assert sum(anchor.income_payments(t)
               for t in range(19, anchor.proj_len())) > 0.0
...
def test_pitfall_the_floor_is_on_the_credit_not_the_account_value(fixed_indexed_annuity):
    ...
    rider charge, so the account value must *fall* over its first period.
    """
    p = fixed_indexed_annuity.Projection[2]
    assert p.credit_rate(0) == 0.0                       # the credit is floored at zero
    assert p.credit_rate_on(-0.30, "cap") == 0.0         # ... however bad the index year
    assert p.av_pp(0) < p.av_pp_at(0, "BEF_INV")         # ... the account value is not
...
def test_pitfall_the_simple_rollup_is_a_flat_dollar_increment(anchor):
    ...
    """
    p = anchor
    assert p.rollup_pp(7) == pytest.approx(0.05 * p.rollup_base_pp_init(), abs=CENT)
    assert p.rollup_pp(7) == pytest.approx(5000.00, abs=CENT)
    assert p.rollup_base_pp_init() == pytest.approx(100000.00, abs=CENT)
    # Simple interest on the *grown* base would have been 0.05 x 180,000 = 9,000.
    assert p.rollup_pp(7) != pytest.approx(
        0.05 * p.benefit_base_pp_at(7, "BEF_ROLLUP"), abs=1.0)
...
def test_pitfall_attribution_at_depletion(fixed_indexed_annuity, anchor):
    ...
    assert p7.wd_excess_pp(7) > 0.0
    assert p7.depletion_cause(7) is True
    assert anchor.depletion_cause(7) is False

    terminated = [t for t in range(7, p7.proj_len()) if p7.phase(t) == "TERMINATED"]
    depleted = [t for t in range(7, anchor.proj_len())
    ...
    # pols_if opens the period, so the deemed full surrender shows up as a zero
    # opening count in the period *after* the one that terminates.
    ...
    assert all(p7.income_payments(t) == 0.0 for t in range(7, p7.proj_len()))
    assert sum(anchor.income_payments(t)
               for t in range(7, anchor.proj_len())) > 0.0
...
def test_the_terminating_branch_pays_only_what_the_account_value_holds(
    ...
    Points 1 and 7 exhaust in the same period and the notes send them opposite ways.
    ...
    """
    p7 = fixed_indexed_annuity.Projection[7]
    assert p7.phase(17) == "INCOME"
    assert p7.phase(18) == "TERMINATED"

    # What the contract could meet, against what was asked for.
    assert p7.av_pp_at(18, "BEF_WD") == pytest.approx(765.92, abs=CENT)
    assert p7.wd_pp(18) == pytest.approx(7856.61, abs=CENT)
    assert p7.lw_pp_at(18, "BEF_WD") == pytest.approx(7482.49, abs=CENT)
    assert p7.wd_excess_pp(18) == pytest.approx(374.12, abs=CENT)
    assert p7.av_depletion_pp(18) == pytest.approx(7090.69, abs=CENT)

    # None of that shortfall is payable, so the cash is the balance and nothing more.
    assert p7.wd_unfunded_pp(18) == pytest.approx(7090.69, abs=CENT)
    assert p7.wd_payment_pp(18) == pytest.approx(765.92, abs=CENT)
    assert p7.withdrawals(18) == pytest.approx(
        765.92 * p7.pols_if(18), abs=CENT)
    # The excess goes unpaid first and the guaranteed portion only after it [S9], [std].
    assert p7.wd_excess_paid_pp(18) == 0.0
    assert p7.wd_guar_paid_pp(18) == pytest.approx(765.92, abs=CENT)
    assert p7.wd_excess(18) == 0.0
    assert p7.wd_guar(18) == pytest.approx(p7.withdrawals(18), abs=EXACT)
    # ... and nothing at all is paid once the contract has terminated.
    assert all(p7.withdrawals(t) == 0.0 for t in range(19, p7.proj_len()))

    # The cap is on the payment, not on the withdrawal: the guarantee is still destroyed.
    assert p7.depletion_cause(18) is True
    assert p7.wd_reduction_rate(18) == 1.0
    assert p7.benefit_base_pp(18) == 0.0

    # The contrast, in the same period: on the DEPLETED branch it IS all paid.
    assert anchor.phase(18) == "DEPLETED"
    assert anchor.av_pp_at(18, "BEF_WD") == pytest.approx(1038.38, abs=CENT)
    assert anchor.av_depletion_pp(18) == pytest.approx(9105.78, abs=CENT)
    assert anchor.wd_unfunded_pp(18) == 0.0
    assert anchor.wd_payment_pp(18) == pytest.approx(10144.16, abs=CENT)
...
def test_pitfall_an_overdraw_permanently_reduces_the_guarantee(fixed_indexed_annuity,
    ...
    p7 = fixed_indexed_annuity.Projection[7]
    assert p7.lw_pp_at(7, "BEF_WD") == pytest.approx(10144.16, abs=CENT)
    assert p7.wd_pp(7) == pytest.approx(1.05 * 10144.16, abs=CENT)
    assert p7.wd_excess_pp(7) == pytest.approx(0.05 * 10144.16, abs=CENT)
    rho = p7.wd_reduction_rate(7)
    assert rho > 0.0
    assert p7.benefit_base_pp(7) == pytest.approx(195080.00 * (1 - rho), abs=CENT)
    assert p7.benefit_base_pp(7) < anchor.benefit_base_pp(7)
    assert p7.lw_pp(7) < anchor.lw_pp(7)
    assert p7.rollup_base_pp(7) == pytest.approx(100000.00 * (1 - rho), abs=CENT)


def test_pitfall_no_lapse_in_depleted(anchor):
    """"Leaving the surrender decrement on silently truncates the ... liability."""
    depleted = [t for t in range(7, anchor.proj_len() - 1)
                if anchor.phase(t) == "DEPLETED"]
    assert depleted
    for t in depleted:
        assert anchor.lapse_rate(t) == 0.0
        assert anchor.pols_lapse(t) == 0.0
        # only mortality exits: the count closing period t opens it, less deaths
...
def test_pitfall_rider_charge_base_and_ordering(anchor):
    ...
    """
    assert anchor.rider_charge_pp(7) == pytest.approx(0.0095 * 180000.00, abs=CENT)
    # ... on the OPENING base, because the base is updated after the charge.
    assert anchor.rider_charge_pp(7) == pytest.approx(
        0.0095 * anchor.benefit_base_pp_at(7, "BEF_ROLLUP"), abs=CENT)
    # ... and after the index credit: the charge lands on AV(1), not on the opening AV.
    assert anchor.av_pp_at(7, "BEF_WD") == pytest.approx(
        anchor.av_pp_at(7, "BEF_FEE") - anchor.rider_charge_pp(7), abs=EXACT)
    assert anchor.av_pp_at(7, "BEF_FEE") > anchor.av_pp_at(7, "BEF_INV")
    ratio = anchor.benefit_base_pp(7) / anchor.av_pp(7)
    assert ratio == pytest.approx(1.59, abs=0.005)
    # Charging on the account value would understate the deduction.
    assert 0.0095 * anchor.av_pp_at(7, "BEF_FEE") < anchor.rider_charge_pp(7)
...
def test_pitfall_mva_sign_collar_and_scope(anchor, fixed_indexed_annuity):
    ...
    assert anchor.mva_rate(9) == 0.0
    assert anchor.mva_pp(10) == 0.0
    # Never on the death benefit: it is max(AV, MGV), with no charge and no adjustment.
    assert anchor.claim_pp(8, "DEATH") == pytest.approx(
        max(anchor.av_pp(8), anchor.mgsv_pp(8)), abs=EXACT)
    assert anchor.claim_pp(8, "DEATH") > anchor.claim_pp(8, "LAPSE")
...
def test_pitfall_the_mva_collar_on_a_partial_withdrawal_is_shipped_both_ways(
    ...
    """
    base, small = anchor.surr_charge_base_pp(7), 20000.0
    # Same input, two readings, on the withdrawal path.
    assert anchor.mva_pp_on(7, base, small) == 0.0                    # "gross", default
    model = mx.read_model(MODEL_PATH, name="FIA_collar")
    try:
        model.Projection.mva_collar_basis = "surrender_value"
        alt = model.Projection[1]
        assert alt.mva_pp_on(7, base, small) == pytest.approx(-1158.64, abs=CENT)
        # ... but the worked example's surrender trace is invariant, because G = AV there.
        assert alt.mva_pp(7) == pytest.approx(anchor.mva_pp(7), abs=1e-9)
        assert alt.surr_value_pp(7) == pytest.approx(anchor.surr_value_pp(7), abs=1e-9)
    finally:
        model.close()
...
def test_pitfall_index_costs_are_deducted_before_the_cap_and_participation_rate(anchor):
    ...
    try:
        ...
        assert p.credit_rate(7) == pytest.approx(0.0525, abs=RATE)
        assert p.credit_rate_on(anchor.index_return(7) / 3.0, "cap") == pytest.approx(
            0.03 - 0.005, abs=RATE)
    finally:
        model.close()
...
def test_the_declared_and_guaranteed_scales_are_kept_apart(anchor):
    ...
    try:
        ...
        assert g.credit_rate(7) == pytest.approx(0.0025, abs=RATE)
        assert g.av_pp(7) < anchor.av_pp(7)
    finally:
        model.close()
...
def test_the_surrender_charge_and_vesting_schedules(anchor):
    ...
    # The schedules are stated by contract year, which is period t + 1.
    for t, (sc, v) in enumerate(zip(expected_sc, expected_v)):
        assert anchor.policy_year(t) == t + 1
        assert anchor.surr_charge_rate(t) == pytest.approx(sc, abs=RATE)
        assert anchor.vest_rate(t) == pytest.approx(v, abs=RATE)
    assert anchor.surr_charge_rate(24) == 0.0        # the last row holds
    assert anchor.vest_rate(24) == 1.0
...
def test_the_payout_percentage_is_locked_at_first_exercise(anchor):
    ...
    assert anchor.is_exercise(7) is True
    assert anchor.exercise_age(7) == 70
    assert anchor.payout_rate_locked(7) == pytest.approx(0.052, abs=RATE)
    for t in range(8, 24):
        assert anchor.is_exercise(t) is False
        assert anchor.payout_rate_locked(t) == pytest.approx(0.052, abs=RATE)
    # The 80+ band is never re-read even though the annuitant passes 80 in period 17.
    assert anchor.exercise_age(17) == 80
    assert anchor.payout_rate(80, "single") == pytest.approx(0.057, abs=RATE)
    assert anchor.payout_rate_locked(17) == pytest.approx(0.052, abs=RATE)
...
def test_the_joint_basis_reads_the_younger_life(fixed_indexed_annuity):
    ...
    exercise = [t for t in range(40) if p.is_exercise(t)]
    assert exercise == [9]              # the younger life reaches 70 closing period 9
    assert p.exercise_age(9) == 70
    assert p.age(9) + 1 == 72                    # the annuitant is already 72
    assert p.payout_rate_locked(9) == pytest.approx(0.047, abs=RATE)
...
def test_income_never_decreases_after_exercise(fixed_indexed_annuity):
    ...
    p = fixed_indexed_annuity.Projection[5]
    started = [t for t in range(p.proj_len()) if p.phase(t) == "INCOME"]
    for t in started[1:]:
...
def test_benefit_base_growth_stops_at_the_first_lifetime_withdrawal(anchor):
    ...
    The period of exercise is still inside the window - the worked example credits both
    the rollup and the stack in period 7 - and every later one is outside it.
    """
    assert anchor.in_growth_period(7) is True
    assert anchor.rollup_pp(7) > 0.0
    assert anchor.stack_pp(7) > 0.0
    for t in range(8, 20):
...
def test_the_growth_period_also_ends_at_contract_year_20(fixed_indexed_annuity):
    ...
    assert q.is_exercise(22) is True              # after the window, not before it
    assert q.in_growth_period(19) is True         # contract year 20
    assert q.in_growth_period(20) is False        # contract year 21
    assert q.rollup_pp(20) == 0.0
    assert q.stack_pp(20) == 0.0
    frozen = q.benefit_base_pp(19)
    ...
    assert p.in_growth_period(19) is False
...
def test_the_rollup_schedule_steps_down_in_year_eleven(fixed_indexed_annuity):
    ...
    for t in (0, 4, 9):                          # contract years 1, 5, 10
        assert q.rollup_rate(t) == pytest.approx(0.0500, abs=RATE)
    for t in (10, 14, 19):                       # contract years 11, 15, 20
        assert q.rollup_rate(t) == pytest.approx(0.0200, abs=RATE)
    for t in (20, 29):                           # contract years 21, 30
        assert q.rollup_rate(t) == 0.0
    assert q.rollup_pp(19) == pytest.approx(0.02 * q.rollup_base_pp(18), abs=CENT)
    # The Nassau schedule is a flat 3% over fifteen anniversaries instead [S9].
    p = fixed_indexed_annuity.Projection[3]
    assert p.rollup_id() == "nassau"
    assert p.rollup_rate(0) == pytest.approx(0.03, abs=RATE)
    assert p.rollup_rate(14) == pytest.approx(0.03, abs=RATE)
    assert p.rollup_rate(15) == 0.0
...
def test_the_payout_percentage_locks_in_the_80_plus_band(fixed_indexed_annuity):
    ...
    q = fixed_indexed_annuity.Projection[9]
    assert q.exercise_age(22) == 85
    assert q.payout_rate_locked(22) == pytest.approx(0.0570, abs=RATE)
    assert q.lw_pp_at(22, "BEF_WD") == pytest.approx(
        0.0570 * q.benefit_base_pp_at(22, "BEF_WD"), abs=CENT)
...
def test_the_shock_lapse_is_suppressed_by_the_rider(fixed_indexed_annuity, anchor):
    ...
    """
    shock_t = anchor.surr_charge_period               # contract year 11 is period 10
...
def test_the_base_surrender_shape(fixed_indexed_annuity):
    ...
    for year, rate in expected.items():           # keyed by contract year = t + 1
        t = year - 1
...
def test_the_moneyness_multiplier_suppresses_surrender(anchor):
    ...
    ratio = anchor.benefit_base_pp(10) / anchor.av_pp(10)
    assert ratio > 1.0
    assert anchor.lapse_moneyness_factor(10) == pytest.approx(
        max(0.2, 1.0 - 0.6 * (ratio - 1.0)), abs=1e-12)
    assert 0.2 <= anchor.lapse_moneyness_factor(10) <= 1.0
    for t in range(7, anchor.proj_len()):
...
def test_a_pre_exercise_withdrawal_charges_on_the_gross_amount(fixed_indexed_annuity):
    ...
    assert p.policy_year(2) == 3            # the withdrawal is in contract year 3
    assert p.wd_pp(2) == pytest.approx(60000.00, abs=CENT)
    assert p.phase_open(2) == "ACCUM"
    assert p.lw_pp_at(2, "BEF_WD") == 0.0
    assert p.wd_excess_pp(2) == pytest.approx(60000.00, abs=CENT)   # LW = 0, all excess
    assert p.free_wd_allow(2) == pytest.approx(0.10 * p.av_pp(1), abs=CENT)
    assert p.wd_charge_base_pp(2) == pytest.approx(
        60000.00 - p.free_wd_allow(2), abs=CENT)
    assert p.wd_charge_pp(2) == pytest.approx(
        0.08 * p.wd_charge_base_pp(2), abs=CENT)                    # sc = 8% in year 3
    assert p.wd_clawback_pp(2) == pytest.approx(
        0.80 * (0.07 / 1.07) * p.wd_charge_base_pp(2), abs=CENT)    # v = 20% in year 3
    assert p.wd_mva_pp(2) < 0.0                                     # the yield has risen
    assert p.wd_reduction_rate(2) == pytest.approx(
        60000.00 / p.av_pp_at(2, "BEF_WD"), rel=1e-12)
    assert p.benefit_base_pp(2) == pytest.approx(
        p.benefit_base_pp_at(2, "BEF_WD") * (1 - p.wd_reduction_rate(2)), abs=CENT)
    assert p.rollup_base_pp(2) == pytest.approx(
        100000.00 * (1 - p.wd_reduction_rate(2)), abs=CENT)
    assert p.depletion_cause(2) is True                             # charge and MVA bit
...
def test_the_model_point_provenance_names_the_schedule_it_points_at(
    ...
    Point 8's ``wd_schedule_id`` is ``preexercise``, whose one row - keyed by the period
    index, so t = 2 for the withdrawal at anniversary 3 - is $60,000, the same
    ...
    assert p.wd_pp(2) == pytest.approx(amount, abs=CENT)
...
def test_the_free_withdrawal_amount_does_not_carry_forward(fixed_indexed_annuity):
    """10% of the opening account value each period, no carry-forward [S9][S10]."""
    p = fixed_indexed_annuity.Projection[8]
    assert p.free_wd_allow(0) == pytest.approx(0.10 * p.av_pp_at(0, "BEF_INV"),
                                               abs=CENT)
    for t in range(1, 10):
        assert p.free_wd_allow(t) == pytest.approx(0.10 * p.av_pp(t - 1), abs=CENT)
    assert p.free_wd_allow(3) < p.free_wd_allow(2)   # the year-3 withdrawal shrank it
...
def test_the_two_accounts_sum_to_the_account_value(fixed_indexed_annuity):
    ...
    assert p.fixed_interest_pp(0) == pytest.approx(0.20 * 107000.0 * 0.023, abs=CENT)
...
def test_the_stack_uses_the_gross_index_credit_not_the_starved_one(
    ...
    assert p.rollup_rate(0) == 0.0                 # pure stacking: g = 0
    assert p.rollup_pp(0) == 0.0
    ic, fi = p.index_credit_pp(0), p.fixed_interest_pp(0)
    assert p.stack_pp(0) == pytest.approx(2.50 * (ic + fi), abs=CENT)
    assert p.inv_income_pp(0) == pytest.approx(0.50 * ic + fi, abs=CENT)


def test_withdrawals_partition_into_the_three_ledger_lines(fixed_indexed_annuity):
    """withdrawals = guaranteed + excess + post-depletion income, in every period."""
    for point_id in fixed_indexed_annuity.Data.model_point_table().index:
        p = fixed_indexed_annuity.Projection[point_id]
        for t in range(p.entry_year(), p.proj_len()):
...
def test_internal_transfers_are_not_cash_flows(anchor):
    ...
    assert anchor.rider_charge_pp(7) > 0.0        # it exists, it just is not a ledger line
    assert anchor.net_cf(7) == pytest.approx(
        anchor.premiums(7) - anchor.withdrawals(7) - anchor.claims(7)
        - anchor.commissions(7) - anchor.expenses(7) - anchor.premium_taxes(7),
        abs=EXACT)
...
def test_expenses_follow_the_notes_ledger(fixed_indexed_annuity):
    ...
    With no issue-instant row the acquisition expense falls in period 0 beside that
    period's own maintenance charge, so the first row carries both.
    """
    p = fixed_indexed_annuity.Projection[2]
    assert p.expenses(0) == pytest.approx(
        0.06 * 100000.0 + 80.0 * p.pols_if(0), abs=CENT)
    assert p.expenses(1) == pytest.approx(80.0 * 1.025 * p.pols_if(1), abs=CENT)
    assert p.expenses(4) == pytest.approx(
        80.0 * 1.025 ** 4 * p.pols_if(4), abs=CENT)
...
def test_an_in_force_model_point_pays_no_premium(anchor, fixed_indexed_annuity):
    ...
    assert all(anchor.premiums(t) == 0.0
               for t in range(anchor.entry_year(), anchor.proj_len()))
    # Its first row carries maintenance only: the acquisition term keys off premiums(t).
    assert anchor.expenses(7) == pytest.approx(
        80.0 * 1.025 ** 7 * anchor.pols_if(7), abs=CENT)
...
def test_result_cf_shape(anchor):
    ...
    assert list(df.index) == list(range(anchor.entry_year(), anchor.proj_len()))
    ...
    assert df.loc[7, "wd_guar"] == pytest.approx(
        10144.16 * anchor.pols_if(7), abs=CENT)


def test_the_pols_if_column_is_the_weight_on_its_own_row(fixed_indexed_annuity):
    """pols_if(t) opens period t and is the divisor of that row's cash flows.
...
def test_the_withdrawals_column_is_published_beside_its_three_part_split(anchor):
    ...
    assert df.loc[7, "withdrawals"] == pytest.approx(10144.16 * anchor.pols_if(7),
                                                     abs=CENT)


def test_result_pols_opens_and_closes_each_row(anchor):
    """pols_if is the notes' l(t) and pols_if_aft_decr, its l(t+1), closes the row."""
...
def test_result_tables_are_indexed_consistently(anchor):
    for frame in (anchor.result_cf(), anchor.result_pols(), anchor.result_av(),
        ...
        assert list(frame.index) == list(
            range(anchor.entry_year(), anchor.proj_len()))
...
def test_invalid_timing_and_kind_arguments_raise(anchor):
    ...
    for call in (lambda: anchor.av_pp_at(7, "BOM"),
                 lambda: anchor.av_fixed_pp_at(7, "BOM"),
                 lambda: anchor.benefit_base_pp_at(7, "BOM"),
                 lambda: anchor.lw_pp_at(7, "BOM"),
                 lambda: anchor.pols_if_at(7, "BOM"),
                 lambda: anchor.av_at(7, "BOM")):
        raises_value_error(call, "invalid timing")
    for call in (lambda: anchor.claim_pp(8, "ANNUITIZATION"),
                 lambda: anchor.claim_from_av_pp(8, "ANNUITIZATION"),
                 lambda: anchor.pols_decr(8, "ANNUITIZATION")):
...
def test_the_timings_the_chassis_shares_still_coincide(anchor):
    ...
    assert anchor.pols_if_at(8, "BEF_MORT") == anchor.pols_if_at(8, "BEF_DECR")
    assert anchor.pols_if_at(8, "BEF_DECR") == anchor.pols_if(8)
    # ... and AFT_DECR is the notes' l(t+1), which opens the next period.
    assert anchor.pols_if_at(8, "AFT_DECR") == anchor.pols_if(9)


def test_maturity_is_confined_to_the_horizon(anchor):
    for t in range(anchor.entry_year(), anchor.proj_len() - 1):
        assert anchor.pols_maturity(t) == 0.0
    # pols_if opens the period, so nothing is left to open the one past the last.
...
def test_the_grid_is_annual_as_the_notes_state(fixed_indexed_annuity, anchor):
    ...
    event date."  The docstrings must say so, and each period must be a contract year:
    the anchor projects 52 rows to attained age 120, not 624.
    """
    assert "annual" in fixed_indexed_annuity.doc.lower()
    assert "contract year" in fixed_indexed_annuity.Projection.doc
    assert anchor.policy_year(7) == 8
    assert anchor.age(7) == anchor.age_at_entry() + 7
    assert anchor.proj_len() == 120 - 62 + 1
    assert len(anchor.result_cf()) == anchor.proj_len() - anchor.entry_year()
    assert len(anchor.result_cf()) == 52


def test_mortality_reads_the_age_entering_the_contract_year(anchor):
    """age(t) is the age at anniversary t, the age entering period t, so q(t) reads it.

    The horizon is the contract year *entered* at the mortality table's terminal age, so
    the last period carries q = 1.000000 and the projection closes itself.
    """
    table = anchor.data.mort_table()
    assert anchor.age(7) == 69                    # entering contract year 8
    assert anchor.exercise_age(7) == 70           # ... and 70 at its anniversary
    assert anchor.mort_rate(7) == pytest.approx(
