...
``t`` is the **period index and it is 0-based**: ``t = 0`` is the first contract year of a
model point projected from issue, period ``t`` runs from anniversary ``t`` to anniversary
``t + 1``, and the contractual label is ``policy_year(t) = t + 1``. The frame is
``t = entry_year() … proj_len() - 1``. Each period is a **contract year**, because the
technical notes make the anniversary the single event date: "All transactions occur **at**
the anniversary and are processed as the last events of the contract year ending there"
**[std]** — so every flow of period ``t`` falls at its closing anniversary ``t + 1``.
Contrast :mod:`.MYGA_US_S`, whose ``t`` counts months. The library's product
...
acquisition expense are beginning-of-period flows of period ``0`` on a new-issue model
point, whose first projected period is ``t = 0``. A model point may instead enter **in
force** after ``entry_year()`` completed contract years, on balances stated in
...
deferral" **[std]** rather than derived. ``entry_year()`` is an elapsed count and is
therefore also the index of the model point's first projected period: every recursion
opens there on the ``*_init`` cells, read through the period's opening timing
(``av_pp_at(t, "BEF_INV")``, ``benefit_base_pp_at(t, "BEF_ROLLUP")``, :func:`phase_open`,
:func:`rider_in_force_open`), and ``result_cf()`` is indexed from ``entry_year()``.
...
================================================  ================================  ========================================
t                                                 (the projection index)            Period index, 0-based; contract year t + 1
(contract year)                                   policy_year(t)                    Contract year containing t; = t + 1
x                                                 age_at_entry                      Issue age (ANB)
x + t                                             age(t)                            Attained age at anniversary t, opening period t
(younger covered life)                            covered_age(t)                    Younger covered age opening period t
x + t + 1                                         exercise_age(t)                   Covered age at the closing anniversary
(none)                                            policy_term                       Contract years from issue to maturity_age
(none)                                            proj_len                          Number of projected contract years
(in-force cell entry)                             entry_year                        Completed years at entry; first period index
...
R(t)                                              index_return(t)                   I(t+1)/I(t) - 1 over period t
...
IC(t)                                             index_credit_pp(t)                Index credit amount for period t
FI(t)                                             fixed_interest_pp(t)              Fixed account interest for period t
...
rider_in_force(t)                                 rider_in_force(t)                 Rider still alive closing period t [S9]
rider_in_force(t-1)                               rider_in_force_open(t)            Rider alive opening period t
depletion_cause(t)                                depletion_cause(t)                Excess / charge / negative MVA flag
q(t)                                              mort_rate(t)                      Annual mortality over period t, at age(t)
...
l(t)                                              pols_if(t)                        In-force at anniversary t, opening period t
l(t+1)                                            pols_if_at(t, "AFT_DECR")         In-force at the end of period t
l(0)                                              pols_if_init                      In-force at entry
l(t), ...                                         pols_if_at(t, timing)             BEF_DECR/BEF_MORT/BEF_LAPSE/AFT_DECR
...
0.06 P; 80 x 1.025^t                              expenses(t)                       Acquisition and maintenance
...
opens: on the 0-based period index the two coincide, ``pols_if(t) = l(t)`` with
``pols_if(entry_year()) = l(0) = 1`` for a new issue. The count *leaving* period ``t`` is
the notes' ``l(t+1)``, reachable as ``pols_if_at(t, "AFT_DECR")`` and equal to
``pols_if(t + 1)``. The reconciliation this buys is that the ``pols_if`` column of
:func:`result_cf` is the divisor of the cash flows on its own row —
``result_cf()["wd_guar"][t] / wd_guar_paid_pp(t)`` is ``pols_if(t)``.
...
``pi(70, single) = 5.20%``. :func:`exercise_age` carries that age — ``covered_age(t) + 1``
— so the ``+ 1`` is named once rather than spelled at each use.
...
``"BEF_PREM"``
    the balance carried into period ``t`` **before** the single premium: zero at ``t = 0``
    on a new-issue model point, and the same as ``"BEF_INV"`` everywhere else. It exists so
    that the account-value roll-forward closes on the first period too, the premium being a
    beginning-of-period flow rather than a row of its own.
``"BEF_INV"``
    ``AV(0)(t)``, the balance opening period ``t`` after the premium: ``av_pp_init()`` at
    the first projected period and ``AV(t-1)`` after it, before step 1.
``"BEF_FEE"``
...
Benefit base, named for the same steps: ``"BEF_ROLLUP"`` is the base opening period ``t``,
...
Policy counts, following the chassis: ``"BEF_DECR"`` is the notes' ``l(t)`` and therefore
equals :func:`pols_if` itself, ``"BEF_MORT"`` is the same (this product has no
annuitization decrement), ``"BEF_LAPSE"`` is after deaths, and ``"AFT_DECR"`` is the
notes' ``l(t+1)``, the closing count, which is also ``pols_if(t + 1)``.
...
``l(t+1) = l(t)(1 - q(t))``. ``TERMINATED`` is the same exhaustion with the guarantee
...
survivors of the last period leave through :func:`pols_maturity`, zero at every other
``t``, so that

    pols_if(t) - pols_if(t+1) = pols_death(t) + pols_lapse(t) + pols_maturity(t)

holds for every ``t``, including the last, ``t = proj_len() - 1``, where
``pols_if(proj_len())`` is zero. Under the shipped table ``q`` reaches 1.000000
...
def entry_year():
    ...
    An elapsed count, so it is also the index of the model point's **first projected
    period**: the frame is ``t = entry_year() … proj_len() - 1``.  ``0`` for a new issue,
    whose period ``0`` carries the premium, the acquisition expense and the notes'
    Initialisation block as its opening state and beginning-of-period flows.  A positive
...
def proj_len():
    """The number of contract years projected; on this annual grid, ``policy_term()``.

    The **exclusive end** of the frame counted from ``t = 0``: ``result_cf()`` runs
    ``t = entry_year() … proj_len() - 1``, so its last row is the contract year entered at
    attained age ``maturity_age``.
    """
    return policy_term()


def policy_year(t):
    """The contract year period t is: ``t + 1``, the contractual 1-based label.

    Period ``t`` runs from anniversary ``t`` to anniversary ``t + 1``, so it *is* contract
    year ``t + 1``.  Kept under the chassis name because it is the key that reads the
    surrender charge, the vesting vector and the rollup schedule, all of which the sources
    state by contract year.
    """
    return t + 1
...
def covered_age(t):
    """The attained age of the covered life opening period t.
...
def exercise_age(t):
    """``covered_age(t) + 1``: the covered age at the anniversary closing period t.

    All transactions occur **at** the anniversary and are processed as the last events of
    the contract year ending there **[std]**, so a lifetime withdrawal taken in period t is
    taken one year older than the age opening it.  This is the age that reads the payout
    percentage band and that clears the contractual minimum exercise age of 50: the anchor
    cell's exercise in period 7, from issue age 62, is at attained age 70 and reads the
    70-79 band.
    """
...
def scenario_value(t, name):
    """Step-function lookup of column ``name`` in the model point's scenario at anniversary t.

    The ``t`` column of *rate_scenario.csv* is an **anniversary** number, ``0`` at issue,
    and anniversary ``k`` is the one that opens period ``k`` — so the file's keys are
    already 0-based and are read unshifted.  Each row states the level that holds from its
    own anniversary until the next row of the same scenario, so a flat path is one row.
    Anniversaries before the first row take the first row's level.
    """
    sub = data.rate_scenario().loc[scenario_id()]                    # noqa: F821
    keys = [i for i in sub.index if i <= t]
    key = max(keys) if keys else min(sub.index)
    return float(sub.loc[key, name])


def index_level(t):
    """I(t): the index level at anniversary t, the anniversary opening period t.

    A time-point reading, not a period quantity: the return over period ``t`` differences
    this level against ``index_level(t + 1)``, the level at the anniversary that closes it.
...
def index_return(t):
    """R(t) = I(t+1)/I(t) - 1: the point-to-point return over period t.

    From the anniversary that opens the period to the one that closes it, which is where
    the annual segment matures.
    """
    prior = index_level(t)
    if prior <= 0.0:
        return 0.0
    return index_level(t + 1) / prior - 1.0
...
def credit_rate(t):
    """cr(t): the credit rate applied to the indexed account over period t."""
    return credit_rate_on(index_return(t), credit_method())


def index_credit_pp(t):
    """IC(t) = A(0)(t) x cr(t): the index credit on the segment maturing at the end of t.

    One annual segment per indexed account, created at the anniversary opening period
    ``t`` with the balance opening it and maturing at the anniversary closing it; the
    credit locks at maturity and cannot be lost to later declines [S1].  The credit base is
    the segment's opening balance less withdrawals from that account during the segment —
    the "Interest Credit Basis" [S6] — which collapses to the opening balance here because
    every transaction happens at an anniversary **[std]**.  Zero in ``DEPLETED`` and
    ``TERMINATED``, where steps 1-3 are skipped.
    """
    if t < entry_year():
        return 0.0
    if phase_open(t) in ("DEPLETED", "TERMINATED"):
        return 0.0
    return av_indexed_pp_at(t, "BEF_INV") * credit_rate(t)


def fixed_interest_pp(t):
    """FI(t) = F(0)(t) x i_F: interest on the fixed account over period t."""
    if t < entry_year():
        return 0.0
    if phase_open(t) in ("DEPLETED", "TERMINATED"):
        return 0.0
    return av_fixed_pp_at(t, "BEF_INV") * fixed_rate_in_force()


def inv_income_pp(t):
    """Interest credited to one contract's account value over period t.
...
def rider_charge_pp(t):
    """Phi(t) = phi x BB(0)(t): the GLWB rider charge deducted in period t [S9].
...
def av_pp_at(t, timing):
    """AV per contract in period t, read at the point given by ``timing``.

    ``"BEF_PREM"`` is the balance carried into the period before the single premium, zero
    at ``t = 0`` on a new issue; ``"BEF_INV"`` is the balance opening the period after it,
    which is ``av_pp_init()`` at the first projected period and ``AV(t-1)`` after that;
    ``"BEF_FEE"`` is after the index credit and fixed interest; ``"BEF_WD"`` is after the
    rider charge; ``"EOY"`` is after the withdrawal, at the closing anniversary.
    ...
    if t < entry_year():
        return av_pp_init()
    if timing == "BEF_PREM":
        return 0.0 if t == entry_year() == 0 else av_pp_at(t, "BEF_INV")
    opening = av_pp_init() if t == entry_year() else av_pp(t - 1)
    if timing == "BEF_INV":
        return opening
...
def av_pp(t):
    """AV(t): the account value per contract closing period t, after all processing."""
    return av_pp_at(t, "EOY")


def av_fixed_pp_at(t, timing):
    """F per contract in period t; see :func:`av_pp_at` for ``timing``.
    ...
    if timing == "BEF_PREM":
        return 0.0 if t == entry_year() == 0 else av_fixed_pp_at(t, "BEF_INV")
    opening = av_fixed_pp_init() if t == entry_year() else av_fixed_pp(t - 1)
    if timing == "BEF_INV":
        return opening
...
def av_fixed_pp(t):
    """F(t): the fixed account balance per contract closing period t."""
    return av_fixed_pp_at(t, "EOY")


def av_indexed_pp_at(t, timing):
    """A per contract in period t: ``AV - F`` at the same timing, by construction."""
    return av_pp_at(t, timing) - av_fixed_pp_at(t, timing)


def av_indexed_pp(t):
    """A(t): the indexed account balance per contract; the credit base for period t + 1."""
    return av_indexed_pp_at(t, "EOY")


def av_depletion_pp(t):
    """The part of period t's withdrawal the account value could not fund.
...
def surr_charge_rate(t):
    """sc(t): the surrender charge percentage in period t's contract year [S5].
    ...
    contractual 1-based contract year, which is ``policy_year(t) = t + 1``.
    """
...
def vest_rate(t):
    """v(t): the vested percentage of the premium bonus in period t's contract year [S5].

    0, 10, ..., 100% over eleven contract years, read at ``policy_year(t) = t + 1``.  On
...
def rollup_rate(t):
    """g(t): the guaranteed simple rollup rate in period t's contract year [S2][S9].

    Read as a step function of the contract year ``policy_year(t) = t + 1`` from the model
...
def payout_rate_locked(t):
    """The payout percentage in force in period t.

    Locked at the attained age of the **first** lifetime withdrawal, :func:`exercise_age`,
    and not re-read afterwards **[std]**.  Documented alternatives: one carrier reads the
    band from the age at the most recent anniversary, letting it step up [S3]; another
...
def is_exercise(t):
    """True in the period whose closing anniversary carries the **first** lifetime withdrawal.

    Requires an in-force rider, the ``ACCUM`` phase and an :func:`exercise_age` at or above
    both the contractual minimum of 50 [S2][S3][S9] and the model point's
    ``income_start_age``.  It depends only on the state **opening** the period, which is
    what lets the phase, the benefit base and the withdrawal be evaluated without a
    circular reference.
    """
    if t < entry_year():
        return False
    if not glwb_elected():
        return False
    prev = phase_init() if t == entry_year() else phase(t - 1)
    if prev != "ACCUM":
        return False
    if not rider_in_force_open(t):
        return False
...
def phase_open(t):
    """The phase steps 1-7 of period t are processed under.

    The phase opening the period — ``phase_init()`` at the first projected period and the
    closing phase of ``t - 1`` after it — promoted to ``INCOME`` when the first lifetime
    withdrawal is taken in the period.  Steps 1-3 are skipped when this is ``DEPLETED`` and
    steps 1-7 when it is ``TERMINATED``.
    """
    if t < entry_year():
        return phase_init()
    prev = phase_init() if t == entry_year() else phase(t - 1)
...
def depletion_cause(t):
    """True when a charge, an excess withdrawal or a negative MVA touched the AV in period t.
...
def phase(t):
    """The phase closing period t, after step 7's transition.
...
def rider_in_force_open(t):
    """Whether the GLWB rider is alive **opening** period t.

    ``glwb_elected()`` at the first projected period — an in-force model point enters on
    the rider state its row states — and :func:`rider_in_force` of the previous period
    after that.  Kept as a cells of its own, like :func:`phase_open`, because four
    formulas need the opening rider state and none of them may reach for a closing one.
    """
    if t <= entry_year():
        return bool(glwb_elected())
    return rider_in_force(t - 1)


def rider_in_force(t):
    """Whether the GLWB rider is still alive at the end of period t [S9].
...
def in_growth_period(t):
    """T_g: whether the benefit base still rolls up and stacks in period t.

    To the earlier of the first lifetime withdrawal and contract year 20 [S1][S2].  The
    period of exercise itself is still inside the window — the worked example credits
    both the rollup and the stack in period 7, the year income starts — which is why the
    test is on the phase *opening* the period.
    """
    if t < entry_year():
        return False
    if not glwb_elected():
    ...
    if policy_year(t) > growth_period_max:                           # noqa: F821
        return False
    return (phase_init() if t == entry_year() else phase(t - 1)) == "ACCUM"


def step_up_applies(t):
    """Whether the annual step-up ``BB <- max(BB, AV)`` is tested in period t.
    ...
    *rarely is not never*: on a new issue the bonus opens period 0 with an account value of
    ``107,000`` against a benefit base of ``100,000``, so a first contract year with a zero
    index credit gives ``AV(2)(0) = 106,050`` against ``BB(3)(0) = 105,000`` and the
    step-up binds.
    """
    if not glwb_elected():
        return False
...
def rollup_pp(t):
    """rollup(t) = g(t) x RB(0)(t): the guaranteed rollup increment in period t.
    ...
    **A flat dollar increment, not simple interest on the grown base.** One carrier
    ...
    if not in_growth_period(t):
        return 0.0
    opening = (rollup_base_pp_init() if t == entry_year()
               else rollup_base_pp(t - 1))
    return rollup_rate(t) * opening


def stack_pp(t):
    """stack(t) = m x max(0, IC(t) + FI(t)): the stacking credit in period t.
...
def benefit_base_pp_at(t, timing):
    """BB per contract in period t, read at the point given by ``timing``.

    ``"BEF_ROLLUP"`` is the base opening the period — ``benefit_base_pp_init()`` at the
    first projected period and ``BB(t-1)`` after it; ``"BEF_STEP_UP"`` is after the rollup
    and the stack; ``"BEF_WD"`` is after the annual step-up and is the base the lifetime
    withdrawal is computed on; ``"EOY"`` is after step 5's proportional reduction.  The
    base is **notional**: it has no cash value, cannot be withdrawn and cannot be taken as
    a lump sum [S1][S9].
    """
    if t < entry_year():
    ...
    if not glwb_elected() or not rider_in_force_open(t):
        return 0.0
    opening = (benefit_base_pp_init() if t == entry_year()
               else benefit_base_pp(t - 1))
...
def benefit_base_pp(t):
    """BB(t): the GLWB benefit base per contract closing period t."""
    return benefit_base_pp_at(t, "EOY")


def rollup_base_pp(t):
    """RB(t): the rollup base closing period t.
    ...
    at another carrier, ``RB(t) = max(0, RB(0)(t) - G(t))`` — "Premium minus
    Withdrawals" [S2].
    """
    if t < entry_year():
...
def lw_pp_at(t, timing):
    """LW per contract in period t, read at the point given by ``timing``.
    ...
    ``LW = pi x BB(4)``; afterwards the ratchet ``LW = max(LW(0)(t), pi x BB(4))`` applies
    ...
    if timing not in ("BEF_WD", "EOY"):
        raise ValueError("invalid timing")
    prior = lw_pp_init() if t == entry_year() else lw_pp(t - 1)
    opening = phase_open(t)
    if opening == "TERMINATED":
        val = 0.0
...
def lw_pp(t):
    """LW(t): the lifetime withdrawal amount per contract closing period t."""
    return lw_pp_at(t, "EOY")


def wd_scheduled_pp(t):
    """The ad hoc gross withdrawal scheduled for period t, else zero.

    Read from *withdrawal_table.csv*, which is **not** a step function: a period with no
    row takes nothing.  Its key column is the period index, so a row keyed ``t`` is a
    withdrawal at the anniversary closing period ``t``.  Lifetime withdrawals are generated
    by the rider, not scheduled here.
    """
...
def wd_pp(t):
    """G(t): the gross withdrawal taken at the anniversary closing period t.
...
def wd_guar_pp(t):
    """min(G(t), LW(t)): the guaranteed portion of period t's withdrawal.
...
def wd_excess_pp(t):
    """E(t) = max(0, G(t) - LW(t)): the **excess withdrawal** in period t.
...
def wd_unfunded_pp(t):
    ...
    ``G(t) - min(G(t), max(0, AV(2)(t)))`` in a period whose closing phase is
...
def wd_guar_paid_pp(t):
    """The guaranteed portion of period t's withdrawal that is actually paid.
    ...
    the account value cannot fund.  Equal to :func:`wd_guar_pp` in every period except a
    terminating one.
    """
...
def wd_excess_paid_pp(t):
    """The excess portion of period t's withdrawal that is actually paid.
    ...
    :func:`wd_excess_pp` in every period except a terminating one.
    """
    return max(0.0, wd_excess_pp(t) - wd_unfunded_pp(t))


def wd_cum_pp(t):
    """Wcum(t): cumulative gross withdrawals from entry to the end of period t."""
    if t < entry_year():
        return 0.0
    prior = 0.0 if t == entry_year() else wd_cum_pp(t - 1)
    return prior + wd_pp(t)


def free_wd_base(t):
    """The base of the free withdrawal amount: the account value opening period t.

    10% of the account value at the anniversary that opens the period, available from
    contract year 1 and with no carry-forward [S1][S3][S5][S6][S9][S10]; the combination
    is **[std]**.  Fixing the base at a *known* prior value is what lets the chargeable
    amount be evaluated without a fixed point.
    """
    return av_pp_at(t, "BEF_INV")


def free_wd_allow(t):
    """FW(t) = 0.10 x AV(0)(t): the free withdrawal amount for period t."""
    return free_wd_rate * free_wd_base(t)                            # noqa: F821


def wd_free_pp(t):
    """The free-allowance portion of period t's withdrawal: the part of FW(t) it uses.
...
def free_wd_remain(t):
    """The free withdrawal amount left after the period's guaranteed withdrawal."""
    return free_wd_allow(t) - wd_free_pp(t)


def wd_charge_base_pp(t):
    """X(t): the part of period t's withdrawal exposed to charge, clawback and MVA.
...
def surr_charge_base_pp(t):
    """X(t) at a **full surrender** closing period t: ``AV(t)`` less the free amount left.
...
def mva_ref_yield(t):
    ...
    A time-point reading like :func:`index_level`, so the level in force at a transaction
    closing period ``t`` is ``mva_ref_yield(t + 1)``.  A declared investment-grade
    corporate bond yield index; generic here **[std]**, and named as Barclay's US Credit
    Index in the linear-form products [S6][S7].
    """
    return scenario_value(t, "mva_ref_yield")


def mva_term(t):
    """n/12: the years remaining in the MVA period at the anniversary closing period t.
...
def mva_in_force(t):
    """Whether an MVA applies to a transaction closing period t.
...
def mva_rate(t):
    ...
    return ((1.0 + mva_ref_yield_at_issue())
            / (1.0 + mva_ref_yield(t + 1))) ** mva_term(t) - 1.0


def mva_pp_on(t, base, gross):
    """The collared MVA on a currency ``base`` withdrawn as ``gross`` in period t.
...
def wd_charge_pp(t):
    """SC(t) on period t's withdrawal: ``sc(t) x X(t)`` [S5][S10]."""
    return surr_charge_rate(t) * wd_charge_base_pp(t)


def wd_clawback_pp(t):
    """CB(t) on period t's withdrawal [S10]."""
    return bonus_clawback_on(wd_charge_base_pp(t), vest_rate(t), bonus_rate())


def wd_mva_pp(t):
    """MVA(t) on period t's withdrawal, collared [S10]."""
    return mva_pp_on(t, wd_charge_base_pp(t), wd_pp(t))


def wd_payment_pp(t):
    """The cash paid on period t's withdrawal, ``G - unfunded - SC - CB + MVA``.
    ...
    :func:`wd_unfunded_pp` is zero in every period except a terminating one, where
...
def wd_reduction_rate(t):
    """rho(t): the proportional reduction applied to BB, LW and RB in period t.
...
def surr_charge_pp(t):
    """SC(t) on a full surrender closing period t."""
    return surr_charge_rate(t) * surr_charge_base_pp(t)


def surr_clawback_pp(t):
    """CB(t) on a full surrender closing period t [S10]."""
    return bonus_clawback_on(surr_charge_base_pp(t), vest_rate(t), bonus_rate())


def mva_pp(t):
    """MVA(t) on a full surrender closing period t, collared [S10]."""
...
def mgsv_pp(t):
    """MGV(t): the Model #805 guaranteed minimum value closing period t.

    ``MGV(t) = max(0, MGV(0)(t) x (1 + i_nf) - G(t))`` on the value opening the period,
    which is ``0.875 x P`` excluding the bonus at issue [S10][R2].  **Accrete, then
    deduct** — the
    ...
    if t < entry_year():
        return mgsv_pp_init()
    opening = mgsv_pp_init() if t == entry_year() else mgsv_pp(t - 1)
    charge = rider_charge_pp(t) if rider_charge_from_mgsv else 0.0    # noqa: F821
    return max(0.0, opening * (1.0 + mgsv_rate)                      # noqa: F821
...
def mort_rate(t):
    """q(t): the annual mortality rate over period t, read at ``age(t)``.
...
def lapse_rate_base(t):
    """w_base(t): the base surrender rate for period t's contract year **[std]**.
...
def lapse_rate(t):
    ...
    the period the contract terminates, the survivors leave as a deemed full
...
def pols_if(t):
    ...
    the probability in force at the end of contract year ``t``; the count *leaving* the
    period is ``pols_if_at(t, "AFT_DECR")``, the notes' ``l(t+1)``, equivalently
    ``pols_if(t + 1)``.  See the Space docstring.
    ...
    **[std]**, and it is zero from ``proj_len()`` on, where the survivors of the last
    projected contract year have left through :func:`pols_maturity`.
    """
    if t <= entry_year():
        return pols_if_init()
    if t >= proj_len():
...
def pols_if_at(t, timing):
    """In-force in period t read at the point given by ``timing``.

    ``"BEF_DECR"`` and ``"BEF_MORT"`` are both the notes' ``l(t)``, which is
    :func:`pols_if` itself — this product has no annuitization decrement, so the chassis's
    two timings coincide.  ``"BEF_LAPSE"`` is after deaths, and ``"AFT_DECR"`` is the
    notes' ``l(t+1)``, the count at the *end* of period t and therefore ``pols_if(t + 1)``.
    """
    if t < entry_year():
        return pols_if_init()
    pols = pols_if(t)
...
def pols_lapse(t):
    """Full surrenders at the end of period t, on the survivors of mortality."""
...
def pols_decr(t, kind):
    """The number of contracts leaving in period t by benefit ``kind``."""
    if kind == "DEATH":
        return pols_death(t)
    elif kind == "LAPSE":
        return pols_lapse(t)
...
def claim_pp(t, kind):
    """The benefit paid per contract at the end of period t by ``kind``.
...
def premiums(t):
    """Premium income: the single premium, at the **start** of period 0 on a new issue.

    There is no issue-instant row, so the premium is a beginning-of-period flow of the
    first projected period rather than a row of its own.  An in-force model point paid its
...
def wd_guar(t):
    """Guaranteed withdrawal outgo in period t, weighted by ``pols_if(t)``.

    The notes' ``l(t)``: the count entering the period, which is the in-force number
    reported on the same row of :func:`result_cf`.
    On :func:`wd_guar_paid_pp`, not on the amount requested, so a terminating period
...
def wd_excess(t):
    """Excess withdrawal outgo in period t: ``E - unfunded - SC - CB + MVA``.

    Weighted by ``pols_if(t)``, the notes' ``l(t)``, and on :func:`wd_excess_paid_pp`:
...
def withdrawals(t):
    """Total withdrawal outgo in period t, ``wd_payment_pp(t) x pols_if(t)``.

    Equal to :func:`wd_guar` + :func:`wd_excess` + :func:`income_payments` in every
    period, the three lines partitioning it by the notes' ledger categories.  All
...
def claims(t, kind=None):
    """Benefit outgo in period t, for one ``kind`` or, with ``kind=None``, all three."""
    if kind is None:
...
def inflation_factor(t):
    """The expense inflation factor in period t, ``1.025^t``."""
...
def expenses(t):
    ...
    6.0% of the single premium at the start of period 0 on a new issue, and $80 per
    contract per year inflating at 2.5%, weighted by ``l(t)``.  Both fall in the same
    period on a new-issue model point, the acquisition expense being a beginning-of-period
    flow rather than a row of its own; ``premiums(t)`` is zero everywhere else, so the
    first term simply drops out.  Both are standardizations: no retrieved document
    ...
    return (expense_acq_rate * premiums(t)                           # noqa: F821
            + expense_maint * inflation_factor(t) * pols_if(t))      # noqa: F821
...
def av_at(t, timing):
    """The in-force weighted account value in period t; see :func:`av_pp_at`.

    Every timing inside the period carries ``pols_if(t)`` — the notes' ``l(t)``, the count
    entering the contract year — because all eight processing steps happen before the
    decrements.  ``"EOY"`` carries the count *leaving* it, the notes' ``l(t+1)``, which is
    ``pols_if(t + 1)``: zero at the horizon, where :func:`pols_maturity` has taken the
    survivors out.
    """
    if t < entry_year():
        return 0.0
...
def inv_income(t):
    """Interest credited to the whole block in period t."""
    return 0.0 if t < entry_year() else inv_income_pp(t) * pols_if(t)


def rider_charges(t):
    """Rider charges deducted from the whole block's account value in period t."""
    return 0.0 if t < entry_year() else rider_charge_pp(t) * pols_if(t)


def wd_from_av(t):
    """The account value released by period t's withdrawals, gross of charge and MVA."""
    return 0.0 if t < entry_year() else wd_pp(t) * pols_if(t)


def av_depletion(t):
    """The block's withdrawal outgo the account value could not fund in period t."""
    return 0.0 if t < entry_year() else av_depletion_pp(t) * pols_if(t)


def av_change(t):
    """The change in the block's account value over period t.

    Closing less **opening**, the opening being read at ``"BEF_PREM"`` so that the single
    premium of a new issue's period 0 is a movement rather than part of the balance
    brought in.  Away from the first period ``av_at(t, "BEF_PREM")`` is exactly the
    previous period's closing value.
    """
    return av_at(t, "EOY") - av_at(t, "BEF_PREM")


def check_av_roll_fwd_resid(t):
    """Account value roll-forward residual for period t; the signed float.
...
def check_av_roll_fwd():
    ...
    for a single period is :func:`check_av_roll_fwd_resid`.
    ...
    """
    res = []
    for t in range(entry_year(), proj_len()):
...
def check_pols_roll_fwd_resid(t):
    """In-force roll-forward residual for period t; the signed float.

    ``pols_if(t) - pols_if(t+1) = deaths + surrenders + horizon exits``, which in the
    notes' own symbols is ``l(t) - l(t+1)``.  Zero to floating point for every ``t``,
    including the last, ``t = proj_len() - 1``, where ``pols_if(proj_len())`` is zero and
...
def check_pols_roll_fwd():
    ...
    single period is :func:`check_pols_roll_fwd_resid`.
    """
    res = []
    for t in range(entry_year(), proj_len()):
...
def result_cf():
    """Result table of cashflows, indexed by the period ``t`` from ``entry_year()``.

    The frame runs ``t = entry_year() … proj_len() - 1``, one row per projected contract
    year and **no issue-instant row**: the first row of a new-issue model point carries
    the premium and the acquisition expense as beginning-of-period flows alongside that
    year's own activity.

    ``pols_if`` is the in-force count at the **start** of period ``t``, which is
    ...
    **They are published alongside it, not instead of it, so summing every column of the
    ...
    in this library.
    """
    ts = list(range(entry_year(), proj_len()))
...
def result_pols():
    """Result table of in-force movements, indexed by the period t.

    ``pols_if`` opens the row and ``pols_if_aft_decr`` closes it.  The closing column is
    the technical notes' own ``l(t+1)``, and is the opening figure of the next row in every
    period but the last, where :func:`pols_maturity` takes the survivors out and the
...
def result_av():
    ...
    its surrender trace, available in every period rather than only at the anniversary the
    notes tabulate.
    """
    ts = list(range(entry_year(), proj_len()))
    return pd.DataFrame(                                             # noqa: F821
...
def result_glwb():
    """Result table of the GLWB rider state per contract, indexed by the period t.
