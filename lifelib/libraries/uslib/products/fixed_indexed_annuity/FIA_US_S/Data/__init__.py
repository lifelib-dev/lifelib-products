...
and anniversary ``t`` is the one that opens period ``t``. The projection differences
consecutive levels into ``R(t) = I(t+1)/I(t) - 1``, the return over period ``t``, so
holding a level flat is how a scenario says "no index credit this year".
``withdrawal_table`` is indexed by ``(wd_schedule_id, t)``, there the **period** index, and
is *not* a step function — a period with no row simply takes no ad hoc withdrawal.
...
def rate_scenario():
    ...
    issue — anniversary ``t`` opens period ``t``, so the file's keys are already 0-based
    and are read unshifted.  A step function: each row states the index level and the MVA
    reference yield that hold from that anniversary until the next row of the same
    scenario.
    """
    return pd.read_csv(                                              # noqa: F821
...
def withdrawal_table():
    ...
    Indexed by ``(wd_schedule_id, t)``, where ``t`` is the projection's **period** index:
    a row keyed ``t`` is withdrawn at the anniversary closing period ``t``.  A period with
    no row takes no ad hoc withdrawal.  Lifetime withdrawals are *not* scheduled here —
